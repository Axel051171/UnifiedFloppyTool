...
#include "TrackAnalyzerWidget.h"  /* NEW: XCopy Pro Track Analyzer */
...
void UftMainWindow::setupCentralWidget()
{
    ...
    /* Create Track Analyzer Widget (NEW - XCopy Pro) */
    m_trackAnalyzer = new TrackAnalyzerWidget(this);
    
    /* Add as new tab or dock widget */
    ui->mainTabs->addTab(m_trackAnalyzer, "🔍 Track Analyzer");
}
...
void UftMainWindow::setupConnections()
{
    ...
    /* Track Analyzer <-> XCopy Panel integration (NEW - XCopy Pro) */
    connect(m_trackAnalyzer, &TrackAnalyzerWidget::applyToXCopy,
            m_xcopyPanel, &UftXCopyPanel::applyAnalysisResults);
    
    connect(m_trackAnalyzer, &TrackAnalyzerWidget::analysisComplete,
            this, &UftMainWindow::onAnalysisComplete);
    
    connect(m_xcopyPanel, &UftXCopyPanel::requestQuickScan,
            m_trackAnalyzer, &TrackAnalyzerWidget::runQuickScan);
    
    connect(m_xcopyPanel, &UftXCopyPanel::requestAnalysis,
            m_trackAnalyzer, &TrackAnalyzerWidget::runFullAnalysis);
    
    /* Track Analyzer quick scan complete -> update XCopy panel */
    connect(m_trackAnalyzer, &TrackAnalyzerWidget::quickScanComplete,
            this, [this](const QuickScanResult &result) {
        if (result.protectionDetected) {
            m_xcopyPanel->setAnalysisInfo(1, result.protectionName);
            
            /* Show notification */
            m_statusLabel->setText(QString("⚠ Protection detected: %1 - Recommended: %2")
                .arg(result.protectionName)
                .arg(result.recommendedMode == CopyModeRecommendation::NibbleCopy ? "Nibble Copy" :
                     result.recommendedMode == CopyModeRecommendation::FluxCopy ? "Flux Copy" :
                     result.recommendedMode == CopyModeRecommendation::TrackCopy ? "Track Copy" : "Normal"));
        }
    });
}
...
/* ============================================================================
 * Track Analysis (XCopy Pro Algorithms)
 * ============================================================================ */

void UftMainWindow::onQuickScan()
{
    m_statusLabel->setText("Quick scanning...");
    m_trackAnalyzer->runQuickScan();
    
    /* Switch to Track Analyzer tab */
    for (int i = 0; i < ui->mainTabs->count(); i++) {
        if (ui->mainTabs->widget(i) == m_trackAnalyzer) {
...
void UftMainWindow::onFullAnalysis()
{
    m_statusLabel->setText("Analyzing all tracks...");
    m_progressBar->setVisible(true);
    m_progressBar->setValue(0);
    
    m_trackAnalyzer->runFullAnalysis();
...
void UftMainWindow::onAnalysisComplete(int tracksAnalyzed, int protectedTracks)
{
    m_progressBar->setVisible(false);
    
    if (protectedTracks > 0) {
        m_statusLabel->setText(QString("Analysis complete: %1 tracks, %2 protected")
            .arg(tracksAnalyzed)
            .arg(protectedTracks));
        
        /* Ask user if they want to apply recommended settings */
        int result = QMessageBox::question(this, "Protection Detected",
            QString("<b>%1 protected tracks detected!</b><br><br>"
                    "Would you like to apply the recommended copy settings?")
                .arg(protectedTracks),
            QMessageBox::Yes | QMessageBox::No);
        
        if (result == QMessageBox::Yes) {
            m_trackAnalyzer->applySettings();
            
            /* Switch to XCopy tab */
            ui->mainTabs->setCurrentWidget(ui->tabXCopy);
        }
    } else {
        m_statusLabel->setText(QString("Analysis complete: %1 tracks, no protection detected")
            .arg(tracksAnalyzed));
    }
}
