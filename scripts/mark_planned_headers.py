...
MARKER_PARTIAL = "UFT_SKELETON_PARTIAL"
 ...
 *
 * This header declares {{decls}} public functions; {{missing}} are NOT implemented
 * in the source tree (only {{implemented}} have a definition). Callers exist
...
def insert_banner(header_path: Path, scope: str, decls: int, missing: int,
                  bucket: str) -> bool:
    text = header_path.read_text(encoding="utf-8", errors="replace")
    if MARKER in text or MARKER_PARTIAL in text:
        return False  # already marked

    if bucket == "IMPLEMENT":
        banner = BANNER_PARTIAL.format(
            scope=scope, decls=decls, missing=missing,
            implemented=decls - missing,
        )
    else:
...
def main() -> int:
    ...
    ap.add_argument("--bucket", choices=("DOCUMENT", "IMPLEMENT", "BOTH"),
                    default="DOCUMENT")
    args = ap.parse_args()
    wanted = {"DOCUMENT", "IMPLEMENT"} if args.bucket == "BOTH" else {args.bucket}

    rows: list[tuple[int, int, str, str]] = []
    with TRIAGE.open("r", encoding="utf-8") as fh:
        reader = csv.DictReader(fh)
        for row in reader:
            if row["bucket"] not in wanted:
                continue
            rows.append((int(row["decls"]), int(row["missing"]),
                         row["path"], row["bucket"]))
    ...
    for decls, missing, rel, bucket in rows:
        p = ROOT / rel
        if not p.exists():
        ...
        by_scope[f"{bucket}/{scope}"].append((decls, missing, rel))
        if insert_banner(p, scope, decls, missing, bucket):
            modified += 1
        else:
            skipped_already_marked += 1

    print(f"Headers processed             : {len(rows)} (buckets: {sorted(wanted)})")
    ...
    with appendix_path.open("w", encoding="utf-8") as fh:
        fh.write("# Planned APIs (MF-011 DOCUMENT+IMPLEMENT-Wellen)\n\n")
        fh.write(f"**Stand:** auto-generated, {len(rows)} headers · "
                 f"{sum(m for _, m, _, _ in rows)} unimplemented declarations\n\n")
        fh.write("Every header listed below declares public `uft_*` functions "
                 ...
                 "in `src/`. DOCUMENT-bucket files have a `/* PLANNED FEATURE */` "
                 "banner (zero impls); IMPLEMENT-bucket files have a "
                 "`/* PARTIALLY IMPLEMENTED */` banner (some impls, some stubs). "
                 "Consumers are warned before adding new call sites. "
