...
static int woz_parse_trks_v1(woz_context_t *ctx, uint32_t size) {
    ...
    for (int t = 0; t < max_tracks; t++) {
        ...
        if (fseek(ctx->fp, track_offset + 6646, SEEK_SET) != 0) { /* I/O error */ }
...
static int woz_parse_trks_v2(woz_context_t *ctx, uint32_t size) {
    ...
    for (int t = 0; t < track_entries; t++) {
        ...
        if (fseek(ctx->fp, ctx->trks_offset + (t * WOZ2_TRKS_ENTRY), SEEK_SET) != 0) { /* I/O error */ }
...
woz_context_t* woz_open(const char *filename) {
    ...
    /* Get file size */
    if (fseek(ctx->fp, 0, SEEK_END) != 0) { /* I/O error */ }
    ctx->file_size = (size_t)ftell(ctx->fp);
    if (fseek(ctx->fp, 0, SEEK_SET) != 0) { /* I/O error */ }
    ...
    while (ftell(ctx->fp) < (long)ctx->file_size - 8) {
        ...
        /* Move to next chunk */
        if (fseek(ctx->fp, chunk_start + chunk.chunk_size, SEEK_SET) != 0) { /* I/O error */ }
    }
...
int woz_read_track(woz_context_t *ctx, int quarter_track, 
    ...
    if (fseek(ctx->fp, track->file_offset, SEEK_SET) != 0) { /* I/O error */ }
