...
int uft_floppy_write_sector(FloppyDevice*dev,uint32_t t,uint32_t h,uint32_t s,const uint8_t*buf){
    ...
    (void)fseek(ctx->fp,(long)s,SEEK_SET); fputc(*buf,ctx->fp); fflush(ctx->fp); return OK; }
int uft_floppy_analyze_protection(FloppyDevice*dev){ logm(dev,"Analyzer(TIFILES): parse header fields next."); return OK; }
