...
typedef struct {
    ...
    /* Soft decision info */
    hxc_soft_bit_t* soft_data;      /* Soft bits for each data bit */
...
typedef struct {
    ...
    /* Format detection */
    hxc_format_detect_t detected_format;
    bool format_locked;
...
static void kalman_pll_init(hxc_kalman_pll_t* pll, double nominal_period, double bandwidth) {
...
static double kalman_pll_update(hxc_kalman_pll_t* pll, double measurement) {
    /* Predict step */
...
typedef struct {
    hxc_viterbi_state_t states[HXC_V3_VITERBI_STATES];
    hxc_viterbi_state_t new_states[HXC_V3_VITERBI_STATES];
    int traceback_depth;
    uint8_t* history;
    size_t history_size;
} hxc_viterbi_ctx_t;

static void viterbi_init(hxc_viterbi_ctx_t* ctx, int depth) {
...
static void viterbi_free(hxc_viterbi_ctx_t* ctx) {
    free(ctx->history);
}
 ...
 */
static float viterbi_process(hxc_viterbi_ctx_t* ctx, 
                             const hxc_soft_bit_t* soft_bits,
                             uint8_t* output) {
 ...
 */
static void detect_weak_bits_v3(hxc_track_v3_t* track, float threshold) {
    if (track->revolution_count < 2) return;
    
    track->total_weak_bits = 0;
    
    for (size_t i = 0; i < track->soft_bit_count; i++) {
        hxc_soft_bit_t* bit = &track->soft_bits[i];
...
typedef struct {
    ...
    uint8_t flags;
    float confidence;
} hxc_protection_sig_t;

static hxc_protection_sig_t known_protections[] = {
...
static void detect_protection_v3(hxc_track_v3_t* track, const hxc_decoder_v3_t* dec) {
...
static void detect_format_v3(hxc_track_v3_t* track, hxc_format_detect_t* result) {
 ...
 */
static int flux_to_soft_bits(hxc_track_v3_t* track, const hxc_config_v3_t* config) {
    ...
    track->soft_bits = calloc(max_bits, sizeof(hxc_soft_bit_t));
 ...
 */
static int soft_bits_to_sectors(hxc_track_v3_t* track, const hxc_config_v3_t* config) {
    ...
    while (pos + 1000 < track->bit_count && track->sector_count < HXC_V3_MAX_SECTORS) {
        ...
        hxc_sector_v3_t* sector = &track->sectors[track->sector_count];
...
hxc_decoder_v3_t* hxc_v3_create(const hxc_config_v3_t* config) {
    hxc_decoder_v3_t* dec = calloc(1, sizeof(hxc_decoder_v3_t));
    if (!dec) return NULL;
    
    if (config) {
        memcpy(&dec->config, config, sizeof(hxc_config_v3_t));
    } else {
        hxc_v3_config_init(&dec->config);
    }
...
int main(void) {
    printf("=== hxc_decoder_v3 Unit Tests ===\n");
    ...
    {
        hxc_decoder_v3_t* dec = hxc_v3_create(NULL);
        assert(dec != NULL);
