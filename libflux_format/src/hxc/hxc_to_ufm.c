...
/*
 * hxc_to_ufm.c - HXC to UFM Converter
 * 
...
int hxc_hfe_to_ufm(
    ...
    for (uint32_t i = 0; i < hfe->track_count; i++) {
        const hxc_track_t *hfe_track = &hfe->tracks[i];
    ...
    snprintf(creator_buf, sizeof(creator_buf),
             ...
             hfe->format_revision,
             hxc_get_encoding_name(hfe->track_encoding),
             hfe->bitrate_kbps,
             hfe->rpm);
 ...
 */
void hxc_free_ufm(ufm_image_t *ufm)
{
    if (!ufm) {
        return;
    }
 ...
 */
void hxc_ufm_print_stats(const ufm_image_t *ufm)
{
    if (!ufm) {
        return;
    }
    ...
    printf("  Encoding:   %s\n", hxc_get_encoding_name(ufm->encoding));
    printf("  Tracks:     %u\n", ufm->track_count);
