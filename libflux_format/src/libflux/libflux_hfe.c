...
/*
 * libflux_hfe.c - HXC HFE Format Parser
 * 
 * Professional implementation of HFE (UFT HFE Format) format.
 * HFE is the native format of UFT HFE Format hardware.
 * 
 * Format Overview:
...
typedef struct {
    ...
    uint16_t uft_floppy_rpm;        /* RPM (typically 300 or 360) */
...
int libflux_parse_hfe(
    ...
    hfe_out->rpm = header.uft_floppy_rpm;
...
int libflux_load_hfe_file(
    ...
    int rc = libflux_parse_hfe(buf, size, hfe_out);
