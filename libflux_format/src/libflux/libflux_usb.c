...
/*
 * libflux_usb.c - HxC USB Hardware Support
 * 
 * Professional implementation of UFT HFE Format USB protocol.
 * Enables direct hardware reading/writing with floppy emulators.
 * 
 * Supported Devices:
 *   - UFT HFE Format (Rev A, B, C)
...
#define HXC_USB_PID         0x4D00  /* UFT HFE Format */
...
typedef struct {
    ...
    bool motor_on;
    libflux_device_info_t info;
} libflux_device_handle_t;
...
/**
 * @brief Send command to floppy emulator
 */
static int send_command(
    libflux_device_handle_t *handle,
    const libflux_cmd_packet_t *cmd
) {
    int transferred;
...
/**
 * @brief Receive response from floppy emulator
 */
static int receive_response(
    libflux_device_handle_t *handle,
    libflux_resp_packet_t *resp
) {
    int transferred;
...
/**
 * @brief Send data to floppy emulator
 */
...
/**
 * @brief Receive data from floppy emulator
 */
...
int libflux_usb_init(libflux_device_handle_t **handle_out)
{
    ...
    libflux_device_handle_t *handle = calloc(1, sizeof(*handle));
    if (!handle) {
...
int libflux_usb_detect_devices(char ***device_list_out, int *count_out)
{
    ...
    char **list = calloc(libflux_count, sizeof(char*));
    if (!list) {
    ...
    /* Fill device list */
    int idx = 0;
    for (ssize_t i = 0; i < cnt && idx < libflux_count; i++) {
        struct libusb_device_descriptor desc;
        if (libusb_get_device_descriptor(devs[i], &desc) == 0) {
            if (desc.idVendor == HXC_USB_VID && desc.idProduct == HXC_USB_PID) {
                char buf[256];
                snprintf(buf, sizeof(buf), "UFT HFE Format (Bus %d Device %d)",
