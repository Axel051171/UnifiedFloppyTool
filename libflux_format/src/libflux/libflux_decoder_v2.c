...
typedef struct {
    int cylinder;
    int head;
    libflux_track_v2_t track;
    bool valid;
    uint64_t access_count;
} libflux_cache_entry_t;
...
/* Main decoder state */
typedef struct {
    libflux_params_v2_t params;
    libflux_thread_pool_t pool;
    libflux_cache_entry_t cache[HXC_V2_CACHE_SIZE];
 ...
 */
static void detect_weak_bits(libflux_track_v2_t* track, float threshold) {
 ...
 */
static void mark_sector_weak_bits(libflux_sector_v2_t* sector, 
...
static int decode_ibm_sector(const uint8_t* track_data, size_t track_bits,
                             size_t sync_pos, libflux_sector_v2_t* sector) {
...
static void* track_worker(void* arg) {
    libflux_work_item_t* work = (libflux_work_item_t*)arg;
    ...
    libflux_track_v2_t* track = work->track;
    ...
    if (work->encoding == 0) {
        ...
        while (pos < track->bit_count && track->sector_count < HXC_V2_MAX_SECTORS) {
            ...
            libflux_sector_v2_t* sector = &track->sectors[track->sector_count];
...
libflux_decoder_v2_t* libflux_v2_create(const libflux_params_v2_t* params) {
    libflux_decoder_v2_t* dec = calloc(1, sizeof(libflux_decoder_v2_t));
    if (!dec) return NULL;
    
    if (params) {
        memcpy(&dec->params, params, sizeof(libflux_params_v2_t));
    } else {
        libflux_v2_params_init(&dec->params);
    }
...
int main(void) {
    ...
    {
        libflux_decoder_v2_t* dec = libflux_v2_create(NULL);
        assert(dec != NULL);
        assert(atomic_load(&dec->initialized));
        libflux_v2_destroy(dec);
    ...
    // Test 7: Statistics tracking
    {
        ...
        libflux_v2_get_stats(dec, &tracks, &sectors, &errors, &weak);
