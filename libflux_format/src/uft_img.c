...
/* ---- internal: standard geometries ---- */

typedef struct uft_img_known {
    uint64_t bytes;
    uft_img_geometry_t g;
} uft_img_known_t;

static const uft_img_known_t g_known[] = {
    {  40ULL * 2ULL *  9ULL * 512ULL, { 40, 2,  9, 512 } }, /* 360KB */
    {  80ULL * 2ULL *  9ULL * 512ULL, { 80, 2,  9, 512 } }, /* 720KB */
    {  80ULL * 2ULL * 15ULL * 512ULL, { 80, 2, 15, 512 } }, /* 1.2MB */
    {  80ULL * 2ULL * 18ULL * 512ULL, { 80, 2, 18, 512 } }, /* 1.44MB */
    {  80ULL * 2ULL * 36ULL * 512ULL, { 80, 2, 36, 512 } }, /* 2.88MB */
};

static bool geom_sane(const uft_img_geometry_t* g)
{
    if (!g) return false;
    if (g->cylinders == 0) return false;
    if (!(g->heads == 1 || g->heads == 2)) return false;
    if (g->spt == 0) return false;
    if (g->sector_size != 512) return false; /* Tier-1 requirement: standard PC geometries */
    if (g->cylinders > 200) return false;
    if (g->spt > 64) return false;
    return true;
}

static uint64_t geom_bytes(const uft_img_geometry_t* g)
{
    return (uint64_t)g->cylinders * (uint64_t)g->heads * (uint64_t)g->spt * (uint64_t)g->sector_size;
}

static bool match_known_by_size(uint64_t size, uft_img_geometry_t* out)
{
...
static int ctx_recalc(uft_img_ctx_t* ctx)
{
...
static uint64_t chs_offset(const uft_img_ctx_t* ctx, uint8_t head, uint8_t track, uint8_t sector_1based)
{
...
{
    (void)buffer; /* IMG has no signature; size-only detection */
...
int uft_img_open(uft_img_ctx_t* ctx, const char* path, bool writable, const uft_img_geometry_t* forced)
{
    ...
    if (rc != UFT_SUCCESS) { uft_img_close(ctx); return rc; }

    uft_img_geometry_t g = {0};

    if (forced) {
        if (!geom_sane(forced)) { uft_img_close(ctx); return UFT_ERR_FORMAT; }
        uint64_t expected = geom_bytes(forced);
        if (expected != ctx->file_size) { uft_img_close(ctx); return UFT_ERR_FORMAT; }
        g = *forced;
    } else {
        if (!match_known_by_size(ctx->file_size, &g)) { uft_img_close(ctx); return UFT_ERR_FORMAT; }
    }
                    ...
                    uft_img_sector_meta_t* meta)
{
    ...
    if (track >= ctx->geom.cylinders) return UFT_IMG_ERR_RANGE;
    if (head  >= ctx->geom.heads)     return UFT_IMG_ERR_RANGE;
    if (sector == 0 || sector > ctx->geom.spt) return UFT_IMG_ERR_RANGE;
    ...
    if (meta) {
        ...
        /* IMG carries no weak bits/timing. Keep flags 0. */
    }
...
{
    if (!raw_path || !output_img_path || !geom) return UFT_ERR_INVALID_ARG;
    ...
    FILE* out = fopen(output_img_path, "wb");
