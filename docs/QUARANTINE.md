...
| **Verdacht** | **nibtools** (rittwage), Zone **GELB** (**Apache-2.0** — berichtigt MF-1008; hier stand GPL-3.0. Eigentuemer-Feststellung 2026-09-10, Quelle `nibtools-extra/LICENSE`. Am Ergebnis aendert das nichts: Apache-2.0 ist mit GPL-2 ebenso unvereinbar wie GPL-3-only, nur ueber die Patent- und Beendigungsklauseln statt ueber den Versionsschritt). Beleg: Kopf `:5` „Based on nibtools by Pete Rittwage and Markus Brenner", `:479` nennt `prot.c align_rl_special()` |
| **Datum der Quelle** | LICENSE erst **2025-01-30** (`a549c18`) — davor **lizenzlos**. Aufnahme in unseren Baum **2026-02-08** (`4d622192`), also nach der Lizenzierung. **MF-1008:** hier stand „nach der GPL-3-Lizenzierung“; welche Lizenz `a549c18` setzte, ist damit offen — der Scout las dort einen GPL-3.0-Volltext, die heute geltende Fassung ist **Apache-2.0** (Eigentuemer-Feststellung 2026-09-10). Für diesen Eintrag folgenlos: beide sind mit GPL-2 unvereinbar, und der Audit-Stand unten sagt ohnehin **portiert** |
...
|---|---|---|---|---|---|
| `src/formats/ipf/uft_ipf_air.c` | 1098 | „**Full port** of AIR `IPFReader.cs`/`IPFStruct.cs`/`IPFWriter.cs` to C. Original: © 2014 Jean Louis-Guerin (**GPL-3.0**)" | **IPF-Lesen** — das Plugin ruft `ipf_air_alloc` (`:70`), `_free` (`:78`), `_get_geometry` (`:85`), `_get_track_meta` (`:135`); keine statischen Gleichnamen | **1/2 seit MF-1003** (vorher 3) — Eigentümer-Entscheidung 2026-09-10: MAMEs `ipf_dsk.cpp` (BSD-3) gilt als unabhängig entstanden und ist damit benannte Referenz nach MF-614-Muster. Weg 3 bleibt Rückfall. Register unten | capsimg — Lizenz **gemessen** (SPS DECODER LIBRARY v1.02, GPL-**un**vereinbar): Helfer über Prozessgrenze, nie Oracle im Baum. **Neu: MAME `ipf_dsk.cpp` als Lese-Referenz** — und seit MF-1075 liegen **zwei echte SPS-Abbilder** im Korpus; MF-1079 hat daran den Zellstrom abgenommen (`ipf` T3 → **T1**) und die vier Zugriffsfunktionen `ipf_air_get_block_count/_sizes/_elem_count/_elem` angehaengt (+56 Zeilen) — reines Herausreichen des schon Geparsten, die Zellrechnung steht in der eigenstaendigen `uft_ipf_zellstrom.c` **ausserhalb** der Quarantaene. Siehe Register |
...
| `src/formats/stx/uft_stx_air.c` | 1087 | „Full port of AIR `PastiRead.cs`/`PastiStruct.cs`/`PastiWrite.cs` to C. © 2014 Jean Louis-Guerin (**GPL-3.0**)" | **STX-Lesen** — berichtigt MF-960. **MF-1165: 1049 → 1087 (+38), und die Differenz ist KEIN fremder Code** — 35 Zeilen eigener Kommentar plus der dreizeilige `stx_sektorgroesse()`, der die Maskierungsregel der Pasti-Beschreibung („only bits 0-1 matter") an EINER Stelle festhält, statt sie dreimal als `128 << rohbyte` zu wiederholen. Dieselbe Buchung wie bei `uft_ipf_air.c` in der Zeile darüber: der Umfang wächst, die Ableitung nicht. Hier stand „**keine** — 0 Aufrufer, steht in `orphan_baseline.txt`". **MF-854 (2026-09-04) hat den Port zum registrierten Leser gemacht**: `src/formats/stx/uft_stx_plugin.c:65,70,74,86` ruft `uft_stx_air_open/_close/_cylinders`. Die alte Angabe bezifferte, was eine Loeschung kostet — nach ihr war Weg 2 billig. Heute kostet sie **STX-Lesen**. Das Verwaisten-Tor meldet die Grundlinien-Zeile seither als streichbar; das Register ist nicht nachgezogen worden | 2 | Pasti-Spec liegt (T2-Quelle) |
...
| Pflichtfeld | Inhalt |
|---|---|
| **Nachbau-Route** | **Weg 1/2 ist seit MF-1003 offen — Eigentümer-Entscheidung.** Hier stand: „**Weg 3** — Helfer über Prozessgrenze. Weg 2 (Clean-Room aus Spec) ist versperrt: das IPF-Format ist **bewusst** undokumentiert, die SPS behält sich seine Erzeugung vor. Weg 1 scheidet aus, weil die Datei sich im eigenen Kopf als „Full port" erklärt". Der Satz über die **Dokumentation** stimmt weiterhin. Was ihn überholt, ist eine **Umsetzung**: MAMEs `src/lib/formats/ipf_dsk.cpp` (835 Z., **BSD-3-Clause**, Olivier Galibert) ist ein vollständiger, unabhängig geschriebener IPF-Dekoder — mit Lückenbeschreibung, schwachen Bits und Zeitverläufen. Der Eigentümer hat am 2026-09-10 bestätigt, dass er als unabhängig entstanden gilt; damit ist er nach dem **MF-614-Muster** eine benannte Referenz (lesen, eigenständig umsetzen, Datei und Zeile zitieren, ausdrücklich festhalten, dass nichts kopiert wurde). BSD-3 ist mit UFTs GPL-2-or-later verträglich. Weg 3 bleibt als Rückfall bestehen |
...
| **Was jetzt WIRKLICH blockiert (MF-1003)** | **Daten, nicht Lizenz.** Im Korpus liegt **kein einziges IPF** — gemessen (`find tests -iname '*.ipf'` = leer, kein Manifest-Eintrag). `tests/test_ipf_air_accessors.c` baut einen **synthetischen** `"CAPS"`-Puffer; `ipf` steht folgerichtig auf **T3**. Ein Ersatz, der heute geschrieben würde, ließe sich nur gegen die eigene Vorstellung vom Format prüfen — das ist die Selbstbestätigung aus **MF-992** (zehn grüne BAM-Tests, die durch dieselbe falsche Struktur zurücklasen), und die EINFRIER-REGEL (b) verlangt gemessene Zahlen. **Kanal nach MF-695: Daten/Fixture.** Was es öffnet: ein IPF, dessen Weitergabe geklärt ist — die SPS-Verteilbedingungen sind selbst eine Eigentümerfrage. Alternativ ein IPF, das aus einer **freien** Quelle stammt |
| **Reihenfolge (belegt, nicht geraten)** | Erst Daten, dann Code. Der Baum hat das zweimal vorgeführt: `f3c8099a` (MF-905) hob den OPD-**Leser** gegen ein Orakel, erst danach verdrahtete `c601a789` (MF-931) den Schreiber; und MF-864 baute den FM-Dekoder überhaupt erst, nachdem `fluxtoimd` als fremde Hand die Prüfspur abnehmen konnte |
