...
| P3-89 | **Drei `#ifdef UFT_UNIT_TESTS`-Bloecke, die nie uebersetzt werden.** `UFT_UNIT_TESTS` wird im ganzen Baum **nirgends definiert** — weder im qmake-`.pro`, noch in einer `CMakeLists.txt`, noch als Compilerschalter. Die Selbsttests in `src/recovery/uft_multiread_pipeline.c`, `src/parsers/a2r/uft_a2r_parser.c` und `src/protection/uft_protection_detect.c` sind damit toter Code: sie werden nie gebaut und koennen nie rot werden | ueber `git ls-files` gemessen, drei Dateien (MF-845) | ⚠ **offen.** Dieselbe Klasse wie MF-830 (21 Testdateien, deren `assert()` unter `-DNDEBUG` ein leerer Ausdruck war) und wie P3-84 in der Anlage: ein Prueflauf, der nicht laufen kann. **Der Weg ist bekannt und billig:** die Faelle nach `tests/` heben, wo CI sie ausfuehrt — genau das ist mit dem Rotbeweis aus MF-845 fuer das Multiread-Modul geschehen. Ein Tor waere moeglich (Praeprozessor-Schalter, der in keiner Bauvorschrift vorkommt), aber bei drei Fundstellen ist das Verschieben billiger als das Messen |
...
| P3-92 | **Zwei einander widersprechende STX-Flag-Tabellen im selben Baum.** `uft_stx_air.c:57-60` fuehrt `SECT_DESC 0x01 · PROT 0x20 · IMAGE 0x40 · SYNC 0x80`; `src/formats/atari/uft_stx_parser.c:38-42` fuehrt fuer dieselben Bits `TRACK_IMAGE 0x01 · TIMING_DATA 0x20 · FUZZY_BITS 0x40`. Auch die Sektor-Flags gehen auseinander (`DELETED` einmal 0x20, einmal 0x04). Eine der beiden Tabellen ist falsch, und MF-847 haengt an der ersten | im Baum feldweise gegenuebergestellt (MF-847) | ⚠ **offen — braucht eine ZWEITE Hand.** Die Lage ist nicht ausgeglichen: die AIR-Tabelle hat mit `pasti/PastiRead.cs` und der Pasti-Dokumentation eine **benannte** Quelle (beide von Jean Louis-Guerin), die Atari-Tabelle hat **keine**; und auf Sektorebene hat MF-335 denselben Streit bereits zugunsten von Louis-Guerin entschieden (`0x08` CRC, `0x20` deleted), mit einem Test, der es festhaelt. Der Ausreisser ist also die Atari-Fassung. **Aber Louis-Guerin ist EINE Hand** — Spec und Referenzcode stammen vom selben Autor, und die Zwei-Quellen-Regel ist damit nicht erfuellt. Beschaffungsauftrag: eine unabhaengige STX-Beschreibung oder ein fremder Leser (Hatari, `pasti.c`) als zweiter Zeuge fuer Bit 0. Bis dahin steht die Wahl im Code begruendet, nicht als gesichert |
| P3-93 | **Der vollstaendige STX-Port hat keinen Aufrufer — der 179-Zeilen-Leser ist der ausgelieferte. Eigentuemer-Entscheidung.** `uft_stx_air.c` (~950 Z.) liest Fuzzy-Masken, Timing-Saetze, Spurbilder mit Sync-Offset, Standardspuren und rechnet CRC nach; ausserhalb der eigenen Datei nennen ihn nur `tests/test_air_cross_validate.c` und ein Kommentar. Zwei weitere STX-Leser (`uft_stx_parser_v2.c`, `atari/uft_stx_parser.c`) haben ebenfalls keinen Aufrufer | ueber `git ls-files` je Bezeichner gemessen (MF-847) | ⚠ **offen, bewusst nicht von mir entschieden.** Das ist die im Baum sechsmal belegte Lage „wo ein Port neben einem nativen Parser steht, ist der Port richtig" — hier verschaerft, weil der Port **abgekoppelt** ist (dieselbe Bauform wie DMS vor MF-837, wo 668 Zeilen eigener Entpacker der Bibliothek wichen). Die Verdrahtung waere die groessere Reparatur, hat aber **zwei Folgen, die dem Eigentuemer gehoeren**: (a) sie zoege eine **GPL-3.0-only**-Uebersetzungseinheit in den REGISTRIERTEN Pfad — MF-698 hat die GPL-3-Bindung fuer das verteilbare Gesamtwerk zwar bereits angenommen, aber die Reichweite waechst, und Lizenzeinstufung ist nach MF-679 nicht meine; (b) sie machte `STX_MAX_SECTORS 32` **erreichbar**, also eine ruhende Klemme scharf (P3-58). Deshalb ist MF-847 die Reparatur der fuenf Feldfehler am ausgelieferten Leser und nicht die Umverdrahtung |
...
| P3-96 | **Greaseweazle: kein Mindest-Firmware-Check — gemeldet, geschuetzter Pfad, Freigabe abgewartet.** `usb.py` fuehrt `EARLIEST_SUPPORTED_FIRMWARE = (0, 31)` und faellt bei aelterer Firmware nicht mit einem Protokollfehler um, sondern setzt `update_needed` und beschraenkt sich auf `GetInfo`. UFT liest `fw_major`/ `fw_minor` in `uft_gw_get_info()` und **protokolliert sie nur** — gemessen: die Zeichenketten `MIN_FW` und `EARLIEST` kommen in `uft_greaseweazle_full.{c,h}` **null**mal vor. Eine zu alte Firmware faellt erst beim ersten nicht unterstuetzten Befehl auf, als `UFT_GW_ERR_UNSUPPORTED` ohne Bezug zur Ursache. Der Bootloader-Fall ist bereits sauber abgefangen (`is_main_fw == 0` → `UFT_GW_ERR_BOOTLOADER`); es fehlt der Fall „laeuft, ist aber zu alt" | vom Eigentuemer aus `keirf/greaseweazle` `src/greaseweazle/usb.py` gemeldet, im Baum durch Zaehlung nachgemessen (MF-848) | ⚠ **offen — `src/hal/uft_greaseweazle_full.c` ist ein GESCHUETZTER PFAD** (`.claude/CLAUDE.md`, „keine Aenderung ohne Rueckfrage"). Der Prueftstand steht seit MF-848 bereit: `gw_fw_set_firmware_version(&fw, 0, 29)` stellt den Automaten auf eine zu alte Firmware, die Bruecke traegt sie bis in `uft_gw_info_t` — der Rotbeweis ist also einen Testfall entfernt. **Vorgeschlagene Form, besser als die Vorlage:** nicht als Zweig IN `uft_gw_open()`, sondern als eigene Zusage `uft_gw_firmware_supported(const uft_gw_info_t*)`, von `uft_gw_open()` gerufen. Der Grund ist derselbe wie ueberall in diesem Baum: eine Regel, die nur im Transportpfad steht, ist ohne Transport nicht pruefbar. `usb.py` faltet sie in `Unit.__init__` — das ist die Stelle, an der wir es BESSER machen koennen, nicht nur gleich |
| P3-97 | **Greaseweazle: der `dev`-Schutz gilt nur im Naht-Zweig, nicht im Rueckfall. Gemeldet, geschuetzter Pfad.** `serial_write_all()`, `serial_read_exact()` und `serial_read_available()` pruefen `dev && dev->stream_ops && ...` und reichen bei NULL trotzdem `serial_*_platform(dev, ...)` weiter, wo `dev->handle` bzw. `dev->fd` ohne weitere Pruefung dereferenziert wird. Der `dev &&`-Test suggeriert einen Schutz, den der Rueckfallzweig nicht hat | vom Eigentuemer (cppcheck) gemeldet, im Baum an allen drei Funktionen nachgelesen (MF-848) | ⚠ **offen, geschuetzter Pfad — und ehrlich zur Schwere: heute nicht erreichbar.** Alle Aufrufer pruefen `dev` vorher; `uft_gw_command()` beginnt mit `if (!dev) return UFT_GW_ERR_NOT_CONNECTED`. Es ist Haertung, kein Bugfix mit sichtbarem Symptom, und das gehoert so in die Commit-Nachricht. **Der Pruefstand aus MF-848 kann es abnehmen**, sobald die Freigabe vorliegt: drei Aufrufe mit `dev = NULL`, erwartet `UFT_GW_ERR_NOT_CONNECTED` statt Absturz |
| P3-98 | **Zwei native Treiberplaene (KryoFlux, FluxEngine) — beide teilen einen blinden Fleck, und der ist nicht die Lizenz.** Die Zulieferung schlaegt vor, `src/samdisk/KryoFlux.cpp` zu verdrahten (liegt im Baum, **ohne Aufrufer** ausserhalb `src/samdisk/`, gemessen) und einen libusb-Treiber fuer FluxEngine `1209:6e00` zu schreiben (~350 Zeilen Rahmenprotokoll aus `davidgiven/fluxengine`) | beide Aussagen im Baum nachgemessen; der FluxEngine-Emulator auf seine Modellierungstiefe gelesen (MF-848) | ⚠ **offen — der Reihe nach.** **(1) Pruefbarkeit, und die trifft beide.** Dieses Projekt hat kein Geraet (MF-310). Beide Plaene fuegen Code in den HAL, den niemand hier gegen Silizium halten kann — und das ist genau die Bauform der fuenf fabrizierten Parser (FMT-2/3/10/11/12): gegen eine gelesene Beschreibung gebaut, gruen, falsch. Der FluxEngine-Emulator im Baum hilft dabei **nicht**: er modelliert die **CLI** (`read -c ibm -s drive:0 …`), nicht die Leitung — sein eigenes `DIVERGENCES.md` sagt es in FE-3. Es gibt also fuer das Drahtprotokoll kein Orakel. **Die Reihenfolge ist damit vorgezeichnet: erst der Automat auf Drahtebene, dann der Treiber** — genau der Weg, den MF-848 fuer Greaseweazle gegangen ist, nur dort war der Automat schon da. **(2) Lizenz, und die trifft nur KryoFlux.** Die SPS-Lizenz nimmt ausdruecklich nur die Dateiformate (IPF/STREAM/DRAFT) aus; das Live-USB-Protokoll faellt nicht darunter, und `firmware_kf_usb_rosalie.bin` ist SPS-Eigentum. Nach MF-679 stufe **ich** keine Lizenzen ein — Zone PRUEFEN, Eigentuemer-Entscheidung. FluxEngine ist davon frei (offenes Protokoll). **(3) Kennzahl.** Nach Regel 9: keiner der beiden Plaene senkt das **Bench-Alter** — dafuer braucht es ein Geraet. Ein Drahtautomat nach dem Muster von MF-848 senkt es auch nicht, aber er verkleinert, was ein Bench fangen muss, und das ist die einzige Richtung, die von hier aus erreichbar ist |
