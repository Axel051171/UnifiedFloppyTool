...
| P3-103 | **31 Makros tragen im Produktivcode denselben Namen mit verschiedenen Werten.** Beim Bau des Tors zu P3-92 gemessen; darunter mehrere, die keine Geschmacksfrage sind: `CMD_BAM_TRACK`/`CMD_BAM_SECTOR`/`CMD_DIR_SECTOR` sind zwischen `uft_cmd.h` und `uft_cmd_fd.c` **vertauscht** (0↔1), `SCP_DISK_APPLE_400K` steht `0x30` gegen `0x24` und `SCP_DISK_APPLE_800K` `0x34` gegen `0x25`, `G64_HEADER_SIZE` `684` gegen `12`, `PRO_MAX_TRACKS` `80` gegen `77`, `GEOS_TYPE_FONT` `15` gegen `14`, `ADF_TRACKS` `80` gegen `160`, `D2M_TRACKS`/`D4M_TRACKS` `81` gegen `80` | ueber `git ls-files` gemessen, `scripts/audit_macro_drift.py` mit Selbsttest 4/4 (MF-852) | ⚠ **offen, weiche Grundlinie 31.** **Ein Fall ist bereits behoben:** `ATX_SIGNATURE` stand `0x41543858` gegen `0x58385441` — beide Kommentare behaupteten `"AT8X"`, aber `uft_atx_parser_v2.c` liest die Rohbytes per `fread()` in einen gepackten Struct (auf Little-Endian also `0x58385441`) und verglich gegen die Big-Endian-Lesung: **die Signaturpruefung traf nie**, `atx_reader_open()` wies jede gueltige ATX-Datei ab. Heute folgenlos, weil die Datei keinen Aufrufer hat — nicht folgenlos in dem Moment, in dem jemand sie verdrahtet. **Der Rest braucht je eine Entscheidung, welche Fassung gilt** — Eigentuemerarbeit wie P3-76, keine Sammelkorrektur: sie wuerde stillschweigend Aufrufer umdeuten. **Nicht jeder Treffer ist ein Fehler:** ein lokales `MAX_SECTORS` oder `INITIAL_CAP` in zwei unverwandten Modulen ist zulaessig. Woran man die echten erkennt, steht im Kopf des Skripts: ein formatgebundener Praefix benennt eine Eigenschaft des FORMATS, und ein Format hat nur eine |
