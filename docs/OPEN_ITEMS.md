...
| P3-238 | **Die HAL reicht Umdrehungsgrenzen nicht durch — es sind DREI Stellen, nicht eine. Teilweise behoben MF-951.** Der Bericht (57. Durchgang) nannte Greaseweazle. Gemessen sind es alle drei Flusspfade: `greaseweazle_backend.c:249` kopiert nur `fd->samples` und ruft dann `uft_gw_flux_free(fd)`; der KryoFlux-Pfad in `uft_hal_unified.c` holt die Grenzen ausdruecklich und gibt sie sofort wieder frei („Free index array (not used in HAL interface currently)"); der SCP-Pfad liest je Umdrehung `rev_hdr[8]` = [index_time(4), data_len(4)] und benutzt nur die Laenge. `uft_hal_read_flux()` hat kein Feld dafuer | je Fundstelle im Baum nachgelesen; `tests/test_rev_grenzen.c` 8/8 gruen (MF-951) | ⚠ **teilweise — das Rechenstueck steht, die Verdrahtung nicht.** MF-951 liefert `uft_rev_grenzen_aus_dauern()`: die Umrechnung von Umdrehungs-DAUERN in Abtast-VERSAETZE, geraeteunabhaengig, mit Kettentest ueber den GW-Flussgenerator und den Produktionsdekoder — **ohne Hardware** (MF-310). **Und sie verhindert einen stillen Fehler des Berichts:** dessen Entwurf schreibt `rev_offsets[i] = fd->index_times[i]` mit dem Kommentar „Sample-Index je Umdrehungsstart". Das ist falsch. `uft_gw_decode_flux_index_times()` liefert **Tick-Dauern** — der eigene Header sagt es woertlich („computes revolution durations") —, und die haben nicht einmal dieselbe Groessenordnung: bei 72 MHz und 200 ms stehen dort rund 14 400 000, waehrend der Strom 12 000 Abtastungen hat. Wer das eine fuer das andere haelt, schneidet an willkuerlichen Stellen — dieselbe Klasse wie MF-950. **Offen bleibt** die Erweiterung von `uft_hal_read_flux()` selbst und das Durchreichen in den drei Backends; das ist ohne Geraet nicht abnehmbar |
