...
| P3-204 | **Elf Formate haben einen fertigen Dateischreiber, den niemand ruft. Er bleibt unverdrahtet, bis je Format ein Rückweg belegt ist.** Betroffen: `apridisk`, `cfi`, `hardsector`, `logical`, `mgt`, `myz80`, `nanowasp`, `opus`, `posix`, `qrst`, `rcpmfs`. Jede dieser Dateien enthält ein vollständiges `uft_<fmt>_write()` mit `fwrite` — seit MF-930 antwortet `write_track` **ehrlich** `UFT_ERROR_NOT_SUPPORTED`, statt einen Erfolg zu melden, den die Datei nicht trägt | **gemessen MF-930**: `plugin->flush` hat im ganzen Baum keinen Aufrufer, `uft_disk_close()` ruft nur `close`; die elf Schreiber sind über **keinen** dieser Wege erreichbar (transitiv innerhalb der Datei) | ⚠ **offen als Arbeit, nicht als Fehler.** Warum nicht sofort verdrahtet: das wäre neues Verhalten auf dem **Schreibpfad** für elf Formate ohne je ein Prüfabbild — elf Wetten. Die EINFRIER-REGEL (MF-363/498) verlangt benannte Referenz, gemessene Zahlen, Referenz im Header. **Der Weg je Format steht fest:** Prüfabbild beschaffen oder herstellen → den Rundlauf aus `tests/test_schreibzusage_erreicht_die_datei.c` auf das Format ziehen → rot sehen → `close()` (oder ein gerüstetes `flush`) verdrahten → grün → `CAP_WRITE` und Merkmalstafel **zusammen mit der Tat** zurückholen. **`opus` ist die naheliegende erste:** Abbild herstellbar (`baue_opd()`), Oracle im eigenen Baum (`src/samdisk/opd.cpp`, MF-905/T2), Rundlauf schon geschrieben. **Kennzahl (Regel 9):** keine der vier direkt — aber jede Hebung nimmt einen Eintrag aus P3-204 und ist Voraussetzung dafür, dass diese Formate je in der Wandlungsmatrix als **Ziel** stehen können |
