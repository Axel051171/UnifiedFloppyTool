...
| P3-241 | **Der GW-Flussgenerator und der Produktionsdekoder meinen mit der Index-Nutzlast Verschiedenes.** `tests/flux_gen/greaseweazle/flux_gen.c` legt die volle Umdrehungsdauer in die N28-Nutzlast des Index-Opcodes (Kommentar dort: „The N28 payload is the ticks since prev index"). `uft_gw_decode_flux_index_times()` setzt dagegen die greaseweazle-Formel um — dort ist die Nutzlast nur der REST vom letzten Wechsel bis zum Puls und wird zur aufgelaufenen Flusszeit ADDIERT | am erzeugten Strom gemessen (MF-951) | ⚠ **offen, fremdes Gebiet.** Zahlen: Generator meldet **14 400 000** Ticks je Umdrehung, Dekoder liest **17 867 232** = 3 467 232 (wirklich geflossen) + 14 400 000 (Nutzlast). Der Strom traegt insgesamt nur ~10,4 Mio Ticks — die behauptete Dauer EINER Umdrehung uebersteigt den ganzen Strom. `DIVERGENCES.md` fuehrt das **nicht**; D-8 betrifft den Jitter, nicht die Bedeutung der Nutzlast. Nicht selbst geaendert: der Generator gehoert zur Emulator-Suite, und ein Fixture zu aendern, waehrend fremde Tests darauf stehen, waere die falsche Reihenfolge. `test_rev_grenzen.c` prueft deshalb, dass die Umrechnung daraufhin KEINE Grenzen erfindet — sie meldet die eine, die sie belegen kann |
