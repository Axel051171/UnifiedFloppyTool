...
| P3-251 | **UFT-33 schlaegt zwei HAL-Erweiterungen vor, die es bereits gibt — und eine dritte Darstellung derselben Groesse** | am Baum gegengeprueft (MF-957) | ⚠ **offen als EINORDNUNG, vom Eigentuemer vorgeschlagen; hier gemessen.** **E1 (Faehigkeitsabfrage):** `uft_hal_cap_t` mit acht `HAL_CAP_*`-Bits steht in `include/uft/hal/uft_hal.h:119`, `uft_hal_get_caps()` in Z175 — und der vorgeschlagene Typname `uft_hal_caps_t` ist dort bereits das **Struct** (Z134). Ein zweites Faehigkeits-Enum daneben waere die naechste „zwei Wahrheiten fuer dieselbe Sache". Richtig waere: `uft_hal_cap_t` um `HAL_CAP_REV_BOUNDARIES` erweitern. **E2 (Sample-Takt):** `uft_hal_caps_t::sample_rate_hz` existiert und wird je Treiber **korrekt** gefuellt (72 M / 24,027 M / 40 M / 8 M, vier Stellen in `uft_hal_unified.c` gemessen). Der vorgeschlagene `_ps`-Getter waere die DRITTE Darstellung (Hz im Struct, ns in `uft_controller_caps_t`, ps neu), und die Begruendung traegt nicht: 24 027 428 Hz ist als Ganzzahl exakt. **Die Luecke hinter E2 ist dagegen echt und schaerfer als beschrieben:** `src/protection/uft_fuzzy_bits.c:457` UND `:505` verdrahten `1e9 / 72000000.0` fest — auf KryoFlux waere jede Zeitangabe um **Faktor 3** falsch, auf SCP um 1,8. Reine Verdrahtung an vorhandenes, kein Neubau. Vier von fuenf genannten Dateipfaden gibt es nicht (`scp_direct_backend.c`, `kryoflux_backend.c`, `docs/REGISTER.md`, `src/archive/`) |
