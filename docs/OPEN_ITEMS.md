...
| P3-32 | **Der SCP-v3-Parser beschriftet Disk-Types systematisch falsch, und der Pfad ist erreichbar.** `uft_scp_parser.c:595` behauptet, die doppelte Tabelle sei entfernt — `uft_scp_parser_v3.c:562` hält eine zweite. Nachgemessen gegen Spec v2.5: `ATARI_ST 0x10` (Spec 0x14), `PC_360K 0x40` (Spec 0x30, dort TRS-80 SS/SD), `TRS80 0x60` (0x40), `TI99 0x70` (0x50), `ROLAND 0x80` (0x60), `AMSTRAD 0x90` (0x70), `OTHER 0xC0` (0x80). Ein TRS-80-Abbild wird als „PC 360K" ausgegeben, ein PC-1.44 (0x33) als „Unknown". `scp_get_expected_sectors()` hängt daran. Erreichbar über `.pro:753` → `uft_v3_bridge.c:302` → `uft_advanced_mode.c`. **Vier weitere Befunde am selben Parser, alle nachgemessen:** (a) der Overflow-Marker `0x0000` (≥65536×25 ns ohne Übergang) wird in `scp_calc_flux_stats():845` als Kurzimpuls gezählt → `SCP_DIAG_FLUX_TOO_SHORT`; **je stärker der NFA-Schutz, desto schlechter die Qualitätsnote** — die Bedeutung ist umgekehrt. (b) `:1345` rechnet `side_count` aus FLAGS Bit 0, das ist **INDEX**; die Seitenzahl steht in Byte 0x0A und wird `:1109` als `disk->heads` gelesen und nur `:1433` zurückgeschrieben, nie ausgewertet. (c) `disk->resolution` (`:1110`) wird gelesen, gerechnet wird überall mit festem `SCP_TICK_NS` (`:531/:538/:555`) — für jedes Abbild mit `resolution > 0` ist jede Zeit und jede RPM falsch. (d) `SCP_FLAG_EXTENDED` (`:86`) wird nirgends abgefragt (TDH beginnt dann bei 0x80 statt 0x10), die Prüfsumme `:1113` nie geprüft. **Warum kein Tor es fing:** `test_scp_readers_agree.c` vergleicht **Flussintervalle** — Disk-Type, Auflösung, Overflow, Seiten und Prüfsumme liegen außerhalb seiner Zusicherung. Die Lücke gehört geschlossen, bevor die Tabelle repariert wird | gemeldet vom Eigentümer, Konstanten nachgemessen (MF-809) | ⚠ **offen** |
| P3-33 | **Zwei SCP-Regeln, die nur im Herstellerforum stehen.** (1) **Fehlerfenster im Bestand:** zwischen **2020-06-16 und 2021-11-13** schrieb die SCP-Software IBM-1.44-Abbildern `0x45` statt `0x33` in den Disk-Type (Jim Drew, tid=861). `0x45` ist ein **gültiger** Wert, kein erkennbarer Müll — ein SCP-Abbild unbekannten Alters mit `0x45` ist nicht zwingend defekt. Provenienz-Regel für den Korpus, keine Codefrage. (2) **VERBOTSREGEL:** „Bitcell-Summe > Index-zu-Index" ist NORMAL (tid=868) — Ursache sind ungültiger Fluss und Drehzahlschwankungen, und das Lesen beginnt und endet mitten in einer Bitcell, weshalb erste und letzte typischerweise kürzer ausfallen. Eine Plausibilitätsprüfung „Summe muss zur Indexzeit passen" würde **jedes zweite echte Abbild** als defekt melden. Der Parser prüft es heute nicht — die Regel steht hier, damit sie niemand später einbaut | gemeldet vom Eigentümer (MF-809) | ⚠ **Fundus.** Nicht angesehen: tid=1223/1240 (neue Geräterevision, berührt den SCP-Direct-Provider), tid=93 (SDK), tid=295, tid=282 (Keir Fraser, weak-flux-Schreibpfad) |
...
| P3-58 | **Die zweite stille Klemme — `STX_MAX_SECTORS 32` — bleibt offen, und zwar aus einem genannten Grund.** MF-830 hat die IPF-Blockklemme sichtbar gemacht (`ipf_air_get_track_loss()`), die STX-Sektorklemme in `uft_stx_air.c:289` NICHT. Zwei Unterschiede entscheiden die Reihenfolge: **(1) Erreichbarkeit.** `IPF_MAX_BLOCKS` ist **16**, und eine PC-HD-Spur mit 18 Sektoren hat mehr als 16 Bloecke — der Verlust passiert im Normalbetrieb. `STX_MAX_SECTORS` ist **32**; Atari-Spuren tragen 9 bis 11 Sektoren, geschuetzte selten ueber 20. Die Klemme ist real, aber nach heutigem Wissen nicht im Alltag erreichbar. **(2) Es gibt keine Tuer.** IPF hat mit `include/uft/formats/ipf/uft_ipf_air.h` eine Zugriffs-API und mit `test_ipf_air_accessors.c` einen Platz fuer den Beweis; `uft_stx_air.c` hat **keinen Header** — die Typen sind uebersetzungseinheitslokal. Ein Befund waere dort nicht abfragbar, ohne vorher eine API zu bauen | beide Klemmstellen und die Erreichbarkeit gemessen (MF-830) | ⚠ **offen, bewusst nachgeordnet.** Der schaerfere Teil steht schon: `trk->sector_count` traegt die ANGEKUENDIGTE Zahl, gelesen werden nur `nsec` Deskriptoren — die Struktur sagt also 40, wenn 32 vorliegen. Das ist keine Kappung, sondern eine **falsche Zahl im Datensatz**, und sie ist schlimmer als der Verlust selbst. Zwei Stellen im selben Modul lesen zwar defensiv (`snum < td.sector_count && snum < STX_MAX_SECTORS`, `:514`), ein fremder Verbraucher tut das nicht. **Naechster Schritt, konkret:** `sectors_stored` neben `sector_count` fuehren und eine schmale Zugriffs-API nach dem Muster von `ipf_air_get_track_loss()` anlegen — dieselbe Form, dieselbe Trennung zwischen „geklemmt" und „Datei abgeschnitten" |
...
| P3-84 | **17 Dateihandles, die bei einem Fehler offen bleiben — gemeldet, noch nicht behoben.** Das Muster ist immer dasselbe: `fopen` gelingt, dann bricht ein `fseek` ab und die Funktion kehrt zurueck, **ohne** `fclose`. Vier Fundstellen einzeln nachgeprueft und bestaetigt: `formats/flex/uft_flex.c:115`, `formats/industrial/uft_cromemco.c:65`, `formats/udi/uft_udi.c:191`, `core/uft_advanced_mode.c:155`. In `uft_flex.c` macht die **unmittelbar vorhergehende** Zeile es richtig (`if (sz <= 0) { fclose(f); return …; }`) — es ist eine vergessene Klammer in einem von zwei Zweigen. Weitere gemeldete Stellen: `uft_heathkit.c:68`, `japanese_ext/uft_{hitachi_s1,sanyo_mbc,sharp_x1}.c`, `minicomputer/uft_{dg_nova,prime}.c`, `nordic/uft_abc800.c:97`, `obscure/uft_{applix,pmc_micromate,pyldin,rc759}.c`, `uft_hdf_parser.c` (zweimal, 297 + 322) | gemeldet vom Eigentuemer (cppcheck), vier Stellen von Hand gegengeprueft (MF-842) | ⚠ **offen, naechster Schritt.** Praktische Folge: bei Stapelkonvertierung vieler Dateien laufen die Handles voll. **Und es ist gatterbar** — aber nur eng: ein grober Durchlauf ueber „`return` waehrend eine Datei offen ist" liefert 467 Kandidaten, weit ueberwiegend das korrekte `if (!f) return …`. Die praezise Form ist die `fseek`-Schranke ohne `fclose` in derselben Zeile; danach gebaut, wird das Tor schmal genug, um zu tragen. Nebenbefund an derselben Stelle: `uft_advanced_mode.c:155` gibt im Fehlerfall `-1` zurueck, waehrend die Funktion sonst einen Format-Enumwert liefert |
