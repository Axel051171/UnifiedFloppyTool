...
| P0-15 | **Zwei Wahrheits-Fehler in der Wandlungs-Anzeige.** `SCP→G64` meldete 35 gewandelte Spuren und schrieb **789 Byte**; `HFE→ADF` meldet „0 gewandelt, 160 gescheitert“ und liefert eine zu **99,92 %** korrekte Datei | ✅ **SCP→G64 behoben (MF-537)**: `d64_gcr_to_flux()` liefert SCP-Ticks, `scp_writer_add_track()` will Nanosekunden — 4000 ns wurden zu 150 ns; jetzt 252782 statt 789 Byte. **Offen:** die Zaehler in `HFE→ADF` |
...
| P0-11 | **Leck-Rueckstand.** Gemessen mit `scripts/audit_track_cleanup.py`: im **Produktionscode kein einziger** Aufrufer, der eine gelesene Spur liegen laesst (`src`: 0). Der Rueckstand im LeakSanitizer-Bericht ist Testcode. Dabei gefunden und behoben: `uft_track_add_sector()` kopierte `confidence_map`, `weak_mask` und `timing_ns` **flach**, waehrend `uft_sector_cleanup()` sie freigibt — ein latentes doppeltes free im gemeinsamen Helfer | ✅ **Produktionspfad belegt sauber (MF-531)**; Testcode-Lecks offen, ohne Benutzerwirkung |
| P0-12 | **„44 Konvertierungspfade“ beschreibt eine Tabelle, keine Faehigkeit** — angeboten werden **8** (2 verlustfrei, 6 mit Zustimmung); 33 weist das Preflight-Tor als UNGEPRUEFT ab, 3 als unmoeglich. Die Abweisung ist Absicht und richtig; die Zusage war es nicht | ✅ **Zusagen korrigiert (MF-526)**; seither **2 LL-Eintraege verdient** (MF-532: D64→D64, ADF→ADF, bitgleich gemessen). Stand: **3 verlustfrei**, **10 mit Zustimmung** (zwei davon mit **bezifferter** Verlustliste), 3 unmoeglich, **28 offen**. Aufgeteilt (`scripts/audit_convert_backlog.py`): **6 messbar**, **12 ohne Korpus** (Beschaffung), **10 ohne Wandler** (Code) |
