...
| P3-111 | **Offen und ausdruecklich NICHT von mir entschieden: legt ein 1541 die Sektoren physisch interleavt auf die Spur?** `d64_write_track_gcr()` erzeugt das **physische** GCR-Spurbild und ordnet die Sektoren dabei interleavt an. Die Zitatstelle, die das begruenden soll, sagt aber etwas anderes: `fndnxt` traegt den Kommentar *„returns next AVAILABLE track & sector"*, und `nxdrbk` heisst woertlich *„allocate next directory block on 18 and mark as used in bam"* — das ist **Blockvergabe**, nicht Spurbelegung. Ein 1541 formatiert seine Spuren nach verbreitetem Verstaendnis aufsteigend (0, 1, 2, …) und benutzt den Versatz 10 erst beim VERGEBEN von Bloecken, damit aufeinanderfolgende Dateibloecke physisch auseinanderliegen | die Zitatstellen des Eigentuemers gelesen, `d64_write_track_gcr()` im Baum als physischer Spurerzeuger nachgewiesen; ein zweiter Zeuge fuer die physische Reihenfolge NICHT gefunden (MF-859) | ⚠ **offen.** MF-859 hat den sicheren Teil behoben (jeder Sektor genau einmal) und die Reihenfolge fuer Zone 0 unveraendert gelassen — mehr laesst sich ohne eine Quelle nicht rechtfertigen. **Zweite offene Frage aus derselben Ecke:** `lib1541img` kennt ZWEI Regeln — modular (`CFF_SIMPLEINTERLEAVE`) und eine, die beim Umbruch **zusaetzlich um eins verringert** (`freeSectorOnTrack`, die Vorgabe). UFTs bisherige 21er-Tabelle entspricht der **modularen**; lib1541imgs Vorgabe ergaebe fuer 21 Sektoren `0,10,20,8,18,6,…` statt `0,10,20,9,19,8,…`. Beide Fragen brauchen eine Quelle zur physischen Spurbelegung — bis dahin ist jede Aenderung an Zone 0 eine Wahl, keine Messung |
