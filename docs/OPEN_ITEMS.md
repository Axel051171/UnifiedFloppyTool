...
| P3-237 | **Weak-Bit-Erkennung fuer ALLE mehrfach-umdrehenden Formate, nicht nur fuer Greaseweazle-Hardware.** Der Bericht verortet die Luecke an der HAL (`gw_backend_read_flux()` verwirft `fd->index_times`) — das trifft zu und ist nachgemessen. Aber es ist weder der einzige Weg noch der beste erste: **SCP-Dateien tragen Umdrehungen von Haus aus** (`uft_scp_parser.c:381-399`, `data->revolutions[r].flux_data`), ebenso A2R und KryoFlux-Rohstroeme. Ein Verbraucher, der von dort gespeist wird, gilt fuer jedes solche Format **und ist ohne Hardware pruefbar** (MF-310) | im Baum gemessen (MF-949) | ⚠ **offen, naechster Schritt.** `uft_multi_rev_fusion` ist seit MF-949 gemessen und hat eine Tuer; was fehlt, ist der Weg von den SCP-Umdrehungen ueber den Dekoder zu Bitstroemen je Umdrehung. Nach Regel 9 bewegt das keine der vier Kennzahlen unmittelbar — es macht eine im Katalog gefuehrte Faehigkeit erstmals erreichbar, dieselbe Klasse wie P0-2 |
