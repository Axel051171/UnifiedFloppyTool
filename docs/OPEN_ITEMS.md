...
| P3-240 | **Die Fensterwahl der Ausrichtung ist Vorsorge ohne Beleg.** `uft_revolutionen_ausrichten()` vergleicht ein Fenster aus der MITTE beider Stroeme. Der Aufwandsgrund ist gemessen (129 Versaetze x 101 343 Bits; das halbe Fenster halbiert das). Der inhaltliche Grund — dass Fuellung und periodische Luecken an den Raendern zu vielen Versaetzen gleich gut passen — ist **nicht belegt**: die Mutationsprobe „Fenster an den Rand legen" blieb GRUEN | Mutationsprobe 5 von 7 (MF-950) | ⚠ **offen, klein.** Mit den vorhandenen Pruefmustern liess sich kein Fall bauen, in dem die Fensterwahl das Ergebnis aendert. Zwei ehrliche Wege: einen Fall mit periodischem Rand bauen, der die Notwendigkeit zeigt — oder das Fenster auf den reinen Aufwandsgrund zurueckfuehren und das so benennen. Steht als solches im Quelltext, nicht nur hier. Nach Regel 9 bewegt es keine Kennzahl |
