...
| P3-116 | **`d64_write_track_gcr()` hat keinen Aufrufer — soll der Erzeuger bleiben?** Ausserhalb seines Moduls nennt ihn nur der Header-Prototyp. Erreichbar ist allein `d64_gcr_to_flux()`, gespeist aus G64-Spuren; der angebotene Pfad D64→G64 laeuft ueber `uft_d64_g64.c::build_gcr_track()` | ueber `git ls-files` gemessen (MF-861) | ⚠ **offen, Eigentuemer-Entscheidung.** Dieselbe Lage wie bei den vier STX-Lesern (P3-93) und dem AIR-Port: zwei Umsetzungen derselben Sache, eine davon angeschlossen. **Die Frage ist nicht, ob der Code gut ist, sondern ob er gebraucht wird** — MF-859 und MF-861 haben zusammen zwei Sitzungen in eine Funktion investiert, die niemand ruft. Drei Wege: verdrahten (dann braucht der angebotene Pfad einen Grund, weiter `build_gcr_track` zu benutzen), loeschen, oder als ausdruecklich unverdrahtet kennzeichnen. Nach Regel 9 bewegt keiner der drei eine Kennzahl — deshalb ist es eine Aufraeum-, keine Vorrangfrage |
