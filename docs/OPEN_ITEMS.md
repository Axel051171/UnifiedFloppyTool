...
| P3-316 | **Der Baum sagt an acht Stellen „nibtools ist GPL-3.0“ — und ein zweites gemessenes Artefakt sagt Apache-2.0.** Nach MF-636 ist eine solche Angabe eine **rechtliche Aussage**, keine Notiz; sie hat in diesem Baum schon zwei Dateien gekostet (MF-635 entfernte `uft_track_align.c` und den Zwilling) und steuert bis heute Entscheidungen — P3-187 begruendet mit ihr, warum **keine nibtools-Bytes** uebernommen wurden | **gemessen MF-1007, zwei Artefakte, beide echt:** (a) `tools/uft-scout/out/nibtools.gutachten.md:45` haelt fest „Repo-Wurzel | `LICENSE` = **GPL-3.0-Volltext**; Commit `a549c18` „Create LICENSE“, 2025-01-30“ — ein gelesener Volltext, keine Vermutung. (b) Das vom Eigentuemer am 2026-09-10 beigesteuerte `neue-ideen/nibtools-extra.zip` traegt eine **229 Byte** grosse `LICENSE`: „Original software: Copyright (C) 2000 Markus Brenner / Copyright (C) 2005 Pete Rittwage … This product is licensed under the **Apache License, Version 2.0**.“ Im ganzen Archiv nennt **keine** Datei GPL-3 oder „version 3“ (ueber alle 110 Eintraege geprueft); die einzige Lizenzaussage ist diese Datei. **Und sie ist in sich unvollstaendig:** sie schliesst mit „See the LICENSE file for details“ — sie IST die LICENSE-Datei und enthaelt den Apache-Text nicht. Ein Verweis auf sich selbst ist keine vollstaendige Rechtseinraeumung | ⚠ **offen — Eigentuemerfrage, und ich entscheide sie nicht durch Auswahl.** Zwei Moeglichkeiten, beide ohne Beleg von hier aus entscheidbar: upstream hat nach `a549c18` umlizenziert, oder `nibtools-extra` ist eine Abspaltung statt upstream. Was **fehlt**, ist eine Feststellung, welcher Stand heute gilt — mit Fundstelle. <br>**Warum es trotzdem heute nichts blockiert:** der einzige benutzte Kanal ist **Oracle** (ausfuehren, nichts uebernehmen), und dafuer ist beides folgenlos; MF-1005 hat `nibscan` gebaut und ausdruecklich NICHT eingetragen, weil es keine Kennzahl bewegt. <br>**Warum es fuer einen PORT entscheidend waere, und in die unerwartete Richtung:** GPL-3-only vertraegt sich mit UFTs GPL-2-**or-later**, indem die Verbindung GPL-3 wird — genau die Entscheidung, die MF-698 fuer `uft_ipf_air.c` schon getroffen hat. **Apache-2.0 ist mit GPL-2 unvereinbar** und nur einseitig mit GPL-3 vereinbar. Die neue Angabe macht einen Port also nicht leichter, sondern **enger** — wer sie fuer eine Entwarnung nimmt, hat sie falsch gelesen. <br>**Die acht Stellen, die nachzuziehen waeren, sobald der Stand feststeht** (aufgezaehlt, weil sie sonst driften — und dann ist es eine Aufzaehlung mit Datum, kein Ersatz fuer eine Messung): `docs/QUARANTINE.md:48`, `docs/OPEN_ITEMS.md` P3-187, `docs/OPEN_ITEMS.md:2060`, `docs/PLAN_UFT04_24.md:43` und `:66`, `docs/SETTINGS_ROADMAP.md:155`, `tools/uft-scout/out/nibtools.gutachten.md` (fuenf Fundstellen), `tools/uft-scout/out/1541-spurbelegung.gutachten.md:261`, `tools/uft-scout/out/OpenCBM.gutachten.md:206`. `docs/ORACLES.md` ist mit MF-1007 bereits nachgezogen. **Kennzahl (Regel 9): fuenfte Kennzahl** (Dateien mit ungeklaerter Herkunft) — der Eintrag bewegt sie nicht, aber er verhindert, dass eine Zahl auf einer Angabe ruht, die zwei Artefakte verschieden beantworten |
