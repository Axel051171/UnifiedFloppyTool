...
| P3-238 | **Die HAL reicht Umdrehungsgrenzen nicht durch — bestaetigt, aber ohne Hardware nicht abnehmbar.** `src/hal/greaseweazle_backend.c:249-267` kopiert aus `uft_gw_flux_data_t` **nur** `fd->samples` und ruft dann `uft_gw_flux_free(fd)`; `index_times`/`index_count` sterben dort. `uft_hal_read_flux()` (`include/uft/hal/uft_hal.h:187`) hat kein Feld dafuer — eine flache Liste und ein Zaehler. Folge: `uft_fuzzy_bits.c:uft_capture_fuzzy_flux()` fordert fuenf Umdrehungen an, bekommt einen zusammenhanglosen Gesamtstrom und behilft sich mit einer Einzelmessungs-Zeitfenster-Heuristik (`is_ambiguous = !in_window`) | woertlich im Baum nachgelesen (MF-949) | ⚠ **offen.** Der Befund traegt. Zwei Dinge gehoeren dazu: **(a)** die Heuristik BLEIBT — sie beantwortet die Frage „ungewoehnliches Timing", nicht „instabil ueber Umdrehungen"; beide Signale sind verschieden und beide nuetzlich. **(b)** Eine HAL-Erweiterung ist ohne Geraet **nicht abnehmbar** (MF-310); pruefbar waere sie allein gegen den Emulator unter `tests/emulators/greaseweazle/`. Deshalb steht der SCP-Weg davor: dort geht es ohne Hardware. **Offene Rueckfrage aus dem Bericht:** liefern die KryoFlux- und SCP-Backends intern ebenfalls Umdrehungsgrenzen? Sonst funktionierte echte Weak-Bit-Erkennung nur mit EINEM Geraet — das gehoert benannt, nicht stillschweigend so gebaut |
