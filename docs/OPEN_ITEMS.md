...
| P3-353 | **Das SPS-Paket v5.21 sagt in DERSELBEN Ablieferung zwei Dinge, die einander ausschliessen — und davon haengt ab, ob IPF jemals ueber den Kanal *Port* gehen darf.** `neue-ideen/SPStudio_Dev .zip` (781 103 Byte, 85 Eintraege, SPS DECODER LIBRARY v5.21, Quelltext von CAPSImg/LibIPF/Codec/Core) | **gemessen 2026-09-12.** Vier Belege, alle am Objekt: **(1)** `LICENCE.txt` sagt woertlich „This product is licensed under the Apache License, Version 2.0. See the LICENSE file for details.“ — und **eine Datei namens LICENSE gibt es im Paket nicht** (gepruefte Eintragsliste: nur `LICENCE.txt`). **(2)** `RELEASE.txt` sagt woertlich „This software is not free for commercial use; please read the licence.“ — das ist mit Apache-2.0 **unvereinbar**, denn Apache-2.0 erlaubt kommerzielle Nutzung ausdruecklich. **(3)** Das mitgelieferte `CAPSLib520a.pdf` traegt „This document is copyright © 2002-2026 KryoFlux Products & Services Limited“ und verweist fuer die Lizenz auf „the enclosed document with the library revisions“ — also auf (2), nicht auf (1). **(4) Keine einzige Quelldatei traegt einen Lizenzkopf** (drei Handproben nach der Regel des Durchgangs: `Codec/DiskEncoding.cpp`, `Core/CRC.cpp`, `LibIPF/CapsAPI.h` — alle beginnen mit `#include` bzw. `#ifndef`, ohne SPDX und ohne Copyright-Zeile). **Warum das gross ist:** UFTs IPF-Faehigkeit haengt heute an `src/formats/ipf/uft_ipf_air.c` (1042 Z., **GPL-3**-Port von Jean Louis-Guerin), `uft_caps_ipf.c` ist seit MF-1002 geloescht, und CLAUDE.md fuehrt `ipf-flux` ausdruecklich als **Fundus** — „bis die capsimg-Frage steht“. Gilt (1), steht der Kanal **Port** offen, der staerkste nach MF-695, und `ipf` (heute T3) koennte mit dem Erzeuger auf die Welt kommen. Gilt (2), bleibt es bei *Oracle* und *Spec*, und **die Quellen duerfen nicht einmal als Vorlage gelesen werden**, ohne die Zwei-Haende-Brandmauer aus `docs/QUARANTINE_PROCESS.md` §5 | ⚠ offen, **Eigentuemer-Entscheidung — und die einzige, die ich nicht durch Messen ersetzen kann.** „Lizenz vor Faehigkeit“ (Konfliktordnung 2) heisst hier: **bis der Widerspruch von SPS selbst aufgeloest ist, wird aus diesem Paket keine Zeile gelesen, die in den Baum koennte.** Der Weg dorthin ist eine Nachfrage beim Urheber — `RELEASE.txt` nennt dafuer `https://forum.kryoflux.com` und sagt ausdruecklich „use the forums for bug reports, do not submit tickets“; eine Lizenzfrage ist kein Fehlerbericht, also ist der Weg offen, aber er geht ueber einen Menschen. **Was bis dahin erlaubt bleibt und NICHTS kostet:** das Paket als **Spec** zu lesen (das PDF beschreibt CAPS-Satzarten und die FDC-Emulation) und als **Oracle** auszufuehren, falls ein Binaerbau gelingt — beides aendert die Quarantaenelage nicht. **Was NICHT erlaubt ist**, auch nicht „nur zum Ansehen“: der Quelltext als Vorlage fuer eine eigene Umsetzung, denn das ist genau die Kontamination, gegen die `uft-nachbau` seine Brandmauer baut. Kennzahl: **T3 ↓ um 1, wenn (1) gilt** — sonst bewegt sich nichts, und auch das ist eine Antwort |
