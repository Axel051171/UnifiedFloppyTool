...
| P3-244 | **Der GW-Pfad in `uft_hal_unified.c` kann einen Greaseweazle-Flussstrom nicht dekodieren — JEDER Wert ist falsch.** Sein Dekoder behandelt `0xFF` als eigenstaendige Indexmarke und addiert 200 erfundene Ticks; im Protokoll ist `0xFF` ein ESCAPE, dem Opcode und N28-Nutzlast folgen. Ebenso gehoert `0xFE` (254) in den 2-Byte-Bereich 250..254, nicht zu „Space" | Differenzlauf gegen `uft_gw_decode_flux_stream()` an einem erzeugten Strom, ohne Hardware (MF-954) | ⚠ **offen, und der schwerste Punkt dieser Reihe.** Gemessen (2 Umdrehungen, 1000 Wechsel): Produktionsdekoder **2000** Abtastungen `1152 1152 1152 1152 864 …`, dieser Dekoder **2015** Abtastungen `201 1 1 1 1 906` — **2000 von 2000 abweichend**; die 201 ist die erfundene Zugabe. `uft_fuzzy_bits.c` liest ueber genau diesen Pfad, die Kopierschutz-Erkennung bekommt also Muell. **Der Weg ist klar:** Abtretung an `uft_gw_decode_flux_stream()`, dieselbe Klasse wie „wo ein Port neben einem nativen Parser steht, ist der Port richtig" (im Baum sechsmal). Nicht in MF-954 behoben, weil das eine eigene Sache ist und nicht der Beifang einer Schnittstellen-Erweiterung. **Der Differenzlauf braucht keine Hardware** — der Fluss-Generator erzeugt den Strom, beide Dekoder lesen ihn |
