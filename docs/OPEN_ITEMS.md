...
| P3-197 | **Zwei Korpusdateien mit demselben Namensstamm sind zwei verschiedene Disketten — und nichts sagt das. Gemessen MF-923.** `vice_c1541_35trk.d64` trägt im BAM den Diskettennamen **`UFTCORPUS`** (ID `42`, DOS `2A`), `vice_c1541_35trk.g64` trägt **`UFTG64`**. Der gemeinsame Stamm legt nahe, es sei dieselbe Diskette in zwei Formaten; sie ist es nicht | **gemessen** beim Bau des Rotbeweises zu P3-36: eine Zusicherung, die den D64-BAM im G64-Export erwartete, fiel — und zwar zu Recht, aber aus dem falschen Grund. Die Fehlersuche kostete drei Runden, bevor der Plugin-Pfad den echten Namen zeigte | ⚠ **offen — eine Zeile Doku, aber eine wichtige.** Das SHA-256-Manifest neben dem Korpus führt Herkunft und Hash, **nicht den Inhalt**. Wer eine Korpusdatei als Vergleich heranzieht, muss wissen, **was darauf steht** — sonst baut er eine Zusicherung auf eine Vermutung. **Vorschlag:** je Datei eine Zeile „was darauf ist“ (Diskettenname, ID, Spurzahl, Belegung), abgeleitet statt gepflegt — der Plugin-Pfad kann sie erzeugen. **Verwandt:** dieselbe Klasse wie P3-178 (eine Prüfung, deren Grundlage nie gemessen wurde) |
