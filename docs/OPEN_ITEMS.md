...
| P3-184 | **Der D64-Schreiber kannte zwei Ausdehnungen, der Leser vier — und ein ANGEBOTENER Wandlungspfad verschwieg, dass er deshalb Spuren abschneidet. Halb behoben MF-908.** `d64_create()` begann mit `if (num_tracks != 35 && num_tracks != 40) return NULL;`, während `d64_is_valid_size()` seit MF-871 **35, 40, 41 und 42** annimmt (je mit und ohne Fehlerkarte). Der Baum konnte D64-Abbilder **lesen, die er nicht schreiben konnte**. Folge auf dem Wandlungspfad G64→D64, der in `uft_roundtrip.c` als `LOSSY_DOCUMENTED` **angeboten** wird: seine Verlustliste nannte drei Sektoren und die Bauart — nicht die **Spurabschneidung**. Gemessen standen dabei **zwei Türen mit zwei verschiedenen Grenzen, beide ohne Meldung**: `uft_cbm_d64_decode_via_plugin()` sondiert 36..40, `g64_export_d64()` läuft `t <= 35` | Rotbeweis `tests/test_d64_schreiber_symmetrie.c`, **1 von 4 Prüfungen rot** — die übrigen drei sind Wächter (SSOT-Herleitung, Rückrichtung, ungültige Spurzahlen). Der Test treibt **beide echten Türen** an, keine nachgebaute: `d64_create()` und die schmale Prüftür `d64_size_is_valid()` aus MF-871. Gegenprobe **3 Mutationen**: zurück auf 35/40 → die Symmetrie-Prüfung; Blockzahl fest 800 statt SSOT-802 → **beide** Türen unabhängig; 36 zusätzlich annehmen → der Wächter | ⚠ **die Sonde ist gehoben — und dahinter lagen drei weitere Befunde.** Beide erreichbaren Türen suchen jetzt bis 42 und behalten die **höchste** Spur mit Inhalt (vorher: Abbruch beim ersten Treffer, Deckel 40); die Ausdehnung kommt aus **einer** Stelle (`d64_extent_for_highest`), nicht aus zwei Deckeln. Gemessen im Rundlauf: **40 → 41**. Gegenprobe **5 von 5** Mutationen. **42 kommt nicht zurück**, und der Grund ist gemessen: `halftrack = track * 2` ergibt für Spur 42 den Index 84, und `g64_set_track()` weist `>= G64_MAX_TRACKS` (84) ab — **P3-194**. Neu behoben nebenbei: **beide** G64-Encoder meldeten `success = true`, wenn der Schreiber eine Spur abwies (MF-555 las die Antwort, sagte sie aber niemandem) — jetzt `success = false`, `errors_found`, benannte Beschreibung **und** eine Warnung. Dabei gefunden und **nicht** behoben: der Datei-Rundlauf verliert alles ab Spur 22 — **P3-195**, und `g64_export_d64` ist verwaist — **P3-193**. Die zweite Tür wird mit einer **Attrappe** gemessen, weil P3-195 den Dateiweg blockiert; eine Zusicherung, die den geänderten Zweig nie betritt, wäre keine (P3-178)  **Nachtrag MF-925 — und das ist ein Beleg, kein Nachtreten:** die Lösungsskizze in `UFT-25-Selbstprüfung` §5 hätte für „Tür 2“ `for (t = 1; t <= disk->num_tracks; t++)` gesetzt — über eine `track_offset[36]`-Tabelle. Das ist **dieselbe Klasse**, die zwei Tage später als v3-Pufferüberlauf gemessen wurde (MF-923: 5,5×/2,3×/11,9×) — hier aber **vor** dem Code gefangen, weil die Skizze gegen den Baum gehalten wurde statt übernommen. Der Vorgang, den dieser Baum betreibt, hat also beide Enden erwischt: einmal davor, einmal danach. |
