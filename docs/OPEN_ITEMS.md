...
| P3-312 | **UFTs MGT-Leser kennt nur EINE von zwei Spuranordnungen — und beansprucht die Endung der anderen.** MGT legt die Spuren mit **wechselnden Seiten** ab, die IMG-Variante desselben Formats **beide Seiten hintereinander**. Bei gleicher Dateigroesse. `uft_mgt.c` liest `for c { for h }`, also nur die MGT-Anordnung, und fuehrt `.extensions = "mgt,img"` | **gemessen MF-997** gegen `src/samdisk/mgt.cpp` — ein Orakel im **eigenen Baum**. SAMdisks `ReadMGT()` unterscheidet die beiden ausdruecklich und belegt es an einer messbaren Stelle: das Bytepaar am Verzeichnisketten-Versatz muss `dir_tracks` und `2` tragen, und dieser Versatz ist bei MGT `dir_tracks * MGT_SIDES * MGT_TRACK_SIZE + SECTOR_SIZE - 2`, bei IMG `dir_tracks * MGT_TRACK_SIZE + SECTOR_SIZE - 2`. **Bei einer zweiseitigen 80-Spur-Diskette landet in der falschen Anordnung nur Spur (0,0) richtig** — dieselbe Form wie `sad` in MF-794, wo 158 von 160 Spuren von der falschen Stelle kamen, bei einem Format, das bereits auf T1b stand | ⚠ **offen, und der Blocker ist benannt: `src/samdisk/SAMCoupe.h` liegt NICHT im Baum.** Der Algorithmus ist lesbar, seine Konstanten sind es nicht — `di.dir_tracks` kommt aus `GetDiskInfo()`, das mit dem fehlenden Header verschwindet. Die drei uebrigen Groessen (819200, 2 Seiten, 5120 Byte je Spur) hat UFT selbst; `dir_tracks` waere geraten, und geraten wird hier nicht. <br>**Das naheliegende Orakel im Baum taugt NICHT, gemessen.** `tests/corpus_free/gw_sam.img` ist **819200 Byte** — genau die MGT-Groesse —, und „SAM“ ist der SAM Coupe, fuer den MGT gemacht ist. Es traegt aber keine MGT-Struktur: die Datei beginnt mit `55 46 54 00`, also **„UFT\0“**, ein greaseweazle-Pruefmuster; die Typbytes der vier Verzeichnissektoren sind `0x55`, was kein gueltiger MGT-Dateityp ist (1..11). **`uft_mgt_probe()` weist die Datei deshalb ab (Konfidenz 0) — und das ist richtig**, nicht der Fehler: die Groesse stimmt, die Struktur nicht. `uft_format_plugin_sam` nimmt sie mit **35** an, was nach MF-729 genau der Stufe „nur die Groesse“ entspricht. <br>**Drei Vermutungen dieser Runde sind gemessen und zurueckgenommen** — sie stehen hier, damit niemand sie noch einmal anstellt: (1) „das ungeprueffte `mgt` gewinnt das Ranking gegen das geprueffte `sam`“ — **nein**, `mgt` beansprucht die Datei gar nicht; (2) „die uneinheitlichen Trennzeichen in `.extensions` machen vier Plugins unfindbar“ — **nein**, `plugin_claims_extension()` trennt an `;` UND `,`; (3) „die `.img`-Kollision ist neu“ — **nein**, `uft_format_plugin.c:424` fuehrt sie seit MF-449 als bekannten Fall. <br>**Kanal nach MF-695: Spec.** Was fehlt, ist die Lage von `dir_tracks` im Sektor 0 aus einer benannten Quelle — die World-of-Spectrum-Formatbeschreibung, die SAMdisks Kopfkommentar selbst nennt, waere der Weg. **Oder Daten:** ein IMG-Abbild im Korpus wuerde die Unterscheidung messbar machen, ohne die Konstante zu kennen. <br>**Was in der Zwischenzeit gilt und was NICHT getan wurde:** die Endung `.img` bleibt stehen. Sie zu entfernen waere ein Eingriff in die Erkennung mit unklarer Folge — `plugin_claims_extension()` trennt an `;` UND `,` (die uneinheitlichen Trennzeichen im Baum sind also folgenlos, geprueft), und `.img` ist ohnehin von **acht** Plugins beansprucht, was `uft_format_plugin.c:424` bereits als bekannten Fall fuehrt. Eine Absage an dieser Stelle waere keine Verbesserung, sondern eine zweite ungeprueffte Aenderung. **Kennzahl (Regel 9):** ungeprueffte Formate (T3) — `mgt` ist einer der beiden billigsten T3-Einstiege (siehe P3-204) |
