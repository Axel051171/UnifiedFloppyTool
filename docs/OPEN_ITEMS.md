...
| P3-246 | **„Was bedeutet diese Zahl?" — fuenfmal in einer Sitzung dieselbe Fehlerklasse.** Gleicher Name, andere Herkunft, andere Bedeutung | je Fundstelle gemessen (MF-951…955) | ⚠ **offen, und die Abhilfe ist billig.** Die Faelle: `index_times[]` (GW) Tick-Dauer, gelesen als Abtast-Index; `index[]` (KryoFlux) Byte-Position, gebraucht als Abtast-Index; `rev_lengths[]` (SCP) Rohwoerter, nicht Ausgabe-Abtastungen, weil die Schleife Ueberlaeufe ueberspringt; `flux[]` (HAL) Nanosekunden, waehrend der Codec Ticks liefert; die N28-Nutzlast des Fluss-Generators volle Dauer statt Restzeit. Vier davon haetten still falsche Daten erzeugt, einer hat es getan (P3-244). **Vorschlag des Eigentuemers, uebernommen:** jede Position, jeder Versatz, jeder Index bekommt die EINHEIT in den Typ oder in den Namen — `sample_idx`, `byte_pos`, `rev_offset_samples`, `ticks`, `ns`. Ein Durchlauf ueber die HAL-Header, und die naechste stille Verwechslung wird ein Uebersetzungsfehler statt 2000 falscher Werte. Naechster Schritt nach P3-244 |
