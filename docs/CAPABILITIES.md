...
| FluxEngine    | 🟡   | 🟡    | nie       | SIMULATED    | CLI subprocess  | **berichtigt MF-1047.** Hier stand „read+write (CLI-wrapper)“ mit ✅ in beiden Spalten. Gemessen gegen `doc/using.md` des Urhebers (davidgiven/fluxengine; Kanal *Spec*, nur gelesen) trug keine der beiden Zusagen für **Fluss**. *Read:* UFT setzte `-o <datei>` — laut Doku der Ausgang für das **dekodierte Dateisystem-Abbild** („*Reads flux … and decodes it into a file system image*“) — und las das Ergebnis dann als SCP-Flussbehälter. Der Fluss geht über `--copy-flux-to=`; im Beispiel des Urhebers stehen beide **nebeneinander**. Berichtigt, und 🟡 statt ✅, weil der Pfad jetzt die dokumentierte Bedeutung trifft, aber nie an einem Gerät lief. *Write:* von ✅ auf ⬜. `fluxengine write -i` **kodiert** ein Abbild; rohen Fluss schreibt das eigene Unterkommando `rawwrite -s <flussdatei> -d drive:N`. Dazu übergab UFT die `transitions_ns` als rohe 32-Bit-Worte — **kein Behälter, den fluxengine liest** (dokumentiert sind `.flux`, `.scp`, KryoFlux-Strom). Der Pfad konnte nie gelingen, und im ungünstigen Fall hätte fluxengine die Worte als Abbild **auf die Diskette kodiert**. Seit MF-1047 sagt er ab, **bevor** ein Prozess läuft. **Warum es nicht auffiel:** `audit/fluxengine/REPORT.md` meldete für beide Zeilen `PASS (recalled)` — geprüft war die Token**form** gegen eine aus dem Gedächtnis geschriebene Erwartung, nicht die **Bedeutung** gegen das Dokument. Weg zurück: P3-342. **NACHTRAG MF-1116 — Write von ⬜ auf 🟡, und die Absage aus MF-1047 ist eingeloest.** P3-342 nannte zwei Schritte; beide sind getan. (a) Der Fluss geht jetzt als **SCP-Behaelter** zu fluxengine — `do_write_raw_flux` ruft `scp_writer_create/add_track/save` (seit MF-1055 abgenommen, Mutationsmatrix 8 von 8) und legt die Datei im System-Temp an. (b) Der Befehl ist `rawwrite -s <datei> -d drive:0 --tracks=cNhM` statt `write -i` — **ohne `-c <profil>`**, weil rawwrite nicht kodiert und ein Profil dort ein Argument ohne Gegenstand waere. **Abgenommen ist die VORBEREITUNG, und die schaerfste Zusage sitzt im Laeufer:** `tests/test_fluxengine_befehl.cpp` prueft im Mock-Laeufer, ob die Datei zum Zeitpunkt des Aufrufs wirklich existiert, groesser als 0 ist und mit der Kennung `SCP` beginnt — sowie dass stdin LEER bleibt (`rawwrite -s` liest die Datei selbst) und die Wegwerfdatei danach entfernt ist. **🟡 und nicht ✅:** ob ein echtes `fluxengine` die Datei annimmt und ein Laufwerk sie schreibt, ist nicht belegt — keine Hardware (MF-310), und `fluxengine` liegt nicht im Baum. **Mit `verify=true` wird abgesagt statt behauptet:** die Nachlese ist nicht verdrahtet, und ein `verified = true` ohne Nachlese waere die Zusage ohne Tat aus MF-883. **Die Tests, die die Absage bewachten, sind dabei GEFALLEN — und es war nicht einer, sondern DREI.** Hier stand „der alte Test“, Singular; gemessen am Lauf sind es `smoke_write_raw_flux_sagt_ab`, `smoke_write_verify_unerreichbar` und `smoke_write_raw_flux_failure`. Rotbeweis in der Gegenrichtung, und sie sollen fallen. **Beim dritten ist die Zusage ZURUECKGEKEHRT statt gewechselt:** vor MF-1047 prueffte er einen fehlgeschlagenen `fluxengine`-Lauf — genau was sein Name sagt —, MF-1047 nahm ihm den vorgemerkten Lauf, weil der Pfad absagte, und seit MF-1116 ist derselbe Pfad wieder erreichbar. Jetzt prueft er wieder, dass ein Fehlschlag als ProviderError ankommt, **mit dem stderr des Werkzeugs im Text**; die Zeichenfolge steht im FluxEngine-Provider nirgends, kann also nur ueber den Mock hereinkommen — gruen aus dem richtigen Grund. **Und gefunden hat ihn kein Ueberlegen, sondern eine Messung:** er stand auf keiner meiner beiden Verdachtslisten, beide verdaechtigten Bereichspruefungen brechen VOR dem Laeufer ab, und `terminate` liess keinen Rueckverfolger-Rahmen uebrig (`#0 <unavailable> in ?? ()`). Seit MF-1116 nennt `main()` deshalb jede Stufe mit `std::endl` — ohne Leerung waere die Zeile genau dann im Puffer verloren, wenn man sie braucht. **NACHTRAG MF-1121 — die NACHLESE ist verdrahtet, auf Eigentuemer-Entscheidung „Vergleich auf Zellebene", und P3-342 ist damit vollstaendig.** `verify = true` liest die Spur jetzt wirklich zurueck (eine Umdrehung ueber `do_read_raw_flux`) und vergleicht **Zellzahlen** statt Nanosekunden. **Das Maß ist kalibrierungsfrei, und das ist der Kern:** die Zellzeit kommt aus dem Histogramm des GESCHRIEBENEN Stroms (`uft_flux_histogram_cell_ns`, im Produktivpfad des Dekoders gerufen). Antwortet der Schaetzer nicht — der Strom sieht nicht wie MFM aus —, wird **abgesagt statt geraten**; eine eigene Toleranz waere eine erfundene Zahl. Ausdruecklich NICHT benutzt wird `uft_pll_classify_flux()`: es sortiert gegen FESTE 4/6/8-µs-Fenster, also gegen eine angenommene Zellzeit, und fuer GCR oder FM landet dort alles in `TOO_LONG` — ohne das zu sagen. **Drei Groessen sind an einer Wegwerfmessung belegt:** die Einheit geht 1:1 durch (ns hinein, ns heraus), der Schaetzer braucht **rund 60** Uebergaenge (mit 12 sagt er nein), und der Absage-Zweig ist erreichbar (ein einzelner Gipfel und ein Verhaeltnis von 1,25 werden abgewiesen). **Die Ausrichtung ist geschenkt statt gesucht:** SCP trennt Umdrehungen an den Indexmarken, Umdrehung 0 beginnt also AN der Marke, und `rawwrite` schreibt ab Index — deshalb genau eine Umdrehung und kein Rotationssuchlauf (der waere bei ~100 000 Uebergaengen quadratisch). Verglichen wird das **Praefix**: die geschriebenen Zellen stehen in dieser Reihenfolge am Anfang der Umdrehung; was dahinter liegt, stand vorher auf der Diskette. **Abnahme: vier Faelle in `tests/test_fluxengine_provider_v2.cpp`** — gleiche Zellfolge ⇒ `verified = true`; eine um mehr als eine halbe Zelle verschobene Stelle ⇒ `WriteVerifyFailed`; **Jitter INNERHALB der Zelle (+200 ns auf jeden Wert) ⇒ haelt**, denn genau das ist auf echter Hardware der Normalfall und ein ns-Vergleich wuerde daran immer scheitern; kuerzerer Ruecklesestrom ⇒ faellt, auch wenn jede vorhandene Zelle stimmt. **Mutationsmatrix 5 von 5, und jede Mutation faellt an IHRER eigenen Zusage** — Nanosekunden statt Zellen am Zellbeweis, uebersprungener Vergleich an der verschobenen Zelle, entfernte Laengenpruefung am kurzen Strom, entfernte Absage am Mock (kein Lauf vorgemerkt), zwei Umdrehungen an `--drive.revolutions=1`. **Erst im zweiten Lauf, und die erste Runde hat zwei Fehler in der MATRIX gefunden:** „Nanosekunden statt Zellen" war zunaechst wirkungslos, weil die Klemmung `n > 255` jeden ns-Wert auf 255 abbildet — die Mutation verglich also nichts statt Nanosekunden und wurde von der falschen Zusage gefangen; und „Laengenpruefung weg" war nicht isolierbar, weil die Laenge an ZWEI Stellen geprueft wird (Gestalt von MF-1031) und die Mutation zusaetzlich ueber das Feldende hinaus las. **Was `verified = true` heisst und was nicht:** das Zurueckgelesene ergibt dieselbe Zellfolge. Nicht, dass ein fremdes Laufwerk die Diskette liest — deshalb bleibt Write bei 🟡, nicht ✅ (MF-310). |
| FC5025        | ✅   | -     | nie       | SIMULATED    | fcimage CLI     | read-only (fcimage-wrapper). **Geprüft MF-1045:** das `-` hält im Ergebnis — im ganzen Baum gibt es **keinen** FC5025-Schreiber (keine `.c` unter `src/hal/`, keine Deklaration in `include/uft/hal/uft_fc5025.h`). Seine Begründung war bis MF-1107 ebenfalls ungemessen: `fc5025_provider_v2.h` nennt „*no write CBW opcode exists*”, und `audit/fc5025/evidence.json` führt die Opcode-Tafel selbst als `”cbw_opcodes”: “needs-source”` — „*CBW/CLI layer not establishable*”. **Seit MF-1107 ist das `-` belegt, und zwar von erster Hand:** der Hersteller schreibt auf seiner eigenen Produktseite wörtlich „**The FC5025 is read-only. It cannot write to floppies.**” (`shop.deviceside.com/prod/FC5025`, abgerufen 2026-09-14). Das ist genau die Art Aussage, die die Legende für ein `-` verlangt — eine über das **Gerät**, von dem, der es baut. **Der Gegenfall macht den Unterschied sichtbar:** bei KryoFlux sagte dieselbe Spalte dasselbe, und das Handbuch des Urhebers widersprach (`-w : write image to disk`, MF-1046). Zwei gleich aussehende Zeichen, zwei entgegengesetzte Messergebnisse — das ist der Grund, warum jede von ihnen eine eigene Quelle braucht. **Offen bleibt eine ANDERE Frage:** ob das USB-Protokoll einen Schreib-Opcode kennt, ist damit nicht beantwortet (`cbw_opcodes` bleibt `needs-source`). Für die Fähigkeitszusage ist sie gegenstandslos — UFT spricht FC5025 ohnehin über die `fcimage`-CLI, nicht über rohe CBWs |
...
| ADF-Copy      | 🟡   | ⬜    | nie       | Emulator-Test | USB-CDC serial  | QSerialPort verdrahtet, **Protokoll ungeprüft** (Opcodes aus eigenen Kommentaren, P3-31). **Berichtigt MF-1106:** hier stand `SIMULATED`, und die Legende oben definiert das als „Tier-2.5 SIMULATED in `tests/hil/run_simulated.py`". Gemessen enthält dessen `CHECKS`-Liste **zehn Einträge über sechs Controller** (kryoflux, fluxengine, fc5025, scp-direct, xum1541, greaseweazle) — ADF-Copy ist **keiner davon**. Was es gibt, ist `test_adfcopy_emulator` unter ctest (#435), und genau das steht jetzt hier. Dieselbe Bewegung wie MF-1045: ein Zeichen, das mehr sagte als die Messung dahinter |
| USB-Floppy    | Linux| Linux | nie       | Emulator-Test | SG_IO ioctl     | Linux-only via SG_IO; Win/Mac (DeviceIoControl/IOKit) **weiter offen**. **Berichtigt MF-1106**, aus demselben Grund wie bei ADF-Copy eine Zeile darueber: `SIMULATED` verweist laut Legende auf `tests/hil/run_simulated.py`, und USB-Floppy steht in dessen `CHECKS`-Liste nicht. Vorhanden ist `test_ufi_emulator` unter ctest (#436) — und der treibt laut der Bench-Tafel unten den **Produktions-HAL** (`src/hal/ufi.c`) ueber ein eingespeiztes `uft_ufi_ops_t`, ist also mehr als ein Mock. Das gehoert genau so benannt statt unter einem Zeichen, das eine andere Messung meint **Berichtigt MF-1045:** in der Sim-Spalte stand `-`. Es gibt eine Simulation — `tests/emulators/ufi/` läuft als `test_ufi_emulator` (ctest #401) und treibt den **Produktions-HAL** `src/hal/ufi.c` über den einspeisbaren `uft_ufi_ops_t`-Unterbau |
...
- **1/9 honest-stub mit Emulator-Test:** ADF-Copy. **Berichtigt MF-1106:** hier stand „mit Sim“, und das las sich als Tier-2.5-Simulation im Sinne der Legende. `tests/hil/run_simulated.py` kennt ADF-Copy nicht; was es gibt, ist `test_adfcopy_emulator` unter ctest.
- **Tier-2.5 nach dem Buchstaben der Legende, gemessen MF-1106:** `run_simulated.py` fuehrt **zehn Pruefungen ueber sechs Controller** — kryoflux (2), fluxengine (4), fc5025 (1), greaseweazle (1, Uebersetzungslauf) sowie scp-direct und xum1541 ueber USB-Mocks. Die dahinterliegenden Simulatoren in `tools/hw_simulators/` sind **drei**: `dtc`, `fcimage`, `fluxengine`. **Applesauce, ADF-Copy und USB-Floppy stehen in keiner dieser beiden Mengen.** Sie haben stattdessen je einen Emulator unter `tests/emulators/` mit einem Test, der unter ctest laeuft — neun von neun tun das (#428-#436). Zwei verschiedene Ebenen, und die Tafel nennt sie jetzt getrennt.
