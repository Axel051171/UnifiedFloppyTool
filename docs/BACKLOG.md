...
| **A2** | **30 von 44 Wandlungspfaden ungeprüft.** Das Preflight-Tor weist sie ab — richtig. Aber die Doku nennt weiter „44 Pfade" als Fähigkeit | gemessen | **offen**; Zahlen seit MF-541 abgeleitet — **aber nur in `CLAUDE.md`**. MF-983 hat vier weitere Dokumente nachgezogen (`MASTER_PLAN`, `SUBSYSTEM_MATURITY`, `SHOWCASE`, dieses) und die Ableitung auf sie ausgedehnt; hier stand „29“ |
...
| # | Sache | Herkunft | Stand |
|---|---|---|---|
| **C1** | **„55+ Kopierschutz-Schemes“ steht als Kernfunktion in `CLAUDE.md` und `README`.** Gemessen (MF-1333): **40 Dateien, 365 Funktionen, 9 von aussen gerufen, 42 von einem Test beruehrt** (MF-876: 33). Vorher (MF-851) stand hier (38, 358, 4, 30), und von den vier war eine ein CRC-Helfer. **328 Funktionen sind weder verdrahtet noch geprueft** (358 minus die 30 beruehrten; frueher 337 aus 359 minus 22). Die Bewegung gegenueber (37, 350, 4, 16) kommt von `uft_schutzbefund.c`: ein neuer Berichtsvertrag mit fuenf Testfaellen. **Von aussen gerufen bleibt bei 4** — auch das Neue ist noch nicht verdrahtet, und das steht hier, damit die gestiegene Testzahl nicht wie Anschluss aussieht. **MF-793: zweiter Detektor (FZS, Fuzzy-Sektor), neun Testfaelle — und die vier Zahlen bewegen sich um NULL** (38/359/4/22, nachgemessen). Das ist kein Versaeumnis, sondern die Aussage: ein Detektor ohne Aufrufer aendert an C1 nichts, egal wie gut er geprueft ist. Wer die Zahl bewegen will, muss verdrahten, nicht bauen. **MF-876: (39, 361, 9, 33) — und zum ersten Mal bewegt sich die Zahl, die C1 eigentlich meint.** „Von aussen gerufen" springt von **4 auf 9**. Grund: `ForensicTab::detectProtection()` — der EINZIGE Schutzerkenner, den ein Benutzer in diesem Baum erreicht — ruft jetzt `uft_schutz_aus_d64()` und damit vier Funktionen aus `uft_schutzbefund.c` (`bericht_init`, `_add`, `_uebersprungen`, `_frei`) plus `uft_schutz_code_name`/`uft_uebersprungen_name`. Der Berichtsvertrag aus MF-792, der seit drei Eintraegen „gebaut, aber nicht verdrahtet" war, hat einen Produktionsaufrufer. **Was das NICHT heisst:** die 55+ benannten Verfahren bleiben unerreichbar; verdrahtet ist der BERICHT, nicht der KATALOG. Die Differenz 361 minus 33 = 328 ungeprueft steht unveraendert. **MF-943: (39, 361, 9, 37).** „Von einem Test beruehrt“ steigt von 33 auf 37, weil `tests/test_copylock_seed_wird_nicht_erfunden.c` vier Funktionen aus `uft_copylock.c` anfasst. **Von aussen gerufen bleibt bei 9** — also genau der Fall, den dieser Eintrag seit MF-793 beschreibt: Pruefung bewegt die Zahl, die C1 NICHT meint. Der Anlass war allerdings kein neuer Detektor, sondern ein Befund: `uft_copylock_extract_seed()` gab einen aus Byte-Positionen geratenen Seed als zurueckgewonnen aus, und `uft_protection_classify.c:433` machte daraus `reconstructable` (P3-224). Die Differenz 361 minus 37 = **324** ungeprueft. **MF-958: (39, 363, 9, 39).** Zwei Funktionen mehr, zwei beruehrte mehr, **von aussen gerufen unveraendert 9** — und damit zum vierten Mal in diesem Eintrag genau das Muster, das er beschreibt: die Zahl, die C1 MEINT, bewegt sich nicht. Die beiden Neuen sind `uft_fuzzy_timings_aus_flux_ns()` und `uft_fuzzy_flux_ns_aus_timings()`, herausgezogen aus `uft_capture_fuzzy_flux()` bzw. `uft_write_fuzzy_flux()`, damit die Rechnung ohne Geraet pruefbar wird (P3-253). Sie werden nur INNERHALB von `src/protection/` gerufen, zaehlen also nicht als verdrahtet. Und die beiden Traeger selbst haben nach wie vor keinen Produktionsaufrufer — die fuenf dort behobenen Fehler waren latent, nicht scharf. Die Differenz 363 minus 39 = **324** ungeprueft, unveraendert. **MF-1333: (40, 365, 9, 42).** Eine Datei mehr (`uft_geos_rebuild.c`, der transaktionale Raeumer der GEOS-Schutzspur), zwei Funktionen mehr, drei beruehrte mehr — und **von aussen gerufen unveraendert 9**, zum fuenften Mal in diesem Eintrag dasselbe Muster. Die drei beruehrten sind die beiden neuen plus `uft_geos_analyze_disk()`, das seit MF-1333 ueberhaupt zum ersten Mal von einem Test angefasst wird: `tests/test_geos.c` prueft trotz seines Namens ein ANDERES Modul (`src/formats/c64/uft_geos.c`, gemessen an dessen `#include`). Bis dahin meldete der Detektor drei Schutzarten, von denen keine aus einer Messung am Datentraeger stammte. Die Differenz 365 minus 42 = **323** ungeprueft. **MF-851: (38, 358, 4, 30).** Die Funktionszahl faellt um eins, weil `uft_protection_detect_tests()` weg ist — sie stand in einem `#ifdef UFT_UNIT_TESTS`-Block, den keine Bauvorschrift definiert, wurde also nie uebersetzt und zaehlte trotzdem mit. **Von einem Test beruehrt: 22 → 30**, weil dieselben acht Zusagen jetzt unter `tests/test_schutz_erkennung_lebt.c` in CI laufen. **Von aussen gerufen bleibt bei 4** — und das ist der Punkt: acht Funktionen sind aus „ungeprueft" in „geprueft" gewandert, ohne dass eine einzige verdrahtet waere. Die Zahl, die C1 eigentlich meint, hat sich nicht bewegt | gemessen (MF-557) | ⚠ **ueberwacht, nicht verdrahtet.** Den Katalog anzuschliessen hiesse, 328 ungeprueste Funktionen an ein forensisches Urteil zu haengen (hier stand 334 neben 337 im selben Eintrag — zwei Zahlen fuer dieselbe Groesse; beide kommen jetzt aus derselben Subtraktion) — genau die Lage, aus der die fuenf fabrizierten Parser kamen. Die Oberflaeche sagt es bereits von sich aus (`ProtectionAnalysisWidget.cpp`); seit MF-557 haelt `scripts/audit_protection_claims.py` die Zahl fest |
...
| ~~**C15**~~ | **Das Bedarfs-Tor der Schutzerkennung prueft beim Fluss die DATEN, beim Dekodierten aber eine BEHAUPTUNG.** `braucht_fluss` testet `!p->fluss_ns`, `braucht_dekodiert` testete `!p->dekodiert_vorhanden` — ein `bool`, hinter dem nichts stand | gemessen (MF-793) | ✅ **MF-793** — beide Zweige standen **nebeneinander in derselben Funktion**, und der Unterschied fiel nicht auf, solange kein Detektor dekodierte Sektoren wirklich anfasste. Gemessen an einer Probe mit `dekodiert_vorhanden = true` und `sektoren = NULL`: das Tor liess FZS durch, FZS fand nichts, kehrte zurueck — der Bericht meldete **null Befunde UND null Uebersprungene**. Also genau die Verwechslung „nicht gefunden" / „nicht geprueft", gegen die diese Datei geschrieben wurde, **im Tor der Datei selbst**, sechs Tage nach ihrer Entstehung. Das `bool` ist entfernt (drei Dateien, kein Fremdnutzer), die Anwesenheit der Sektoren entscheidet. Gehalten von `mit_fluss_und_normaler_laenge_ist_wirklich_sauber`, das jetzt `skips(&b, UFT_SCHUTZ_FZS) == 1` verlangt |
