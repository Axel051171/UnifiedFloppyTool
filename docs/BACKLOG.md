...
| # | Sache | Herkunft | Stand |
|---|---|---|---|
| **C1** | **„55+ Kopierschutz-Schemes“ steht als Kernfunktion in `CLAUDE.md` und `README`.** Gemessen (MF-792): **38 Dateien, 359 Funktionen, 4 von aussen gerufen, 22 von einem Test beruehrt** — und eine der vier ist ein CRC-Helfer. **337 Funktionen sind weder verdrahtet noch geprueft.** Die Bewegung gegenueber (37, 350, 4, 16) kommt von `uft_schutzbefund.c`: ein neuer Berichtsvertrag mit fuenf Testfaellen. **Von aussen gerufen bleibt bei 4** — auch das Neue ist noch nicht verdrahtet, und das steht hier, damit die gestiegene Testzahl nicht wie Anschluss aussieht. **MF-793: zweiter Detektor (FZS, Fuzzy-Sektor), neun Testfaelle — und die vier Zahlen bewegen sich um NULL** (38/359/4/22, nachgemessen). Das ist kein Versaeumnis, sondern die Aussage: ein Detektor ohne Aufrufer aendert an C1 nichts, egal wie gut er geprueft ist. Wer die Zahl bewegen will, muss verdrahten, nicht bauen | gemessen (MF-557) | ⚠ **ueberwacht, nicht verdrahtet.** Den Katalog anzuschliessen hiesse, 334 ungeprueste Funktionen an ein forensisches Urteil zu haengen — genau die Lage, aus der die fuenf fabrizierten Parser kamen. Die Oberflaeche sagt es bereits von sich aus (`ProtectionAnalysisWidget.cpp`); seit MF-557 haelt `scripts/audit_protection_claims.py` die Zahl fest |
