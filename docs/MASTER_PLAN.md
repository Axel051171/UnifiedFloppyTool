...
- 88 Format-Plugin-Registrierungen (138 Format-IDs); Verifikations-Stand
  ...
  Tests mit.)
- 44 Konvertierungspfade registriert, 20 Roundtrip-Matrix-Einträge
...
| UFT-004 | ✓ CLOSED MF-260 | `uft_format_plugin_t` bekam `api_version`-Field + `UFT_PLUGIN_API_VERSION` Macro + Runtime-Gate (`uft_register_format_plugin` lehnt `api_version > host` ab, warnt bei `== 0` als Legacy). Static_assert pinnt `sizeof == 240` (MinGW-w64 x86_64; war 216, dann 224 in **MF-404** wegen `read_half_track`, dann 240 in **MF-665** wegen `variants` + `variant_count` vor `api_version` — beides reine Anfügungen, kein Reorder, alle Plugins nutzen Designated Initializer. Die 240 sind GEMESSEN (`sizeof` ausgegeben, gcc 13.1.0), nicht gerechnet; +16 ist genau Zeiger plus `size_t`. Kein API-Versionssprung: quelltextverträglich, und externe Plugins gibt es nicht.) Die Prozedur im Header verlangt ausdrücklich, den Wert *auch hier* zu bestätigen; MF-404 hatte das versäumt, nachgeholt in MF-409. Test `tests/test_plugin_abi.c` mit 8 Assertions: api_version-Macro, struct-size-floor, field-offset-bound, registrar-reject-null/unnamed/future, accept-current/legacy. | — | include/uft/uft_format_plugin.h:516-580, src/core/uft_format_plugin.c:30-65, tests/test_plugin_abi.c |
...
offen führen, eine Struct-Größe von 216 nennen (real 240 seit MF-665) und
