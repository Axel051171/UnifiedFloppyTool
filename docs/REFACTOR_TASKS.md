...
| P1.21 | Migrate `FluxWriteJob`   to V2 outcome surface (drop uft_gw_* direct path) | P1.18      | 🔄 — Drive-unit-Infrastruktur steht (MF-199); verbleibend: `FluxWriteJob`-Migration spiegelbildlich zu P1.20 (`write_raw_flux` + `WriteFluxParams`). Nutzt bis dahin `raw_handle()` (WorkflowTab reicht es weiter). |
