...
Stand der erklaerten Auswahl: 2026-09-17 (MF-1226; vorher 2026-09-15
MF-1149, davor 2026-08-30 MF-693). **Und diese Zeile stand drei Commits
lang falsch:** MF-1222 (`disk-analyse`), MF-1223 (`wav2cas`) und MF-1224
(`hdm_to_fdi`) haben je eine Zeile ergaenzt und das Datum nicht
nachgezogen — gemessen an dieser Datei, nicht vermutet. Genau die
Zahlendrift, gegen die MF-541 die abgeleiteten Zahlen eingefuehrt hat;
hier ist sie nur nicht abgeleitet, weil diese Tafel eine ERKLAERTE
Auswahl ist und keine vollstaendige (die vollstaendige nennt
`ctest -R oracle_registry`, Stand **30** Eintraege). Eine abgeleitete
Tafel waere besser und steht als **P3-404**.
...
| `disk-analyse` | `DISK_ANALYSE` | **Unlicense / public domain** — am `COPYING` **im Paket** gemessen (1211 Byte), nicht nur am Ursprung nachgelesen. Die sauberste Lage im Baum: hier wäre sogar ein **Port** erlaubt; gebraucht wird er nicht | **keine** Versionsabfrage (der argumentlose Aufruf druckt nur die Gebrauchsanweisung). Dreiteilig wie bei `to_woz2`: Quellstand (`libdisk/container/eadf.c` 5593 Byte, sha256 `f56103d17290a3c5…`, byteidentisch mit dem Ursprung) + Baurezept (`mingw32-make SHARED_LIB=n`, gcc 13.1.0, **erster Versuch rc 0**) + Ausgabe-SHA für eine benannte Eingabe (`6d6f73504c37bf82…`, zweiter Lauf identisch). Binär: `ce1864de0c410ed9…` | **Der erste ERZEUGER für Extended ADF (`adf_ext`), seit MF-1222** — und er lag die ganze Zeit im Baum, ungebaut (`neue-ideen/disk-utilities-master.zip`). Warum der Erzeuger-Zensus ihn nicht fand, ist gemessen: er ordnet Werkzeuge über **Endungen** zu (`d["ext"] & exts`), und `adf_ext` teilt `adf` mit dem längst gehobenen `adf`. Direkt nach Modulnamen gefragt sagen die drei gebauten Familien dasselbe Nein — hxcfe `AMIGA_EXTADF;R` (nur lesen), floptool kein Modul, libdsk keiner seiner 27 Typen. **Der Wert liegt im Umweg** (MF-1084): ADF→ADF könnte durchreichen, ADF→**Zellstrom**→ADF muss Sync, Kopffeld, Prüfsummen und Anordnung modellieren. Gemessen liest UFTs eigener Dekoder daraus **1760 von 1760** Sektoren an ihrer eigenen Ortsmarke. **Zwei Fallstricke, beide gemessen:** `-c` mit einem Windows-Pfad endet mit rc 1 und „could not open config file", obwohl die Datei existiert — `config.c::open_file()` hält mit `if (name[0] != '/')` auch `C:/…` für relativ; und Schreib- **wie** Leseformat werden über die Endung gewählt, weshalb dieselbe Datei beim Schreiben `.eadf` heißen muss und hxcfe sie nur als `.adf` lädt. **Was es NICHT entscheidet:** AmigaDOS-Spuren im Behälter (`eadf_close()` setzt `thdr.type = htobe16(1)` einmal vor der Schleife — Typ 0 entsteht nie) und die HD-Spielart mit 22 Sektoren |
| `wav2cas` (+ `cas2wav`, `casdir`) | `WAV2CAS` | **GPL-2.0**, am `COPYING` **im Paket** gemessen (18 009 Byte). Lage wie `hxcfe`: ausführen ja, übernehmen nein | **keine** Versionsabfrage — bei falschem Aufruf drucken die Werkzeuge nur ihre Gebrauchsanweisung; die `Version History` steht im README, nicht im Programm. Dreiteilig wie bei `to_woz2`: Quellstand (master-Tarball, 8 Einträge, 15 289 Byte; `cas2wav.c:69` trägt die Kennung) + Baurezept (`mingw32-make`, gcc 13.1.0, **erster Versuch rc 0**) + Ausgabe-SHA für eine benannte Eingabe (`878684b4902d68fa…`, zweiter Lauf identisch) | **Der erste ERZEUGER für MSX-Kassetten (`cas`), seit MF-1223.** `cas` stand seit MF-1040 auf T2 mit benannter Begründung — „MAMEs Lader wandelt CAS in WAV-Abtastwerte und gibt keine Blöcke zurück", also **kein fremder Erzeuger**. **MAMEs `imgtool` wurde dafür geprüft und VERWORFEN:** es kennt ein `fmsx_cas`-Modul, aber sein Code *ist* die Referenz, gegen die UFTs Leser abgenommen wurde — das wäre die `dms`-Falle (MF-1135). castools ist unabhängig, gemessen am Quelltext: `cas2wav.c:69` deklariert `1F A6 DE BA CC 13 7D 74` selbst, und `wav2cas.c` nennt die MSX-Blocktypen `0xD0`/`0xD3`/`0xEA` **0 Mal** — ein reiner Signaldekoder, der keinen Inhalt deutet und die Blockstruktur folglich nicht erraten kann. **Der Wert liegt im Umweg** (MF-1084): 1584 Byte → **1 853 036 Byte FSK-Wellenform** (43 200 Hz, 8 Bit mono) → 1584 Byte, **0 abweichend**, mit vier protokollierten Kopf/Daten-Paaren. **`casdir` ist die dritte Hand** und entscheidet über die Wohlgeformtheit: es muss `UFTKOR  binary` nennen, sonst liefert das Rezept nichts aus. **Fallstrick, gemessen:** `cas2wav` ohne Ausgabedatei endet mit rc 1 und druckt nur die Gebrauchsanweisung — es ist kein Filter auf stdout. **Was es NICHT entscheidet:** Bänder ohne BIOS-Laderoutinen; das README nennt das ausdrücklich („a lot of Gremlin games used a entirely different format"), und dafür liegt kein Abbild vor |
| `hdm_to_fdi` | `HDM_TO_FDI` | **KEINE.** Das Paket hat keine Lizenzdatei, und im ganzen README (3549 Byte, **vollständig** gelesen) steht kein Lizenzwort — gemessen, nicht überflogen; ohne Rechteeinräumung gilt der Vorbehalt aller Rechte (`P3-356`). **Eigentümerentscheidung vom 2026-09-17**, wörtlich: *„Lizenzentscheidung (fdi_pc98, nfd) ja wir machen das"*. Gehandhabt wie `dtc` und `epstool`: **ausführen ja, weitergeben nein** — nichts aus dem Paket wandert in den Baum | **keine** Versionsabfrage (reines Python, kein Bau). Dreiteilig: Quellarchiv-SHA `90505a65d15ef707…` + Quellstand (`hdm_to_fdi.py` 2558 Byte) + Ausgabe-SHA für eine benannte Eingabe (`880ce8ab84649d96…`, zweiter Lauf identisch) | **Der erste ERZEUGER für PC-98-FDI (`fdi_pc98`), seit MF-1224 — und der schwächste Beleg dieser drei Hebungen, was hier stehen muss und nicht in einer Fußnote.** Er **modelliert nichts**: sein ganzer Rumpf ist `pack('<8L4064x', …)` plus `fdi_header + hdm_blob`, und die Geometriewerte sind **fest verdrahtet** (1024/8/2/77), nicht aus der Eingabe abgeleitet. Belegt ist die **Behälter-Zerlegung** — dieselben acht Dwords an denselben acht Versätzen —, **nicht** die Geometrie-Herleitung. Mit nur EINER fremden Hand wäre das nahe an der „Gleichheit ohne Aussage", die MF-1039 bei `cpm` abgelehnt hat; deshalb verlangt `gen_fdi_pc98_corpus.py` **`hxcfe` als dritte Hand** und liefert sonst nichts aus: es muss die Datei mit `NEC_FDI` laden und daraus eine **IMD** schreiben, die je Sektor Zylinder, Kopf und Nummer ausdrücklich nennt — gemessen **1232 von 1232** an ihrer Stelle, 0 abweichend, **ohne UFT** (Muster von MF-1037). **Zwei Fallstricke, beide gemessen:** die Endung `.fdi` beanspruchen **zwei** hxcfe-Module (`ZXSPECTRUM_FDI` und `NEC_FDI`), deshalb bricht das Rezept ab, wenn nicht `NEC_FDI` geladen wurde; und das Werkzeug **warnt** bei anderen Größen als 1 261 568 Byte und **bricht nicht ab** — es schreibt dann Konstanten, die nicht zur Nutzlast passen. **Was es NICHT entscheidet:** `nfd` (das Paket erwähnt NFD in keiner seiner 14 Dateien) und die übrigen PC-98-Spielarten HD5/HD4/DD6/DD9/HDB/DDB, die sein eigener Kopfkommentar aufführt, während sein Code nur 2HD kann |
| `fdtc` (+ `bintofdf`) | `FDTC` | **BSD-3-Clause**, „Copyright 2024 Matthew Gilmore" — am `COPYING` **im Paket** gemessen (1457 Byte, alle drei Standardklauseln). **Und das ist die Lehre dieses Eintrags:** die GitLab-API meldete `license: KEINE`, weil sie nur die **Projekteinstellung** liest, nicht den Inhalt — `P3-474` hat das übernommen und „alle drei ohne jede Lizenz" geschrieben. Dieselbe Falle wie LisaEms `NOASSERTION`, das gemessen GPL-2 ist. Wer eine Lizenz feststellt, liest die **DATEI**. Permissiv wie MAMEs BSD-3: hier wäre sogar ein Port erlaubt, gebraucht wird er nicht | **keine** Versionsabfrage (die Skripte drucken bei falschem Aufruf nur `Usage:`). Dreiteilig wie bei `to_woz2`: Quellstand (`fdtc.sh` 3331 Byte, `bintofdf.sh` 3093 Byte) + Aufrufweg (`sh fdtc.sh` mit der Kopfzeile auf stdin) + Ausgabe-SHA für eine benannte Eingabe (`d58dc05a094eacd2…` nach dem fwNES-Kopf von `fdstool`, zweiter Lauf identisch) | **Der erste ERZEUGER für Famicom Disk System (`fds`), seit MF-1226 — und er baut den GANZEN Behälter, nicht nur eine Hülle.** `bintofdf` setzt den **Typ-3-Dateikopf** (16 Byte: Dateinummer, Code, 8-Byte-Name, Ladeadresse, Größe, Art) und hängt `0x04` plus Nutzlast an; `fdtc` setzt den **Typ-1-Block** (56 Byte mit `*NINTENDO-HVC*`, Titel, Showa-Datum, Region) und den **Typ-2-Block** (Dateizahl) davor und füllt die Dateinummern ein. Von UFT kommen nur die Nutzdaten. Gemessen geht die Rechnung auf: 56 + 2 + 2 × (17 + 8192) = **16 476** Byte Blöcke. **Zwei Fallstricke, beide gemessen:** beide Skripte brauchen `bc`, und diese Umgebung hat keines (`command -v bc` leer) — das Werkzeug wird deshalb **NICHT gepatcht**, `gen_fds_corpus.py` legt einen `bc`-Ersatz an, der nur `obase=16;N` und `ibase=16;X` kennt und alles andere mit rc 2 abweist, und berührt damit ausschließlich die **Zahlenbasis**, nie die FDS-Struktur; und `fdtc` **polstert nicht** auf die Seitengröße, weshalb seine 16 476-Byte-Datei sowohl von `fdstool` als auch von UFT abgewiesen wird. **Was es NICHT entscheidet:** mehrseitige Disketten (das Rezept baut eine Seite) und die QD-Spielart mit Prüfsummen |
| `fdstool` | `FDSTOOL` | **KEINE.** GitHub-API meldet `license: KEINE`, das README hat **10 Byte**, und im Paket liegt keine Lizenzdatei — gemessen, nicht vermutet; ohne Rechteeinräumung gilt der Vorbehalt aller Rechte (`P3-356`). **Eigentümerentscheidung vom 2026-09-17**, wörtlich: *„fds auch, ja machen"*. Gehandhabt wie `dtc`, `epstool` und `hdm_to_fdi`: **ausführen ja, weitergeben nein** — nichts aus dem Paket wandert in den Baum. Der **Behälter** des Belegs kommt bewusst vom BSD-3-lizenzierten `fdtc`; `fdstool` setzt nur den 16-Byte-Kopf und liest | es nennt seine Version selbst im Kopf der Hilfe („fdstool 0.2 beta"), aber nur bei `-h` und **nicht maschinenlesbar**. Anker deshalb dreiteilig: Quellstand (Tarball 6104 Byte, eine C-Datei) + Baurezept (`gcc -O2 -Wall`, rc 0, **zwei Warnungen im FREMDEN Code** — `-Wdangling-else` und `-Wmaybe-uninitialized`) + Ausgabe-SHA (`d58dc05a094eacd2…` für eine benannte Eingabe) | **Die unabhängige zweite Hand neben `fdtc` (MF-1226) und der Setzer des 16-Byte-fwNES-Kopfes.** `-a -o` stellt `FDS\x1a` + Seitenzahl + 11 Nullbytes voran; **ohne** Ausgabedatei zerlegt es das Abbild und nennt jede Strukturzahl — gemessen `Found FDS header with 1 side`, `File amount: 2`, `File address: $8000`, `File size: 8192 bytes`. Es kennt außerdem die **QD-Seite mit Prüfsummen** (`-c` rechnet QD-CRCs neu, `-z` nullt die des Disk-Info-Blocks); ein FDS→QD-Umweg wäre der nächste, stärkere Beleg und ist **NICHT** gemacht. **BEFUND AM ORAKEL, am Quelltext gemessen:** es meldet jeden **Dateinamen nur dreistellig** — `fdstool.c:641` läuft `for (x = 0; x < 3; x++)` über das Namensfeld, das **8 Byte** hat (Versatz 3..10); die Schranke 3 ist die des DISKETTEN-Namens. Aus `UFTK0` wird `UFT`. Alle Strukturzahlen stimmen, die Namensanzeige nicht — ein Orakel ist eine **Referenz, kein Beweis** (MF-1015), und `test_fds_gegen_fdtc` prüft die Namen deshalb **selbst**. **Zwei weitere Fallstricke:** `-a` druckt „ERROR: cannot correct crcs for fds infile or outfile" und endet trotzdem mit **rc 0** und geschriebener Datei — eine Fehlermeldung ohne Fehler; und es **weist** eine Datei ab, die nicht 65 500 Byte je Seite hat („not in qd/fds format"), wobei `fdstool.c:19` `FDS_LENGTH 65500` definiert — damit ist UFTs Seitengröße von fremder Hand bestätigt. **Was es NICHT entscheidet:** die 476-Byte-Grenze des letzten virtuellen Sektors, denn die 128-mal-512-Aufteilung ist eine **Hausregel** von UFT und keine Formateigenschaft (MF-1038) |
...
| GeoCopy (Christian Meilinger, C64/C128) | **Kein Oracle, sondern eine *Spec*-Quelle — und die stärkste Sorte davon: der SCHREIBER des Schutzes (MF-1333).** `neue-ideen/geocopy.zip`, 48 118 Byte, 24 Einträge, sha256 `00b396f7…f764ee6`, nicht versioniert (`.gitignore:90`). **Lizenz an der DATEI gemessen**, und zwar in `geocopy/READ.ME.cvt` — nicht in einer Datei namens LICENSE, die es im Paket nicht gibt; beide Textdateien sind GEOS-CVT-Behälter, die druckbaren Zeichen holt `unzip -p … \| tr -c '[:print:]\n' '\n'`. Wörtlich: *„Diese Diskette darf und soll von allen Usern an andere GEOS-Besitzer vollständig weitergegeben werden"* **und** *„Diese Programme sind somit Public Domain und dürfen nicht kommerziell ohne Einwilligung des Autors vertrieben werden!"* — eine Gewährung und eine Einschränkung. Trotz des Wortes „Public Domain" ist die NC-Klausel mit diesem GPL-2-Baum **unvereinbar**; ein Port ist damit ausgeschlossen, das Lesen nicht. Ein Oracle wäre es ohnehin nicht: es sind fünf 6502-Programme für C64/C128, die hier weder gebaut noch ausgeführt werden. **Was es entscheidet** — und keine andere Quelle im Baum sagt es: die Schutzspur ist *„auf allen GEOS-Boot-Disketten die Nummer 21 (dezimal)"*; der Lückeninhalt ist im Original `… $55 $55 $67 $55 $55 $67 SYNC …`, bei Standardformatierung `… $55 $55 $55 …`, bei GeoCopy-Rekonstruktion durchgehend `$67`; und der Satz, der die Rekonstruktion überhaupt erlaubt: *„GEOS prüft nur auf Vorhandensein von $55 oder $67 und nicht beide Werte an den richtigen Stellen"*. Dazu: Spur 21 ist *„in der BAM geschützt, aber nicht mit einem Directory-Eintrag verbunden"*, Kernelblöcke werden vorher ausgelagert, und **alle fünf Programme können nur einseitig** — der Autor bittet ausdrücklich darum, sie für 1571 zu erweitern, was die D71-Sperre belegt statt sie zu vermuten. **Übernommen sind Zahlen mit Fundstelle, keine Zeilen**; der 6502-Quelltext auf der Diskette wird nicht gelesen. Zitiert in `src/protection/uft_geos_protection.c`; A-032-Urteil in `docs/neue_ideen_urteile.json` (stand bis MF-1333 auf „ungemessen", mit dem zutreffenden Hinweis *„die Datei ist ein GEOS-Behälter, das Readme steckt darin"*) |
| WHDLoad Development Package / RawDIC-Imager-Beispiele (Bert Jahn) | **Kein Oracle, sondern der Kanal *Nachbau* — uebernommen sind Zahlen, keine Zeilen.** `neue-ideen/fertige/2/WHDLoad_dev/`, die acht Vorlagen unter `Src/imager-examples/`. **Lizenz an den DATEIEN gemessen:** die Paketlizenz `Docs/en/copy.html` sagt woertlich *"Copyright 1995-2025 by Bert Jahn. All rights reserved. Commercial use is only allowed with an explicit, written permission by the author. It is strictly prohibited to modify any part of the WHDLoad package, except the source files which may be modified for reuse."* — Disassemblierung wird **nicht** untersagt (anders als bei `x50conv`), der Weg ist also offen; "non-commercial only" ist mit diesem GPL-2-Baum aber **unvereinbar**, ein Port damit ausgeschlossen. Je Vorlage gemessen: fuenf tragen selbst `:Copyright. Public Domain` (arkanoid, ik+, Lotus2, robocop2, worldsoflegend), `Cavitas.islave.s` und `Gremlin.islave.s` tragen **keine** Lizenzzeile, `WoodysWorld.islave.s:2` ein **fremdes** `(c) 1993 Vision`. Ein Oracle waere es ohnehin nicht: es sind 68000-Quelltexte fuer den Amiga, hier weder gebaut noch ausgefuehrt. **Was es entscheidet:** die Spurbereiche, Syncworte und Laengen der acht Profile in `src/core/uft_amiga_recipe_profiles.c` — und die einzige davon nachgemessene Zahl, die Seitenreihenfolge: `Gremlin.islave.s:53`/`:64` und `Lotus2.islave.asm:66` setzen `dc.w DFLG_SWAPSIDES` ausdruecklich. **Was es NICHT entscheidet:** was die Flagge genau bedeutet — `Src/resource/RawDICDiskFlags.s:11` listet nur ihren Namen, eine Beschreibung liegt dem Paket nicht bei. Die Abbildung auf `UFT_RECIPE_SIDE_SWAPPED` folgt dem Namen, und mehr wird nicht behauptet. **Belegstand aller acht Profile: `verified_with_real_media = false`** — keine echte Aufnahme, kein Abgleich gegen ein fremdes Werkzeug. Eigentuemer-Entscheidung 2026-09-21; A-032-Urteil in `docs/neue_ideen_urteile.json` |
