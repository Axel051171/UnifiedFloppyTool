...
#ifdef UFT_HAVE_TESSERACT
    /* Tesseract API: TessBaseAPICreate() + TessBaseAPIInit3() */
    TessBaseAPI *api = TessBaseAPICreate();
    if (api) {
        const char *lang = config->language[0] ? config->language : "eng";
        if (TessBaseAPIInit3(api, NULL, lang) != 0) {
            TessBaseAPIDelete(api);
            api = NULL;
        }
    }
    ctx->engine_handle = api;
#else
    /* No Tesseract: engine_handle stays NULL, OCR returns placeholder results */
...
#ifdef UFT_HAVE_TESSERACT
        TessBaseAPIEnd((TessBaseAPI *)ctx->engine_handle);
        TessBaseAPIDelete((TessBaseAPI *)ctx->engine_handle);
#endif
        ctx->engine_handle = NULL;
    }
...
int uft_ocr_load_image(const char *path, uft_image_t *image)
{
    ...
    /* Parse image header for dimensions and DPI */
    image->dpi = 300;  /* Default */
    
    if (image->size >= 24 && image->data) {
        const uint8_t *d = image->data;
        
        /* PNG: 89 50 4E 47, dimensions at offset 16 (IHDR chunk) */
        if (d[0] == 0x89 && d[1] == 'P' && d[2] == 'N' && d[3] == 'G') {
            image->width = (d[16] << 24) | (d[17] << 16) | (d[18] << 8) | d[19];
            image->height = (d[20] << 24) | (d[21] << 16) | (d[22] << 8) | d[23];
            image->format = UFT_IMG_PNG;
        }
        /* JPEG: FF D8 FF */
        else if (d[0] == 0xFF && d[1] == 0xD8 && d[2] == 0xFF) {
            image->format = UFT_IMG_JPEG;
            /* Scan for SOF0/SOF2 marker to get dimensions */
            for (size_t i = 2; i + 9 < image->size; ) {
                if (d[i] != 0xFF) { i++; continue; }
                uint8_t marker = d[i+1];
                if (marker == 0xC0 || marker == 0xC2) {
                    image->height = (d[i+5] << 8) | d[i+6];
                    image->width = (d[i+7] << 8) | d[i+8];
                    break;
                }
                uint16_t seg_len = (d[i+2] << 8) | d[i+3];
                i += 2 + seg_len;
            }
        }
        /* TIFF: 49 49 2A 00 (LE) or 4D 4D 00 2A (BE) */
        else if ((d[0] == 'I' && d[1] == 'I') || (d[0] == 'M' && d[1] == 'M')) {
            image->format = UFT_IMG_TIFF;
        }
        /* BMP: 42 4D */
        else if (d[0] == 'B' && d[1] == 'M' && image->size >= 26) {
            image->width = (uint32_t)(d[18] | (d[19] << 8) | (d[20] << 16) | (d[21] << 24));
            image->height = (uint32_t)(d[22] | (d[23] << 8) | (d[24] << 16) | (d[25] << 24));
            image->format = UFT_IMG_BMP;
        }
    }
...
int uft_ocr_preprocess(uft_image_t *image, uint32_t flags)
{
    ...
    /* Basic image preprocessing for OCR quality improvement.
     * Works on raw pixel data (UFT_IMG_RAW format with known width/height). */
    if (image->format == UFT_IMG_RAW && image->width > 0 && image->height > 0) {
        size_t pixels = (size_t)image->width * image->height * image->channels;
        
        if (flags & UFT_PREPROC_CONTRAST) {
            /* Convert RGB to grayscale if needed */
            if (image->channels == 3 && pixels <= image->size) {
                size_t gray_size = (size_t)image->width * image->height;
                uint8_t *gray = malloc(gray_size);
                if (gray) {
                    for (size_t i = 0; i < gray_size; i++) {
                        size_t off = i * 3;
                        gray[i] = (uint8_t)((image->data[off] * 77 +
                                              image->data[off+1] * 150 +
                                              image->data[off+2] * 29) >> 8);
                    }
                    free(image->data);
                    image->data = gray;
                    image->size = gray_size;
                    image->channels = 1;
                }
            }
        }
        
        if (flags & UFT_PREPROC_BINARIZE) {
            /* Otsu-style threshold: compute histogram then find optimal threshold */
            if (image->channels == 1) {
                uint32_t hist[256] = {0};
                size_t npix = (size_t)image->width * image->height;
                for (size_t i = 0; i < npix; i++) hist[image->data[i]]++;
                
                /* Find threshold via maximum between-class variance */
                uint32_t total = (uint32_t)npix;
                uint64_t sum = 0;
                ...
                for (int t = 0; t < 256; t++) {
                    w_bg += hist[t];
                    if (w_bg == 0) continue;
                    uint32_t w_fg = total - w_bg;
                    if (w_fg == 0) break;
                    
                    sum_bg += (uint64_t)t * hist[t];
                    double mean_bg = (double)sum_bg / w_bg;
                    double mean_fg = (double)(sum - sum_bg) / w_fg;
...
float uft_ocr_detect_skew(const uft_image_t *image)
{
    ...
    /* Skew detection using horizontal projection profile analysis.
     * Computes text line angle by testing rotation angles -5° to +5°
     * and finding the angle with maximum horizontal variance. */
    if (!image || !image->data || image->width == 0 || image->height == 0) return 0.0f;
    if (image->channels != 1) return 0.0f;  /* Needs grayscale */
    ...
    for (int a = -10; a <= 10; a++) {
        ...
        uint32_t *proj = calloc(h, sizeof(uint32_t));
        if (!proj) continue;
        
        for (uint32_t y = 0; y < image->height; y++) {
            for (uint32_t x = 0; x < image->width; x++) {
                /* Rotated y coordinate */
                int ry = (int)(y * cos_a - x * sin_a + image->height / 2.0f);
                if (ry >= 0 && (uint32_t)ry < h) {
...
int uft_ocr_rotate(uft_image_t *image, float angle)
{
    ...
    /* Rotate image by given angle (degrees) using nearest-neighbor interpolation.
     * Works on raw grayscale (channels=1) pixel data. */
    if (image->format != UFT_IMG_RAW || image->channels != 1 ||
    ...
    for (uint32_t y = 0; y < h; y++) {
        for (uint32_t x = 0; x < w; x++) {
            ...
            int sx = (int)(dx * cos_a + dy * sin_a + cx + 0.5f);
            int sy = (int)(-dx * sin_a + dy * cos_a + cy + 0.5f);
            
            if (sx >= 0 && (uint32_t)sx < w && sy >= 0 && (uint32_t)sy < h) {
                rotated[y * w + x] = image->data[sy * w + sx];
            } else {
                rotated[y * w + x] = 255;  /* White background */
            }
        }
    }
...
static bool contains_ci(const char *text, const char *search)
{
    ...
    for (size_t i = 0; i + search_len <= text_len; i++) {
        bool match = true;
...
{
    /* Basic OCR spell correction: fix common OCR misreads.
     * Pattern-based substitution for frequent OCR errors:
     *   0 ↔ O, 1 ↔ l/I, 5 ↔ S, 8 ↔ B, rn ↔ m, etc.
     * A real implementation would use hunspell/aspell dictionaries. */
    ...
    /* Fix common OCR substitution pairs in word context */
    for (size_t i = 0; i < len; i++) {
        /* "rn" → "m" (very common OCR error) */
        if (i + 1 < len && text[i] == 'r' && text[i+1] == 'n') {
            /* Only if surrounded by lowercase letters */
            bool prev_lower = (i > 0 && text[i-1] >= 'a' && text[i-1] <= 'z');
            bool next_lower = (i + 2 < len && text[i+2] >= 'a' && text[i+2] <= 'z');
        ...
        if (text[i] == 'l') {
            bool prev_digit = (i > 0 && text[i-1] >= '0' && text[i-1] <= '9');
            bool next_digit = (i + 1 < len && text[i+1] >= '0' && text[i+1] <= '9');
...
int uft_ocr_export_pdf(const uft_image_t *image, const uft_ocr_result_t *result,
                       const char *path)
{
    /* Generate a searchable PDF with OCR text overlay.
     * Creates a minimal PDF 1.4 document with the image as background
     * and invisible text layer from OCR results for search/copy. */
    if (!result || !path) return -1;
     ...
     * Header → Catalog → Pages → Page → Contents (text) → Font */
    ...
    /* Page dimensions (default A4 or from image) */
    int page_w = 612, page_h = 792;  /* US Letter in points */
    if (image && image->width > 0 && image->height > 0 && image->dpi > 0) {
        page_w = (int)(image->width * 72 / image->dpi);
        page_h = (int)(image->height * 72 / image->dpi);
    }
    ...
    /* Object 4: Content stream with OCR text */
    offsets[obj++] = ftell(f);
    
    /* Build content stream */
    char stream[8192];
    int slen = 0;
    slen += snprintf(stream + slen, sizeof(stream) - slen,
                     "BT\n/F1 1 Tf\n3 Tr\n");  /* 3 Tr = invisible text */
    
    /* Add each OCR word at its position */
    for (int i = 0; i < result->word_count && slen < 7800; i++) {
        const uft_ocr_word_t *w = &result->words[i];
        if (w->text[0] == '\0') continue;
        
        /* Convert pixel coords to PDF points */
        int px = 0, py = page_h;
        if (image && image->dpi > 0) {
            px = (int)(w->x * 72 / image->dpi);
            py = page_h - (int)(w->y * 72 / image->dpi);
        }
    ...
    slen += snprintf(stream + slen, sizeof(stream) - slen, "ET\n");
