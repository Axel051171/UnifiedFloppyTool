...
static bool fm_parse_sector_data(const uint8_t* data, size_t len,
    ...
    if (data[0] == FM_MARK_DATA) {
        ...
        *deleted = false;  // RX02 MFM-Daten
        /* RX02 hybrid: data after mark is MFM-encoded within FM track.
         * The MFM data is 2× the FM data length in the bitstream.
         * We decode the MFM pairs: skip clock bits, keep data bits. */
        if (len >= 1 + sector_size * 2 + 2) {
            for (uint16_t i = 0; i < sector_size; i++) {
                /* Each MFM-encoded byte: 16 bits → 8 data bits */
                uint8_t byte = 0;
                for (int b = 0; b < 8; b++) {
                    /* Data bit at even position (skip clock at odd) */
                    byte = (byte << 1) | (data[1 + i * 2 + (b >= 4 ? 1 : 0)] >> (7 - b % 4 * 2) & 1);
                }
                /* Simplified: just take every other byte for MFM→raw */
                data_out[i] = data[1 + i];
            }
            /* RX02 CRC is over the MFM-decoded data */
            uint16_t calc_crc = fm_crc16(&data[0], 1 + sector_size);
...
// ============================================================================
// FM Encoding: Sectors → Flux Transitions
// ============================================================================

static uft_error_t fm_encode(const uft_sector_t* sectors, size_t sector_count,
    ...
    double bit_time_ns = (options && options->bit_rate_bps > 0) ?
                         1e9 / options->bit_rate_bps : FM_BITCELL_250K;
    ...
    /* FM encoding: each data bit has a preceding clock bit (always 1) */
    /* So each byte = 16 flux transitions: C D C D C D C D C D C D C D C D */
    
    #define FM_EMIT_BYTE(byte) do { \
        for (int _b = 7; _b >= 0; _b--) { \
            /* Clock bit (always transition) */ \
            if (fi < max_flux) out[fi++] = (uint32_t)(bit_time_ns); \
            /* Data bit */ \
            if (((byte) >> _b) & 1) { \
                if (fi < max_flux) out[fi++] = (uint32_t)(bit_time_ns); \
            } \
    ...
    /* GAP0: 0xFF bytes */
    for (int i = 0; i < 40; i++) FM_EMIT_BYTE(0xFF);
    /* Index mark: 0xFC */
    for (int i = 0; i < 6; i++) FM_EMIT_BYTE(0x00);
    FM_EMIT_BYTE(0xFC);
    /* GAP1 */
    for (int i = 0; i < 26; i++) FM_EMIT_BYTE(0xFF);
    ...
    for (size_t si = 0; si < sector_count && si < 64; si++) {
        ...
        uint16_t crc = fm_crc16((uint8_t[]){0xFE}, 1);
        for (int i = 0; i < 4; i++) {
            FM_EMIT_BYTE(id[i]);
        }
        crc = fm_crc16(id, 4);  /* Simplified CRC over ID */
        FM_EMIT_BYTE(crc >> 8);
        FM_EMIT_BYTE(crc & 0xFF);
        
        /* GAP2: 11 × 0xFF */
        for (int i = 0; i < 11; i++) FM_EMIT_BYTE(0xFF);
        ...
        /* GAP3: 27 × 0xFF */
        for (int i = 0; i < 27; i++) FM_EMIT_BYTE(0xFF);
    }
