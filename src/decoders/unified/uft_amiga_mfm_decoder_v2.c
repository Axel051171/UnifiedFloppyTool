...
static uint32_t read_mfm_long(const uint8_t* bits, size_t bit_count, size_t* pos) {
    uint32_t val = 0;
    for (int i = 0; i < 32 && *pos < bit_count; i++) {
...
static uft_error_t amiga_mfm_decode_track(const uft_flux_track_data_t* flux,
    ...
    while (bit_pos >= 0 && sectors->sector_count < (size_t)(max_sectors + 2)) {
        ...
        // Read header (4 longs: info odd, info even, label, hdr checksum)
        ...
        // Read 512 bytes data (256 odd longs, 256 even longs)
        uint8_t data[512];
        uint32_t odd_data[128], even_data[128];
        
        for (int i = 0; i < 128; i++) odd_data[i] = read_mfm_long(bits, bit_count, &pos);
        for (int i = 0; i < 128; i++) even_data[i] = read_mfm_long(bits, bit_count, &pos);
        ...
        s->id.crc_ok = true;  // TODO: proper check
        
        s->data = malloc(512);
        if (s->data) { memcpy(s->data, data, 512); s->data_size = 512; }
