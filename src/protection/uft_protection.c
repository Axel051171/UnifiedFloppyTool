...
bool uft_detect_copylock(uft_protection_ctx_t* ctx) {
    ...
    if (ctx->copylock.num_sectors >= 6) {
        ...
        /* Try to recover LFSR seed from CopyLock track data.
         * CopyLock uses an LFSR (Linear Feedback Shift Register) keyed
         * by a 32-bit seed. The seed is derived from the sync mark
         * positions and first data bytes of the protected sectors.
         * Rob Northen Computing embeds the seed as XOR of the first
         * 4 bytes after the initial sync mark on the key track. */
        uint32_t seed = 0;
        size_t seed_offset;
        if (find_sync_mark(ctx->track_data, ctx->track_size,
                           ctx->copylock.sync_marks[0], &seed_offset)) {
            /* Skip past sync mark (2 bytes) + address mark (4 bytes) */
            size_t data_start = seed_offset + 6;
            if (data_start + 4 <= ctx->track_size) {
                seed = ((uint32_t)ctx->track_data[data_start]     << 24) |
                       ((uint32_t)ctx->track_data[data_start + 1] << 16) |
                       ((uint32_t)ctx->track_data[data_start + 2] <<  8) |
                       ((uint32_t)ctx->track_data[data_start + 3]);
            }
        }
        ctx->copylock.seed = seed;
    }
...
bool uft_detect_weakbits(uft_protection_ctx_t* ctx) {
    ...
    for (size_t i = 0; i + 1 < ctx->track_size; i++) {
...
bool uft_detect_custom_sync(uft_protection_ctx_t* ctx) {
    ...
    for (size_t i = 0; i + 2 < ctx->track_size; i++) {
        ...
        /* Count occurrences */
        int count = 0;
        for (size_t j = i; j + 1 < ctx->track_size; j++) {
