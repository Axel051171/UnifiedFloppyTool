...
int uft_protect_detect_c64(const uint8_t *track_data,
                            ...
                            uint8_t head,
                            uft_protection_analysis_t *result) {
    /* Integrate with existing C64 protection suite.
     * Delegates to the specialized C64 detectors for:
     * V-MAX!, RapidLok, Vorpal, fat tracks, timing-based protections.
     */
    if (!track_data || !result) return 0;
    (void)head;
    
    size_t data_len = (track_bits + 7) / 8;
    int detections = 0;
    
    /* V-MAX! detection (GCR-based, sector layout analysis) */
    uft_vmax_result_t vmax_result;
    memset(&vmax_result, 0, sizeof(vmax_result));
    if (uft_c64_detect_vmax(track_data, data_len, track, NULL, &vmax_result) > 0) {
        if (vmax_result.detected && result->detection_count < UFT_PROTECT_MAX_DETECTIONS) {
            uft_protection_detection_t *det = &result->detections[result->detection_count];
            memset(det, 0, sizeof(*det));
            det->type = UFT_PROT_VMAX_V1;
            det->category = UFT_PCAT_GCR_TIMING;
            det->platform = UFT_PLATFORM_C64;
            det->confidence = UFT_PCONF_PROBABLE;
            det->confidence_pct = 90;
            det->track = track;
            snprintf(det->name, sizeof(det->name), "V-MAX!");
            snprintf(det->detail, sizeof(det->detail),
                     "V-MAX! protection detected on track %u", track);
            result->detection_count++;
    ...
    /* RapidLok detection */
    uft_rapidlok_result_t rl_result;
    memset(&rl_result, 0, sizeof(rl_result));
    if (uft_c64_detect_rapidlok(track_data, data_len, track, &rl_result) > 0) {
        if (rl_result.detected && result->detection_count < UFT_PROTECT_MAX_DETECTIONS) {
            uft_protection_detection_t *det = &result->detections[result->detection_count];
            memset(det, 0, sizeof(*det));
            det->type = UFT_PROT_RAPIDLOK_V1;
            det->category = UFT_PCAT_GCR_TIMING;
            det->platform = UFT_PLATFORM_C64;
            det->confidence = UFT_PCONF_PROBABLE;
            det->confidence_pct = 92;
            det->track = track;
            snprintf(det->name, sizeof(det->name), "RapidLok");
            result->detection_count++;
            result->is_protected = true;
            detections++;
        }
    ...
    /* Vorpal detection */
    uft_vorpal_result_t vorpal_result;
    memset(&vorpal_result, 0, sizeof(vorpal_result));
    if (uft_c64_detect_vorpal(track_data, data_len, track, &vorpal_result) > 0) {
        if (vorpal_result.detected && result->detection_count < UFT_PROTECT_MAX_DETECTIONS) {
            uft_protection_detection_t *det = &result->detections[result->detection_count];
            memset(det, 0, sizeof(*det));
            det->type = UFT_PROT_UNKNOWN;
            det->category = UFT_PCAT_CUSTOM_FORMAT;
            det->platform = UFT_PLATFORM_C64;
            det->confidence = UFT_PCONF_LIKELY;
            det->confidence_pct = 88;
            det->track = track;
            snprintf(det->name, sizeof(det->name), "Vorpal");
            result->detection_count++;
            result->is_protected = true;
            detections++;
        }
    ...
    /* Timing-based protection (requires flux intervals) */
    if (timing_data) {
        size_t interval_count = track_bits > 0 ? track_bits : data_len * 8;
        if (interval_count > 20000) interval_count = 20000;
        uint32_t *flux_intervals = (uint32_t *)malloc(interval_count * sizeof(uint32_t));
        if (flux_intervals) {
            for (size_t i = 0; i < interval_count; i++) {
                flux_intervals[i] = timing_data[i];
            }
            float confidence = 0;
            char prot_name[64] = {0};
            if (uft_c64_detect_timing_protection(flux_intervals, interval_count,
                                                  track, &confidence,
                                                  prot_name, sizeof(prot_name)) > 0) {
                if (result->detection_count < UFT_PROTECT_MAX_DETECTIONS) {
                    uft_protection_detection_t *det = &result->detections[result->detection_count];
                    memset(det, 0, sizeof(*det));
                    det->type = UFT_PROT_UNKNOWN;
                    det->category = UFT_PCAT_TIMING_SENSITIVE;
                    det->platform = UFT_PLATFORM_C64;
                    det->confidence_pct = (uint8_t)(confidence * 100);
                    det->confidence = confidence > 0.75 ? UFT_PCONF_PROBABLE : UFT_PCONF_LIKELY;
                    det->track = track;
                    snprintf(det->name, sizeof(det->name), "%s", prot_name);
                    result->detection_count++;
                    result->is_protected = true;
                    detections++;
                }
...
int uft_protect_detect_apple2(const uint8_t *track_data,
                               ...
                               uint8_t head,
                               uft_protection_analysis_t *result) {
    /* Integrate with existing Apple II protection suite.
     * Delegates to specialized detectors for:
     * Nibble count, timing bits, spiral protection, custom address marks.
     */
    if (!track_data || !result) return 0;
    (void)head;
    ...
    /* Nibble count protection: non-standard number of nibbles per track */
    uft_nibble_count_t nibble_result;
    memset(&nibble_result, 0, sizeof(nibble_result));
    if (uft_apple2_detect_nibble_count(track_data, data_len, track,
                                        &nibble_result) > 0) {
        if (nibble_result.is_protected && 
            result->detection_count < UFT_PROTECT_MAX_DETECTIONS) {
            uft_protection_detection_t *det = &result->detections[result->detection_count];
            memset(det, 0, sizeof(*det));
            det->type = UFT_PROT_UNKNOWN;
            det->category = UFT_PCAT_CUSTOM_FORMAT;
            det->platform = UFT_PLATFORM_APPLE2;
            det->confidence_pct = (uint8_t)(nibble_result.confidence * 100);
            det->confidence = nibble_result.confidence > 0.75 ? UFT_PCONF_PROBABLE : UFT_PCONF_LIKELY;
            det->track = track;
            snprintf(det->name, sizeof(det->name), "Nibble Count");
            snprintf(det->detail, sizeof(det->detail),
                     "Track %u: expected %u nibbles, found %u (diff %+d)",
                     track, nibble_result.expected_nibbles,
                     nibble_result.actual_nibbles, nibble_result.difference);
            result->detection_count++;
            result->is_protected = true;
            detections++;
        }
    ...
    /* Custom address/data marks detection */
    uft_custom_mark_t marks[16];
    memset(marks, 0, sizeof(marks));
    size_t mark_count = uft_apple2_detect_custom_marks(
        track_data, data_len, track, marks, 16);
    if (mark_count > 0 && result->detection_count < UFT_PROTECT_MAX_DETECTIONS) {
        uft_protection_detection_t *det = &result->detections[result->detection_count];
        memset(det, 0, sizeof(*det));
        det->type = UFT_PROT_UNKNOWN;
        det->category = UFT_PCAT_CUSTOM_SYNC;
        det->platform = UFT_PLATFORM_APPLE2;
        det->confidence = UFT_PCONF_LIKELY;
        det->confidence_pct = 85;
        det->track = track;
        snprintf(det->name, sizeof(det->name), "Custom Marks");
        snprintf(det->detail, sizeof(det->detail),
                 "Track %u: %zu non-standard address/data marks found",
                 track, mark_count);
        result->detection_count++;
        result->is_protected = true;
        detections++;
...
int uft_protect_detect_atari_st(const uint8_t *track_data,
                                 ...
                                 uint8_t head,
                                 uft_protection_analysis_t *result) {
    /* Integrate with existing Atari ST protection suite.
     * Delegates to uft_atarist_prot_detect() which covers:
     * CopyLock (Rob Northen), MacroDOS, Dec0de, Prosec, key disks.
     */
    if (!track_data || !result) return 0;
    (void)timing_data;
    (void)track;
    ...
    /* Run full Atari ST protection analysis */
    uft_atarist_prot_result_t st_result;
    memset(&st_result, 0, sizeof(st_result));
    if (uft_atarist_prot_detect(track_data, data_len, &st_result) > 0) {
        /* CopyLock detection */
        if (st_result.copylock.detected && 
            result->detection_count < UFT_PROTECT_MAX_DETECTIONS) {
            uft_protection_detection_t *det = &result->detections[result->detection_count];
            memset(det, 0, sizeof(*det));
            det->type = UFT_PROT_COPYLOCK;
            det->category = UFT_PCAT_LFSR_ENCODED;
            det->platform = UFT_PLATFORM_ATARI_ST;
            det->confidence = UFT_PCONF_CERTAIN;
            det->confidence_pct = 95;
            det->track = track;
            det->head = head;
            det->requires_timing = true;
            det->requires_flux = true;
            det->reconstructable = true;
            snprintf(det->name, sizeof(det->name), "CopyLock");
            snprintf(det->variant, sizeof(det->variant), "Rob Northen Computing");
            result->detection_count++;
            result->is_protected = true;
            detections++;
        }
        ...
        if (st_result.long_track_count > 0 &&
            result->detection_count < UFT_PROTECT_MAX_DETECTIONS) {
            uft_protection_detection_t *det = &result->detections[result->detection_count];
            memset(det, 0, sizeof(*det));
            det->type = UFT_PROT_LONGTRACK_PROTEC;
            ...
            det->confidence_pct = 88;
            det->track = track;
            snprintf(det->name, sizeof(det->name), "Long Track");
            snprintf(det->detail, sizeof(det->detail),
                     "%u long tracks detected", st_result.long_track_count);
            result->detection_count++;
            result->is_protected = true;
            detections++;
        }
        
        /* Fuzzy sector detection */
        if (st_result.fuzzy_sector_count > 0 &&
            result->detection_count < UFT_PROTECT_MAX_DETECTIONS) {
            uft_protection_detection_t *det = &result->detections[result->detection_count];
            memset(det, 0, sizeof(*det));
            det->type = UFT_PROT_UNKNOWN;
            det->category = UFT_PCAT_WEAK_BITS;
            det->platform = UFT_PLATFORM_ATARI_ST;
            det->confidence = UFT_PCONF_PROBABLE;
            det->confidence_pct = 90;
            det->track = track;
            det->requires_flux = true;
            snprintf(det->name, sizeof(det->name), "Fuzzy Sectors");
            snprintf(det->detail, sizeof(det->detail),
                     "%u fuzzy sectors detected", st_result.fuzzy_sector_count);
            result->detection_count++;
            result->is_protected = true;
            detections++;
        }
        ...
        /* Update summary */
        if (detections > 0 && result->summary[0] == '\0') {
            snprintf(result->summary, sizeof(result->summary), "%s",
                     st_result.description);
        }
    }
