...
static int import_adf(xdf_context_t *ctx, const uint8_t *data, size_t size) {
    ...
    /* Track data stored in context->tracks[] array.
     * Each track contains sectors_per_track × sector_size bytes. */
    int total_sectors = sectors_per_track * 80 * 2;  /* tracks × heads */
    (void)total_sectors;
    (void)sector_size;
...
static int import_d64(xdf_context_t *ctx, const uint8_t *data, size_t size) {
    ...
    /* Error bytes are appended after sector data (1 byte per sector).
     * Error codes: 0x01=OK, 0x02-0x0F=various read errors.
     * When present, store them for quality analysis. */
    int error_offset = has_errors ? (tracks <= 35 ? 174848 : 196608) : -1;
    (void)error_offset;
