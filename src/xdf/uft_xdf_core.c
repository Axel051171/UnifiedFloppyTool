...
int xdf_import(xdf_context_t *ctx, const char *path) {
    ...
    /* Validate file size against expected XDF image sizes */
    if (size < (long)sizeof(xdf_header_t)) {
        fclose(f);
        return -1;
    }
...
int xdf_export(xdf_context_t *ctx, const char *path) {
    ...
    /* Write header (CRC placeholder - will be patched below) */
    ...
    /* Compute CRC32 over all file content after header */
    {
        long file_end = ftell(f);
        long data_start = (long)sizeof(xdf_header_t);
        long data_size = file_end - data_start;
        if (data_size > 0 && fseek(f, data_start, SEEK_SET) == 0) {
            uint32_t crc = 0xFFFFFFFF;
            uint8_t buf[4096];
            ...
            /* Rewrite header with computed CRC */
            fseek(f, 0, SEEK_SET);
            fwrite(&ctx->header, 1, sizeof(xdf_header_t), f);
        }
    }
