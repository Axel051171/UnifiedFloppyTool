...
void uft_d64_close(d64_handle_t *h) {
    ...
    if (!h->readonly && h->fp && h->data) {
        if (fseek(h->fp, 0, SEEK_SET) != 0) { /* seek error */ }
        if (fwrite(h->data, 1, h->data_size, h->fp) != h->data_size) { /* I/O error */ }
        if (h->error_table) {
            size_t error_size = h->has_errors ? 
                ((h->tracks == 35) ? 683 : 768) : 0;
            if (error_size > 0) {
                if (fwrite(h->error_table, 1, error_size, h->fp) != error_size) { /* I/O error */ }
            }
        }
    }
...
int uft_g64_open(const char *path, bool readonly, g64_handle_t **handle) {
    ...
    for (size_t i = 0; i < num_tracks; i++) {
        ...
        if (fseek(fp, (long)h->track_offsets[i], SEEK_SET) != 0) { /* seek error */ }
        uint16_t size;
...
int uft_d64_to_g64(d64_handle_t *d64, const char *g64_path) {
    ...
    if (fwrite(offsets, sizeof(uint32_t), header.track_count, fp) != header.track_count) { /* I/O error */ }
    /* Write speed zone table (placeholder) */
    uint32_t *speeds = calloc(header.track_count, sizeof(uint32_t));
    if (fwrite(speeds, sizeof(uint32_t), header.track_count, fp) != header.track_count) { /* I/O error */ }
    /* Encode and write tracks */
    uint8_t sector_buf[256];
    uint8_t *gcr_track = (uint8_t*)malloc(8192); if (!gcr_track) return UFT_ERR_MEMORY;
    ...
    for (uint8_t track = 1; track <= d64->tracks; track++) {
        ...
        if (fseek(fp, (long)data_offset, SEEK_SET) != 0) { /* seek error */ }
        if (fwrite(&track_size, sizeof(uint16_t), 1, fp) != 1) { /* I/O error */ }
        if (fwrite(gcr_track, 1, gcr_offset, fp) != gcr_offset) { /* I/O error */ }
        data_offset += 2 + (uint32_t)gcr_offset;
    }
    
    /* Write updated offset tables */
    if (fseek(fp, sizeof(header), SEEK_SET) != 0) { /* seek error */ }
    if (fwrite(offsets, sizeof(uint32_t), header.track_count, fp) != header.track_count) { /* I/O error */ }
    if (fwrite(speeds, sizeof(uint32_t), header.track_count, fp) != header.track_count) { /* I/O error */ }
    free(offsets);
    free(speeds);
...
int uft_g64_to_d64(g64_handle_t *g64, const char *d64_path) {
    ...
    if (fwrite(d64_data, 1, d64_size, fp) != d64_size) { /* I/O error */ }
    free(d64_data);
    fclose(fp);
