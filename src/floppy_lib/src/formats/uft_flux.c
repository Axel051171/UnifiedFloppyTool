...
uft_flux_track_t* uft_flux_track_create(size_t capacity) {
    ...
    if (capacity > 0) {
        ...
        track->sample_capacity = capacity;  /* BUGFIX: Track capacity per-object */
    }
...
uft_flux_error_t uft_flux_track_add_sample(uft_flux_track_t *track,
    ...
    /* BUGFIX: Use per-track capacity instead of static variable!
     * The previous static variable caused buffer overflow when multiple
     * tracks were processed - Track A's capacity was shared with Track B. */
    
    /* Initialize capacity if needed */
    if (track->sample_capacity == 0) {
        track->sample_capacity = 4096;  /* Initial capacity */
        track->samples = calloc(track->sample_capacity, sizeof(uft_flux_sample_t));
        if (!track->samples) return UFT_FLUX_ERR_BUFFER_TOO_SMALL;
    }
    
    /* Grow array if needed */
    if (track->sample_count >= track->sample_capacity) {
        size_t new_cap = track->sample_capacity * FLUX_GROWTH_FACTOR;
