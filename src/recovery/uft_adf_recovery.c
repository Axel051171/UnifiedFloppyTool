...
int uft_adf_scan_deleted(const char *path, 
    ...
    fseek(f, root_block * ADF_BLOCK_SIZE, SEEK_SET);
    ...
    for (int i = 0; i < 25; i++) {
        ...
        fseek(f, bm_block * ADF_BLOCK_SIZE, SEEK_SET);
    ...
    for (int blk = 2; blk < total_blocks && found < max_entries; blk++) {
        ...
        fseek(f, blk * ADF_BLOCK_SIZE, SEEK_SET);
...
uft_recovery_status_t uft_adf_recover_file(const char *path,
    ...
    fseek(adf, entry->header_block * ADF_BLOCK_SIZE, SEEK_SET);
    ...
    for (int i = 0; i < 72 && bytes_left > 0; i++) {
        ...
        fseek(adf, data_blk * ADF_BLOCK_SIZE, SEEK_SET);
    ...
    /* TODO: Handle extension blocks for files > 72 blocks */
