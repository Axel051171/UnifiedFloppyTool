...
multiread_error_t multiread_track(multiread_ctx_t *ctx,
    ...
    /* Vote on track data using byte-level majority across passes */
    {
        /* Find longest pass for output buffer size */
        size_t max_len = 0;
        for (uint8_t i = 0; i < ctx->pass_count; i++) {
            if (ctx->passes[i].data_len > max_len)
                max_len = ctx->passes[i].data_len;
        }
        
        if (max_len > 0) {
            uint8_t *voted_data = (uint8_t *)calloc(max_len, 1);
            uint8_t *conf_map = (uint8_t *)calloc(max_len, 1);
            uint8_t *weak_map = (uint8_t *)calloc(max_len, 1);
            uint8_t avg_conf = 0;
            uint32_t weak_cnt = 0;
            
            if (voted_data && conf_map && weak_map) {
                vote_buffer(ctx->passes, ctx->pass_count,
                            voted_data, max_len, conf_map, weak_map,
                            &avg_conf, &weak_cnt);
                
                result->overall_confidence = avg_conf;
                
                /* Estimate sector count from track size.
                 * Standard MFM: ~6250 raw bytes/track → 9-18 sectors.
                 * Sector boundaries can be detected by scanning for sync marks
                 * in voted data, but for now estimate from track length. */
                size_t sector_size = 512;  /* Default for MFM HD */
                if (max_len < 4000) sector_size = 256;  /* DD/SD */
                result->sector_count = (uint8_t)(max_len / (sector_size + 64));
                if (result->sector_count > 32) result->sector_count = 32;
                
                /* Per-sector confidence: check 512-byte windows */
                for (uint8_t s = 0; s < result->sector_count; s++) {
                    size_t start = s * (sector_size + 64) + 64;  /* Skip header gap */
                    if (start + sector_size > max_len) break;
                    
                    uint32_t sec_conf = 0;
                    for (size_t b = 0; b < sector_size && start + b < max_len; b++) {
                        sec_conf += conf_map[start + b];
                    }
                    /* Track sector quality in overall confidence */
                    (void)sec_conf;  /* Would populate sector result array */
                }
            }
