...
bool uft_usb_get_port_name(uint16_t vid, uint16_t pid, char *port_name, size_t max_len)
{
...
#ifdef _WIN32
    /* Windows: Try COM3-COM10 - return first accessible port as candidate */
    for (int i = 3; i <= 10; i++) {
        char candidate[32];
        snprintf(candidate, sizeof(candidate), "\\\\.\\COM%d", i);
        
        /* Test if port exists by trying to open it briefly */
        HANDLE h = CreateFileA(candidate, GENERIC_READ | GENERIC_WRITE,
    ...
    for (int i = 0; candidates[i]; i++) {
        /* Port existence check would use stat() and VID/PID matching via sysfs */
...
/* libusbp open/close/transfer: link with -lusbp at build time */
