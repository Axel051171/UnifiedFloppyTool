...
static int serial_open(const char *port) {
    ...
    /* 8N1, no flow control */
    /* Reference: ControlCmd.Normal = 9600 for CDC ACM control path */
...
static int gw_recv_ack(gw_handle_t *gw, uint8_t *response, size_t max_len) {
    /* Greaseweazle response format: [CMD_ECHO, ACK_STATUS, ...response data...]
     * We need to read the 2-byte header first, then any remaining data. */
    uint8_t header[2];
    
#ifdef _WIN32
    DWORD read_count;
    if (!ReadFile(gw->serial, header, 2, &read_count, NULL) || read_count != 2) {
        snprintf(gw->last_error, sizeof(gw->last_error), "Read header failed");
        return -1;
    }
#else
    if (serial_read(gw->fd, header, 2) != 2) {
        snprintf(gw->last_error, sizeof(gw->last_error), "Read header failed");
        return -1;
    }
#endif
    
    /* header[0] = command echo, header[1] = ACK status */
    ...
    /* Read response data if caller wants it */
    if (response && max_len > 0) {
#ifdef _WIN32
        DWORD total_read = 0;
        /* Read available data with timeout */
        if (!ReadFile(gw->serial, response, (DWORD)max_len, &read_count, NULL)) {
            return -1;
        }
        /* read_count contains bytes actually read */
#else
        ssize_t n = serial_read(gw->fd, response, max_len);
        if (n < 0) {
            return -1;
        }
#endif
...
gw_handle_t* gw_open(const char *port_name) {
    ...
    dcb.BaudRate = 9600;  /* Reference: ControlCmd.Normal = 9600 */
    dcb.ByteSize = 8;
    dcb.Parity = NOPARITY;
    ...
    snprintf(gw->version_string, sizeof(gw->version_string),
            "Greaseweazle F%u v%u.%u",
            gw->info.hw_model, (unsigned)gw->info.major, (unsigned)gw->info.minor);
...
int gw_get_info(gw_handle_t *gw, gw_info_t *info) {
    ...
    /* GET_INFO uses a 1-byte subindex (reference: pack("3B", GetInfo, 3, Firmware)) */
...
int gw_motor(gw_handle_t *gw, bool on) {
    if (!gw) return -1;
    /* Reference: pack("4B", Motor, 4, unit, state) */
    uint8_t params[] = {0, on ? 1 : 0};  /* unit=0, state */
