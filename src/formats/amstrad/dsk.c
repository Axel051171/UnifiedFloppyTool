...
int uft_cpc_dsk_open(FloppyDevice *dev,const char*path){
    ...
    char sig[16]={0};
    if (fread(sig,1,16,fp) != 16) { /* I/O error */ }
...
int uft_cpc_dsk_close(FloppyDevice *dev){
...
int uft_cpc_dsk_read_sector(FloppyDevice *dev,uint32_t t,uint32_t h,uint32_t s,uint8_t *buf){
...
int uft_cpc_dsk_write_sector(FloppyDevice *dev,uint32_t t,uint32_t h,uint32_t s,const uint8_t *buf){
    if(!dev||!dev->internal_ctx||!buf) return UFT_EINVAL;
    DskCtx *ctx=dev->internal_ctx;
...
int uft_cpc_dsk_analyze_protection(FloppyDevice *dev){
