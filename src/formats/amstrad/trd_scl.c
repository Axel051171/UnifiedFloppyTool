...
int uft_cpc_trd_scl_open(FloppyDevice *dev,const char*path){
...
int uft_cpc_trd_scl_close(FloppyDevice *dev){
...
int uft_cpc_trd_scl_read_sector(FloppyDevice *dev,uint32_t t,uint32_t h,uint32_t s,uint8_t *buf){
...
int uft_cpc_trd_scl_write_sector(FloppyDevice *dev,uint32_t t,uint32_t h,uint32_t s,const uint8_t *buf){
    if(!dev||!dev->internal_ctx||!buf) return UFT_EINVAL;
    TrdCtx *ctx=dev->internal_ctx;
...
int uft_cpc_trd_scl_analyze_protection(FloppyDevice *dev){
    TrdCtx *ctx=dev->internal_ctx;
    if(ctx->type==IMG_SCL){
