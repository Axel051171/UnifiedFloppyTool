...
uft_error_t uft_validate_d64(const uint8_t* data, size_t size,
    ...
    /* P1-ARR-001: Bounds check before array access */
    if (bam_offset + 3 > size) {
        add_issue(result, 3, bam_offset, 18, 0, "bounds",
                  "BAM offset %zu + 3 exceeds size %zu", bam_offset, size);
        result->valid = false;
...
uft_error_t uft_validate_adf(const uint8_t* data, size_t size,
    ...
    /* P1-ARR-001: Bounds check for root block access */
    if (root_offset + 512 > size) {
        add_issue(result, 3, root_offset, -1, root_block, "bounds",
                  "Root block offset %zu exceeds file size %zu", root_offset, size);
        result->valid = false;
        return UFT_ERROR_BOUNDS;
    }
    
    // Type should be 2 (T_HEADER)
    uint32_t root_type = ((uint32_t)data[root_offset] << 24) | ((uint32_t)data[root_offset+1] << 16) |
    ...
    uint32_t sec_type = ((uint32_t)data[root_offset + 508] << 24) | 
                        ((uint32_t)data[root_offset + 509] << 16) |
...
uft_error_t uft_validate_scp(const uint8_t* data, size_t size,
    ...
    for (int t = start_track; t <= end_track && t < 168; t++) {
        ...
        uint32_t track_offset = data[th_offset] | (data[th_offset+1] << 8) |
                                ((uint32_t)data[th_offset+2] << 16) | ((uint32_t)data[th_offset+3] << 24);
        ...
        if (track_offset + 16 <= size) {
            if (memcmp(data + track_offset, "TRK", 3) == 0) {
                ...
                uint32_t track_len = data[track_offset + 8] | 
                                    ...
                                    ((uint32_t)data[track_offset + 10] << 16) | 
                                    ((uint32_t)data[track_offset + 11] << 24);
...
uft_error_t uft_validate_g64(const uint8_t* data, size_t size,
    ...
    for (int t = 0; t < num_tracks; t++) {
        ...
        uint32_t track_offset = data[entry_offset] | (data[entry_offset+1] << 8) |
                                ((uint32_t)data[entry_offset+2] << 16) | ((uint32_t)data[entry_offset+3] << 24);
...
/**
 * @brief Format-Encoding compatibility rules table
 */
static const uft_format_encoding_rule_t format_encoding_rules[] = {
    // Commodore formats → GCR only
    { UFT_FMT_D64,  { UFT_ENC_GCR_CBM, UFT_ENC_GCR_CBM_V, 0 }, "D64" },
    { UFT_FMT_G64,  { UFT_ENC_GCR_CBM, UFT_ENC_GCR_CBM_V, 0 }, "G64" },
    { UFT_FMT_D71,  { UFT_ENC_GCR_CBM, 0 }, "D71" },
    { UFT_FMT_D81,  { UFT_ENC_MFM, 0 }, "D81" },  // D81 uses MFM!
    
    // Amiga formats → Amiga MFM only
    { UFT_FMT_ADF,  { UFT_ENC_AMIGA_MFM, 0 }, "ADF" },
    { UFT_FMT_DMS,  { UFT_ENC_AMIGA_MFM, 0 }, "DMS" },
    { UFT_FMT_ADZ,  { UFT_ENC_AMIGA_MFM, 0 }, "ADZ" },
    
    // Apple formats → Apple GCR
    { UFT_FMT_NIB,  { UFT_ENC_GCR_APPLE_525, UFT_ENC_GCR_APPLE, 0 }, "NIB" },
    { UFT_FMT_WOZ,  { UFT_ENC_GCR_APPLE_525, UFT_ENC_GCR_APPLE, 0 }, "WOZ" },
    { UFT_FMT_DO,   { UFT_ENC_GCR_APPLE_525, 0 }, "DO" },
    { UFT_FMT_PO,   { UFT_ENC_GCR_APPLE_525, 0 }, "PO" },
    
    // PC/DOS formats → MFM only
    { UFT_FMT_IMG,  { UFT_ENC_MFM, UFT_ENC_MFM_DD, UFT_ENC_MFM_HD, 0 }, "IMG" },
    { UFT_FMT_IMA,  { UFT_ENC_MFM, UFT_ENC_MFM_DD, UFT_ENC_MFM_HD, 0 }, "IMA" },
    { UFT_FMT_DSK,  { UFT_ENC_MFM, UFT_ENC_MFM_DD, UFT_ENC_FM, 0 }, "DSK" },
    
    // Atari ST → MFM
    { UFT_FMT_ST,   { UFT_ENC_MFM, UFT_ENC_MFM_DD, 0 }, "ST" },
    { UFT_FMT_MSA,  { UFT_ENC_MFM, 0 }, "MSA" },
    { UFT_FMT_STX,  { UFT_ENC_MFM, 0 }, "STX" },
    
    // Flux formats → Any encoding
    { UFT_FMT_SCP,  { UFT_ENC_AUTO, UFT_ENC_MFM, UFT_ENC_GCR_CBM, UFT_ENC_GCR_APPLE, UFT_ENC_AMIGA_MFM, 0 }, "SCP" },
    { UFT_FMT_HFE,  { UFT_ENC_AUTO, UFT_ENC_MFM, UFT_ENC_GCR_CBM, UFT_ENC_GCR_APPLE, UFT_ENC_AMIGA_MFM, 0 }, "HFE" },
    { UFT_FMT_KF,   { UFT_ENC_AUTO, UFT_ENC_MFM, UFT_ENC_GCR_CBM, UFT_ENC_GCR_APPLE, UFT_ENC_AMIGA_MFM, 0 }, "KryoFlux" },
    { UFT_FMT_IPF,  { UFT_ENC_AUTO, UFT_ENC_MFM, UFT_ENC_GCR_CBM, UFT_ENC_AMIGA_MFM, 0 }, "IPF" },
    { UFT_FMT_A2R,  { UFT_ENC_GCR_APPLE_525, UFT_ENC_GCR_APPLE, 0 }, "A2R" },
    
    // Universal container
    { UFT_FMT_UFF,  { UFT_ENC_AUTO, 0 }, "UFF" },
    
    // Terminator
    { UFT_FMT_UNKNOWN, { 0 }, NULL }
};
...
bool uft_validate_format_encoding(
    ...
    // Auto encoding is always valid
    if (encoding == UFT_ENC_AUTO || encoding == UFT_ENC_UNKNOWN) {
        return true;
    }
    
    // Find format in rules
    for (int i = 0; format_encoding_rules[i].format_name != NULL; i++) {
        if (format_encoding_rules[i].format == format) {
            // Check if encoding is in valid list
            for (int j = 0; format_encoding_rules[i].valid_encodings[j] != 0; j++) {
                uft_encoding_t valid = format_encoding_rules[i].valid_encodings[j];
                if (valid == encoding || valid == UFT_ENC_AUTO) {
                    return true;
                }
            }
            ...
            if (error_msg && msg_size > 0) {
                ...
                switch (encoding) {
                    ...
                    case UFT_ENC_GCR_CBM:    enc_name = "GCR (CBM)"; break;
                    case UFT_ENC_GCR_APPLE:  
                    case UFT_ENC_GCR_APPLE_525: enc_name = "GCR (Apple)"; break;
                    case UFT_ENC_AMIGA_MFM:  enc_name = "Amiga MFM"; break;
                    case UFT_ENC_FM:         enc_name = "FM"; break;
                    default: break;
                }
                snprintf(error_msg, msg_size,
                    "Invalid encoding '%s' for format '%s'",
                    enc_name, format_encoding_rules[i].format_name);
            }
            return false;
        }
    }
    
    // Format not in rules = allow any encoding
    return true;
}

int uft_get_valid_encodings(uft_format_id_t format, uft_encoding_t* encodings) {
    if (!encodings) return 0;
    
    for (int i = 0; format_encoding_rules[i].format_name != NULL; i++) {
        if (format_encoding_rules[i].format == format) {
            int count = 0;
            for (int j = 0; format_encoding_rules[i].valid_encodings[j] != 0 && j < 8; j++) {
                encodings[count++] = format_encoding_rules[i].valid_encodings[j];
            }
            return count;
        }
    }
    
    // Unknown format - return AUTO
    encodings[0] = UFT_ENC_AUTO;
    return 1;
}
...
uft_encoding_t uft_get_default_encoding(uft_format_id_t format) {
    for (int i = 0; format_encoding_rules[i].format_name != NULL; i++) {
        if (format_encoding_rules[i].format == format) {
            return format_encoding_rules[i].valid_encodings[0];
        }
    }
    return UFT_ENC_AUTO;
}
