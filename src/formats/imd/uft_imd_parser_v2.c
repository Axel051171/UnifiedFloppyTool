...
static int imd_parse_tracks(imd_context_t *ctx) {
    fseek(ctx->fp, ctx->data_offset, SEEK_SET);
    ...
    while (ctx->track_count < IMD_MAX_TRACKS) {
        ...
        for (int s = 0; s < track->header.sectors; s++) {
            ...
            if (type == IMD_SEC_UNAVAIL) {
                ...
                /* Full sector data */
                fseek(ctx->fp, track->sector_size, SEEK_CUR);
            }
        }
...
int imd_read_sector(imd_context_t *ctx, uint8_t cyl, uint8_t head,
    ...
    /* Seek to track start */
    fseek(ctx->fp, track->file_offset + sizeof(imd_track_header_t), SEEK_SET);
    
    /* Skip sector map */
    fseek(ctx->fp, track->header.sectors, SEEK_CUR);
