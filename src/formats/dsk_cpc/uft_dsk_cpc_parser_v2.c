...
const char* dsk_st1_description(uint8_t st1) {
    ...
    if (st1 & DSK_ST1_MISSING_AM)   strncat(buf, "Missing AM, ", sizeof(buf) - strlen(buf) - 1);
    if (st1 & DSK_ST1_NOT_WRITABLE) strncat(buf, "Not Writable, ", sizeof(buf) - strlen(buf) - 1);
    if (st1 & DSK_ST1_NO_DATA)      strncat(buf, "No Data, ", sizeof(buf) - strlen(buf) - 1);
    if (st1 & DSK_ST1_OVERRUN)      strncat(buf, "Overrun, ", sizeof(buf) - strlen(buf) - 1);
    if (st1 & DSK_ST1_CRC_ERROR)    strncat(buf, "ID CRC Error, ", sizeof(buf) - strlen(buf) - 1);
    if (st1 & DSK_ST1_END_OF_CYL)   strncat(buf, "End of Cylinder, ", sizeof(buf) - strlen(buf) - 1);
...
const char* dsk_st2_description(uint8_t st2) {
    ...
    if (st2 & DSK_ST2_MISSING_DAM)  strncat(buf, "Missing DAM, ", sizeof(buf) - strlen(buf) - 1);
    if (st2 & DSK_ST2_BAD_CYLINDER) strncat(buf, "Bad Cylinder, ", sizeof(buf) - strlen(buf) - 1);
    if (st2 & DSK_ST2_SCAN_NOT_SAT) strncat(buf, "Scan Not Satisfied, ", sizeof(buf) - strlen(buf) - 1);
    if (st2 & DSK_ST2_SCAN_EQUAL)   strncat(buf, "Scan Equal, ", sizeof(buf) - strlen(buf) - 1);
    if (st2 & DSK_ST2_WRONG_CYL)    strncat(buf, "Wrong Cylinder, ", sizeof(buf) - strlen(buf) - 1);
    if (st2 & DSK_ST2_CRC_ERROR)    strncat(buf, "Data CRC Error, ", sizeof(buf) - strlen(buf) - 1);
    if (st2 & DSK_ST2_DELETED)      strncat(buf, "Deleted Mark, ", sizeof(buf) - strlen(buf) - 1);
