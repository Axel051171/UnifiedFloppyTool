...
int uft_msc_dhd_open(FloppyDevice *dev,const char*path){
...
int uft_msc_dhd_close(FloppyDevice *dev){
...
int uft_msc_dhd_read_sector(FloppyDevice *dev,uint32_t t,uint32_t h,uint32_t s,uint8_t *buf){
    (void)t; (void)h;
...
int uft_msc_dhd_write_sector(FloppyDevice *dev,uint32_t t,uint32_t h,uint32_t s,const uint8_t *buf){
    (void)t; (void)h;
    if(!dev||!dev->internal_ctx||!buf) return UFT_EINVAL;
    DHDCtx *ctx=dev->internal_ctx;
...
int uft_msc_dhd_analyze_protection(FloppyDevice *dev){
