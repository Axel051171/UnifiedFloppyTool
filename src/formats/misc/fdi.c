...
/* Flux metadata for copy-protection analysis */
typedef struct { uint32_t offset; uint32_t length; } WeakRegion;
typedef struct { uint32_t nominal_cell_ns; uint32_t jitter_ns; uint32_t encoding_hint; } FluxTiming;
typedef struct { FluxTiming timing; WeakRegion *weak_regions; uint32_t weak_count; } FluxMeta;
...
int uft_msc_fdi_open(FloppyDevice *dev, const char *path) {
...
int uft_msc_fdi_close(FloppyDevice *dev) {
...
int uft_msc_fdi_read_sector(FloppyDevice *dev, uint32_t t, uint32_t h, uint32_t s, uint8_t *buf) {
...
int uft_msc_fdi_write_sector(FloppyDevice *dev, uint32_t t, uint32_t h, uint32_t s, const uint8_t *buf) {
    if (!dev || !dev->internal_ctx || !buf) return UFT_EINVAL;
    FdiCtx *ctx = (FdiCtx*)dev->internal_ctx;
...
int uft_msc_fdi_analyze_protection(FloppyDevice *dev) {
    if (!dev || !dev->internal_ctx) return UFT_EINVAL;
    FdiCtx *ctx = (FdiCtx*)dev->internal_ctx;
    ...
    if (dev->sectorSize >= 16) {
        ...
        if (b) {
            if (uft_msc_fdi_read_sector(dev, 0, 0, 1, b) == UFT_OK) {
