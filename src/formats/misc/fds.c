...
int uft_msc_fds_close(FloppyDevice*dev){ if(!dev||!dev->internal_ctx) return EINVAL;
...
int uft_msc_fds_write_sector(FloppyDevice*dev,uint32_t t,uint32_t h,uint32_t s,const uint8_t*buf){ (void)dev;(void)t;(void)h;(void)s;(void)buf; return ENOTSUP; }
int uft_msc_fds_analyze_protection(FloppyDevice*dev){ logm(dev,"Analyzer(FDS): FDS side/block parsing pending."); return OK; }
