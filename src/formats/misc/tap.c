...
int uft_msc_tap_open(FloppyDevice *dev,const char*path){
...
int uft_msc_tap_close(FloppyDevice *dev){
...
int uft_msc_tap_read_sector(FloppyDevice *dev,uint32_t t,uint32_t h,uint32_t s,uint8_t *buf){
    (void)t;(void)h;
...
int uft_msc_tap_write_sector(FloppyDevice *dev,uint32_t t,uint32_t h,uint32_t s,const uint8_t *buf){
    (void)dev;(void)t;(void)h;(void)s;(void)buf;
    return UFT_ENOTSUP;
}

int uft_msc_tap_analyze_protection(FloppyDevice *dev){
