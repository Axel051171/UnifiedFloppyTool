...
int uft_msc_d4m_open(FloppyDevice *dev,const char*path){
...
int uft_msc_d4m_close(FloppyDevice *dev){
...
int uft_msc_d4m_read_sector(FloppyDevice *dev,uint32_t t,uint32_t h,uint32_t s,uint8_t *buf){
...
int uft_msc_d4m_write_sector(FloppyDevice *dev,uint32_t t,uint32_t h,uint32_t s,const uint8_t *buf){
    if(!dev||!dev->internal_ctx||!buf) return UFT_EINVAL;
    D4MCtx *ctx=dev->internal_ctx;
...
int uft_msc_d4m_analyze_protection(FloppyDevice *dev){
