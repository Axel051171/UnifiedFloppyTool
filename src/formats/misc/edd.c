...
int uft_msc_edd_open(FloppyDevice*dev,const char*path){
    ...
    logm(dev,"EDD opened (container stub: raw bytes).");
    return 0;
}
int uft_msc_edd_close(FloppyDevice*dev){ if(!dev||!dev->internal_ctx) return UFT_EINVAL;
    Ctx*ctx=dev->internal_ctx; fclose(ctx->fp); free(ctx); dev->internal_ctx=NULL; return 0; }
int uft_msc_edd_read_sector(FloppyDevice*dev,uint32_t t,uint32_t h,uint32_t s,uint8_t*buf){
...
int uft_msc_edd_write_sector(FloppyDevice*dev,uint32_t t,uint32_t h,uint32_t s,const uint8_t*buf){ (void)dev;(void)t;(void)h;(void)s;(void)buf; return UFT_ENOTSUP; }
int uft_msc_edd_analyze_protection(FloppyDevice*dev){ logm(dev,"Analyzer(EDD): Apple II track/nibble image; implement GCR decoder."); return 0; }
