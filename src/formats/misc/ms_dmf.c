...
int uft_msc_ms_dmf_open(FloppyDevice *dev,const char*path){
...
int uft_msc_ms_dmf_close(FloppyDevice *dev){
...
int uft_msc_ms_dmf_read_sector(FloppyDevice *dev,uint32_t t,uint32_t h,uint32_t s,uint8_t *buf){
...
int uft_msc_ms_dmf_write_sector(FloppyDevice *dev,uint32_t t,uint32_t h,uint32_t s,const uint8_t *buf){
    if(!dev||!dev->internal_ctx||!buf) return UFT_EINVAL;
    MsDmfCtx *ctx=dev->internal_ctx;
...
int uft_msc_ms_dmf_analyze_protection(FloppyDevice *dev){
