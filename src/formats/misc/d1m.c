...
static uint32_t per_image_blocks(void){
    uint32_t total=0;
...
int uft_msc_d1m_open(FloppyDevice *dev,const char*path){
    ...
    if(szl<=0 || (szl % (per_image_blocks()*256u))!=0){
        fclose(fp); free(ctx); return UFT_EINVAL;
    }

    ctx->blocks = (uint32_t)(szl / (per_image_blocks()*256u));
    ...
    log_msg(dev,"D1M opened (Commodore 8050 Mega Image).");
    return UFT_OK;
}

int uft_msc_d1m_close(FloppyDevice *dev){
...
int uft_msc_d1m_read_sector(FloppyDevice *dev,uint32_t t,uint32_t h,uint32_t s,uint8_t *buf){
...
int uft_msc_d1m_write_sector(FloppyDevice *dev,uint32_t t,uint32_t h,uint32_t s,const uint8_t *buf){
    if(!dev||!dev->internal_ctx||!buf) return UFT_EINVAL;
    D1MCtx *ctx=dev->internal_ctx;
...
int uft_msc_d1m_analyze_protection(FloppyDevice *dev){
