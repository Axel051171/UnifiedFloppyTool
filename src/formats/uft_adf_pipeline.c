...
uft_error_t adf_merge_revisions(const adf_revision_t *revisions,
    ...
    /* Byte-level merge: for each byte position, vote across revisions
     * weighted by revision quality. If majority agrees, use that value.
     * If disagreement, use the value from the highest-quality revision. */
    if (count > 1) {
        for (size_t i = 0; i < size; i++) {
            /* Count value occurrences weighted by quality */
            uint8_t values[16];
            uint8_t qualities[16];
            int nvals = 0;
            
            for (size_t r = 0; r < count && r < 16; r++) {
                if (i < revisions[r].size) {
                    values[nvals] = revisions[r].data[i];
                    qualities[nvals] = revisions[r].quality;
                    nvals++;
                }
            }
            ...
            /* Find majority value (weighted) */
            uint8_t best_val = values[0];
            int best_weight = 0;
            
            for (int v = 0; v < nvals; v++) {
                int weight = 0;
                for (int w = 0; w < nvals; w++) {
                    if (values[w] == values[v])
                        weight += qualities[w];
                }
