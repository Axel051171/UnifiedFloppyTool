...
{
	int exec_offset = -1, blocknb, first_offset, last_offset;
...
unsigned char * loadfile(char *filename,void * destptr, int * size)
{
	...
	if(f_in)
	{
		...
		if(code_size  <= *size )
		{
			...
			if(destptr)
			{
				if (fread(destptr,code_size,1,f_in) != 1) { /* I/O error */ }
				*size = code_size;
			}
			else
			{
...
int main (int argc, char *argv[])
{
	...
	if(argc == 4 )
	{
		...
		memcpy(BOOTBLOCK.dos_sign, "DOS", 3);
		/*
    ...
    if (!f_out) { fprintf(stderr, "Cannot open output\n"); return 1; }
		if(f_out)
		{
			if (fwrite(&BOOTBLOCK,sizeof(BOOTBLOCK),1,f_out) != 1) { /* I/O error */ }
			fclose(f_out);
		}
		else
		{
