...
static bool bps_parse(const uint8_t* data, size_t size, bps_file_t* bps) {
    ...
    if (memcmp(bps->signature, BPS_MAGIC, 4) == 0) {
        /* CRCs at end */
        bps->source_crc = data[size-12] | (data[size-11]<<8) | ((uint32_t)data[size-10] << 16) | ((uint32_t)data[size-9] << 24);
        bps->target_crc = data[size-8] | (data[size-7]<<8) | ((uint32_t)data[size-6] << 16) | ((uint32_t)data[size-5] << 24);
        bps->patch_crc = data[size-4] | (data[size-3]<<8) | ((uint32_t)data[size-2] << 16) | ((uint32_t)data[size-1] << 24);
        bps->valid = true;
    }
    return true;
}
