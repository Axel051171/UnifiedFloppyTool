...
static bool ups_parse(const uint8_t* data, size_t size, ups_file_t* ups) {
    ...
    if (memcmp(ups->signature, UPS_MAGIC, 4) == 0) {
        /* CRCs at end of file */
        ups->input_crc = data[size-12] | (data[size-11]<<8) | (data[size-10]<<16) | (data[size-9]<<24);
        ups->output_crc = data[size-8] | (data[size-7]<<8) | (data[size-6]<<16) | (data[size-5]<<24);
        ups->patch_crc = data[size-4] | (data[size-3]<<8) | (data[size-2]<<16) | (data[size-1]<<24);
        ups->valid = true;
    }
    return true;
}
