...
static bool pdf_parse(const uint8_t* data, size_t size, pdf_file_t* pdf) {
    ...
    if (memcmp(data, PDF_MAGIC, 5) == 0) {
        ...
        if (strstr(text, "/Linearized") != NULL) {
            pdf->is_linearized = true;
        }
        if (strstr(text, "/Encrypt") != NULL) {
...
int main(void) {
    ...
    pdf_file_t file;
    assert(pdf_parse((const uint8_t*)pdf, strlen(pdf), &file) && file.major_version == 1);
    printf("✓\n=== All tests passed! ===\n");
    return 0;
}
#endif
