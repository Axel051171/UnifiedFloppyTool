                         ...
                         uint8_t *data, size_t *data_size)
{
    ...
    for (size_t i = 0; i < out_size / 4; i++) {
        uint32_t odd = ((uint32_t)mfm[i * 4] << 24) | ((uint32_t)mfm[i * 4 + 1] << 16) |
                       (mfm[i * 4 + 2] << 8) | mfm[i * 4 + 3];
        uint32_t even = ((uint32_t)mfm[out_size + i * 4] << 24) | 
                        ((uint32_t)mfm[out_size + i * 4 + 1] << 16) |
                         ...
                         uint8_t *mfm, size_t *mfm_size)
{
    ...
    for (size_t i = 0; i < data_size / 4; i++) {
        uint32_t value = ((uint32_t)data[i * 4] << 24) | ((uint32_t)data[i * 4 + 1] << 16) |
...
uint32_t uft_amiga_checksum(const uint8_t *data, size_t size)
{
    ...
    for (size_t i = 0; i < size; i += 4) {
        uint32_t value = ((uint32_t)data[i] << 24) | ((uint32_t)data[i + 1] << 16) |
                         (data[i + 2] << 8) | data[i + 3];
        checksum ^= value;
    }
...
int uft_amiga_read_bootblock(const uft_amiga_ctx_t *ctx, uft_amiga_bootblock_t *boot)
{
    ...
    boot->checksum = ((uint32_t)bb[4] << 24) | ((uint32_t)bb[5] << 16) | (bb[6] << 8) | bb[7];
    
    /* Root block pointer (at offset 8) */
    boot->rootblock = ((uint32_t)bb[8] << 24) | ((uint32_t)bb[9] << 16) | (bb[10] << 8) | bb[11];
                             ...
                             uint8_t *mfm_track, size_t *mfm_size)
{
    ...
    for (size_t i = 0; i < 64 && pos < *mfm_size; i++) {
        mfm_track[pos++] = 0xAA;
    }
    ...
    for (int s = 0; s < sector_count; s++) {
        ...
        /* Sector header info */
        uint32_t info = 0xFF000000 | ((uint32_t)track << 16) | (side << 8) | s;
        info |= ((uint32_t)(sector_count - s) << 24);  /* Sectors to gap */
        ...
        for (size_t i = 0; i < 32 && pos < *mfm_size; i++) {
            mfm_track[pos++] = 0xAA;
        }
        ...
        /* Inter-sector gap */
        for (size_t i = 0; i < 16 && pos < *mfm_size; i++) {
            mfm_track[pos++] = 0xAA;
        }
    }
