...
static const char* stx_track_flags_str(uint16_t flags) {
    ...
    if (flags & STX_TRACK_SYNC_OFFSET) strcat(buf, "SYNC ");
    if (flags & STX_TRACK_SECTOR_READ) strcat(buf, "READ ");
    if (flags & STX_TRACK_TIMING_DATA) strcat(buf, "TIMING ");
    if (flags & STX_TRACK_FUZZY_MASK) strcat(buf, "FUZZY ");
    if (flags & STX_TRACK_PROTECTED) strcat(buf, "PROTECTED ");
...
static const char* stx_sector_flags_str(uint8_t fdcr) {
    ...
    if (fdcr & STX_SECTOR_CRC_ERROR) strcat(buf, "CRC ");
    if (fdcr & STX_SECTOR_RNF) strcat(buf, "RNF ");
    if (fdcr & STX_SECTOR_DELETED) strcat(buf, "DEL ");
    if (fdcr & STX_SECTOR_FUZZY) strcat(buf, "FUZZY ");
