...
const char *uft_zx_token_to_keyword(uint8_t token) {
    /* token <= 0xFF always true for uint8_t. Only check lower bound. */
    if (token >= ZX_TOKEN_FIRST) {
...
int uft_zx_char_to_utf8(uint8_t zx_char, char *output) {
    ...
    /* Copyright symbol (0x7F) — output as ASCII "(c)" for portability. */
    if (zx_char == 0x7F) {
        strcpy(output, "(c)");
        return 3;
    }
