...
static void xfd_detect_dos(xfd_context_t* ctx) {
    ...
    strcpy(boot->dos_name, "Unknown");
    
    /* No boot */
    if (boot->boot_flag == 0 && boot->boot_sectors == 0) {
        boot->dos_type = XFD_DOS_UNKNOWN;
        strcpy(boot->dos_name, "No DOS (not bootable)");
        return;
    }
    ...
    if (bs[7] == 'S' || (bs[7] >= 0x20 && strncmp((char*)&bs[7], "SPARTA", 6) == 0)) {
        ...
        strcpy(boot->dos_name, "SpartaDOS");
        return;
    }
    ...
    if (strstr((char*)bs + 16, "TURBO") != NULL) {
        ...
        strcpy(boot->dos_name, "Turbo-DOS");
        return;
    }
    ...
    if (boot->boot_flag != 0) {
        ...
        strcpy(boot->dos_name, "Atari DOS 2.x compatible");
    }
}
...
TEST(file_io) {
    ...
    /* Verify ATR */
    fp = fopen("/tmp/test_output.atr", "rb");
    ASSERT(fp != NULL);
