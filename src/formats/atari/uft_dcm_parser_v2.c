...
TEST(probe) {
    ...
    FILE* fp = fopen("/tmp/test.dcm", "wb"); if (!fp) return;
    ASSERT(fp != NULL);
    ...
    if (fwrite(header, 1, sizeof(header), fp) != sizeof(header)) { /* I/O error */ }
    fclose(fp);
    ...
    /* Create non-DCM file */
    fp = fopen("/tmp/test_not_dcm.bin", "wb"); if (!fp) return;
    uint8_t not_dcm[] = {0x00, 0x00, 0x00, 0x00};
    if (fwrite(not_dcm, 1, sizeof(not_dcm), fp) != sizeof(not_dcm)) { /* I/O error */ }
    fclose(fp);
...
TEST(atr_header) {
    ...
    /* Verify ATR file */
    FILE* fp = fopen("/tmp/test_output.atr", "rb"); if (!fp) return;
    ASSERT(fp != NULL);
