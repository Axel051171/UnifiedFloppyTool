...
static const char* pro_sector_flags_str(uint8_t flags) {
    ...
    if (flags & PRO_FLAG_PHANTOM) strncat(buf, "P", sizeof(buf) - strlen(buf) - 1);
    if (flags & PRO_FLAG_WEAK) strncat(buf, "W", sizeof(buf) - strlen(buf) - 1);
    if (flags & PRO_FLAG_BAD_CRC) strncat(buf, "C", sizeof(buf) - strlen(buf) - 1);
    if (flags & PRO_FLAG_DELETED) strncat(buf, "D", sizeof(buf) - strlen(buf) - 1);
    if (flags & PRO_FLAG_MISSING) strncat(buf, "M", sizeof(buf) - strlen(buf) - 1);
    if (flags & PRO_FLAG_DUPLICATE) strncat(buf, "U", sizeof(buf) - strlen(buf) - 1);
