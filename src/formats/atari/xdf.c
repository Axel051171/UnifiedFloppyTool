...
int uft_ata_xdf_close(FloppyDevice*dev){ if(!dev||!dev->internal_ctx) return EINVAL;
...
int uft_ata_xdf_write_sector(FloppyDevice*dev,uint32_t t,uint32_t h,uint32_t s,const uint8_t*buf){ (void)dev;(void)t;(void)h;(void)s;(void)buf; return ENOTSUP; }
int uft_ata_xdf_analyze_protection(FloppyDevice*dev){ logm(dev,"Analyzer(XDF): X68000 XDF ambiguity; need samples/spec for correct geometry."); return OK; }
