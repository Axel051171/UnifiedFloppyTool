...
int uft_ata_atr_open(FloppyDevice *dev, const char *path){
...
int uft_ata_atr_close(FloppyDevice *dev){
...
int uft_ata_atr_read_sector(FloppyDevice *dev,uint32_t t,uint32_t h,uint32_t s,uint8_t *buf){
    (void)h;
...
int uft_ata_atr_write_sector(FloppyDevice *dev,uint32_t t,uint32_t h,uint32_t s,const uint8_t *buf){
    (void)h;
    if(!dev||!dev->internal_ctx||!buf) return UFT_EINVAL;
    AtrCtx *ctx=dev->internal_ctx;
...
int uft_ata_atr_analyze_protection(FloppyDevice *dev){
    if(!dev||!dev->internal_ctx) return UFT_EINVAL;
