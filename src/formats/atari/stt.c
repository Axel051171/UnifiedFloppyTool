...
int uft_ata_stt_open(FloppyDevice *dev, const char *path){
...
int uft_ata_stt_close(FloppyDevice *dev){
...
int uft_ata_stt_read_sector(FloppyDevice *dev, uint32_t t, uint32_t h, uint32_t s, uint8_t *buf){
...
int uft_ata_stt_write_sector(FloppyDevice *dev, uint32_t t, uint32_t h, uint32_t s, const uint8_t *buf){
    (void)dev; (void)t; (void)h; (void)s; (void)buf;
    return UFT_ENOTSUP;
}

int uft_ata_stt_analyze_protection(FloppyDevice *dev){
    if(!dev || !dev->internal_ctx) return UFT_EINVAL;
    SttCtx *ctx = (SttCtx*)dev->internal_ctx;
