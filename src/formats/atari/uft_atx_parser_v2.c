...
atx_reader_t* atx_open(const char* path) {
    ...
    fseek(fp, reader->header.start_track, SEEK_SET);
...
int atx_read_track_v2(atx_reader_t* reader, uint8_t track_num, uint8_t side,
    ...
    fseek(reader->fp, offset, SEEK_SET);
    ...
    fseek(reader->fp, offset + sizeof(track_chunk), SEEK_SET);
        ...
        if (chunk_type == ATX_CHUNK_SECTOR_LIST) {
            ...
            for (int i = 0; i < track->sector_count && i < ATX_MAX_SECTORS; i++) {
                ...
                if (hdr.data_offset > 0 && hdr.data_size > 0) {
                    ...
                    fseek(reader->fp, offset + hdr.data_offset, SEEK_SET);
                    ...
                    fseek(reader->fp, save_pos, SEEK_SET);
                }
            }
        }
            ...
            fseek(reader->fp, chunk_size - sizeof(weak_chunk), SEEK_CUR);
        }
        else {
            /* Unbekannter Chunk - überspringen */
            fseek(reader->fp, chunk_size, SEEK_CUR);
        }
    }
