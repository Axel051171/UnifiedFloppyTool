...
static uft_error_t d71_open(uft_disk_t* disk, const char* path, bool read_only) {
    ...
    if (has_errors) {
        ...
        if (pdata->error_table) {
            if (fseek(f, D71_SIZE_STANDARD, SEEK_SET) != 0) { free(pdata->error_table); pdata->error_table = NULL; pdata->has_errors = false; }
            else if (fread(pdata->error_table, 1, D71_TOTAL_SECTORS, f) != D71_TOTAL_SECTORS) { free(pdata->error_table); pdata->error_table = NULL; pdata->has_errors = false; }
        }
    }
...
static const uft_plugin_feature_t uft_format_plugin_d71_features[] = {
...
const uft_format_plugin_t uft_format_plugin_d71 = {
    ...
    .features = uft_format_plugin_d71_features,  /* V415-PLAN PLUGIN.features (MF-263) */
    .feature_count = sizeof(uft_format_plugin_d71_features) / sizeof(uft_format_plugin_d71_features[0]),
};
