...
uft_error_t uft_rcpmfs_read_mem(const uint8_t *data, size_t size,
    ...
    if (cylinders == 0 || cylinders > 1024 ||
        ...
        if (result) {
            result->error = UFT_ERR_FORMAT;
            result->error_detail =
                "RCPMFS geometry out of range (MF-554)";
        }
        return UFT_ERR_FORMAT;
    }
    {
        ...
        if (need > (uint64_t)size) {
            if (result) {
                result->error = UFT_ERR_FORMAT;
                result->error_detail =
                    "RCPMFS header claims more data than the file holds "
                    "(MF-554)";
            }
            return UFT_ERR_FORMAT;
        }
...
static uft_error_t rcpmfs_write_track(uft_disk_t *disk, int cyl, int head,
     ...
     *
     * Diese Datei HAT einen echten Dateischreiber — `uft_rcpmfs_write()`,
...
const uft_format_plugin_t uft_format_plugin_rcpmfs = {
    ...
    /* MF-1077: libdsks `rcpmfs` ist ein VERZEICHNISTREIBER, kein Behaelter (MF-1035).
    ...
    .features = uft_format_plugin_rcpmfs_features,  /* V415-PLAN PLUGIN.features (MF-263) */
    .feature_count = sizeof(uft_format_plugin_rcpmfs_features) / sizeof(uft_format_plugin_rcpmfs_features[0]),
};
