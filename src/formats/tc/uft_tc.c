...
void uft_tc_close(uft_tc_t *tc) {
    ...
    /* Auto-save on close: write modified tracks back to TC file */
    if (tc->modified && tc->path) {
        uft_tc_save(tc, tc->path);
    }
    ...
    for (int t = 0; t < uft_tc_get_tracks(tc); t++) {
        for (int s = 0; s < uft_tc_get_sides(tc); s++) {
            ...
            for (int sec = 0; sec < spt; sec++) {
                ...
                /* Decode MFM data from TC track bitstream */
                if (tc->tracks && tc->tracks[t * uft_tc_get_sides(tc) + s]) {
                    memcpy(sector, tc->tracks[t * uft_tc_get_sides(tc) + s]->data + sec * 512, 512);
                }
...
int uft_img_to_tc(const char *img_path, const char *tc_path) {
    ...
    for (int t = 0; t < tracks; t++) {
        for (int s = 0; s < sides; s++) {
            ...
            /* Encode sector data to TC track format (raw storage) */
    ...
    /* Save TC: file built in-memory, written on close */
    uft_tc_close(tc);
    return 0;
}
