...
int uft_pc98_hdm_open(FloppyDevice *dev, const char *path) {
...
int uft_pc98_hdm_close(FloppyDevice *dev) {
...
int uft_pc98_hdm_read_sector(FloppyDevice *dev, uint32_t t, uint32_t h, uint32_t s, uint8_t *buf) {
...
int uft_pc98_hdm_write_sector(FloppyDevice *dev, uint32_t t, uint32_t h, uint32_t s, const uint8_t *buf) {
    if (!dev || !dev->internal_ctx || !buf) return UFT_EINVAL;
    HdmCtx *ctx = (HdmCtx*)dev->internal_ctx;
...
int uft_pc98_hdm_analyze_protection(FloppyDevice *dev) {
    if (!dev || !dev->internal_ctx) return UFT_EINVAL;
