...
int uft_pc98_fdd_open(FloppyDevice*dev,const char*path){
    ...
    logm(dev,"FDD opened (container stub: raw bytes).");
    return 0;
}
int uft_pc98_fdd_close(FloppyDevice*dev){ if(!dev||!dev->internal_ctx) return UFT_EINVAL;
    Ctx*ctx=dev->internal_ctx; fclose(ctx->fp); free(ctx); dev->internal_ctx=NULL; return 0; }
int uft_pc98_fdd_read_sector(FloppyDevice*dev,uint32_t t,uint32_t h,uint32_t s,uint8_t*buf){
...
int uft_pc98_fdd_write_sector(FloppyDevice*dev,uint32_t t,uint32_t h,uint32_t s,const uint8_t*buf){ (void)dev;(void)t;(void)h;(void)s;(void)buf; return UFT_ENOTSUP; }
int uft_pc98_fdd_analyze_protection(FloppyDevice*dev){ logm(dev,"Analyzer(FDD): PC-98 emulator format; implement header+geometry inference once we have samples."); return 0; }
