...
int floppy_write_sector(FloppyDevice*dev,uint32_t t,uint32_t h,uint32_t s,const uint8_t*buf){ (void)dev;(void)t;(void)h;(void)s;(void)buf; return ENOTSUP; }
int floppy_analyze_protection(FloppyDevice*dev){ logm(dev,"Analyzer(FDD): PC-98 emulator format; implement header+geometry inference once we have samples."); return OK; }
