...
typedef struct {
    ...
    uint8_t n;              /* size code */
    uint16_t nsec;          /* number of sectors on track */
    uint16_t sectors_in_track; /* sectors in this track */
    uint8_t density_flag;   /* 0=double, 0x40=single */
    uint8_t deleted_flag;   /* 0=normal, 0x10=deleted */
    uint8_t status;         /* 0=ok, other=error */
    uint8_t reserved[5];
...
int uft_pc98_d88_open(FloppyDevice *dev, const char *path) {
...
int uft_pc98_d88_close(FloppyDevice *dev) {
...
int uft_pc98_d88_read_sector(FloppyDevice *dev, uint32_t t, uint32_t h, uint32_t s, uint8_t *buf) {
...
int uft_pc98_d88_write_sector(FloppyDevice *dev, uint32_t t, uint32_t h, uint32_t s, const uint8_t *buf) {
    if (!dev || !dev->internal_ctx || !buf) return UFT_EINVAL;
    D88Ctx *ctx = (D88Ctx*)dev->internal_ctx;
...
int uft_pc98_d88_analyze_protection(FloppyDevice *dev) {
    if (!dev || !dev->internal_ctx) return UFT_EINVAL;
    D88Ctx *ctx = (D88Ctx*)dev->internal_ctx;
    ...
    for (uint32_t t = 0; t < dev->tracks; t++) {
        for (uint32_t h = 0; h < dev->heads; h++) {
            for (uint32_t s = 1; s <= dev->sectors; s++) {
                ...
                D88SectorInfo *si = find_sector(ctx, t, h, s);
                if (si) (void)uft_pc98_d88_read_sector(dev, t, h, s, buf);
