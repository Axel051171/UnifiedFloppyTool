...
/* In-memory write: updates cached sector data. Persist via uft_hardsector_write(). */
static uft_error_t hardsector_write_track(uft_disk_t *disk, int cyl, int head,
     ...
     *
     * Diese Datei HAT einen echten Dateischreiber — `uft_hardsector_write()`,
...
const uft_format_plugin_t uft_format_plugin_hardsector = {
    ...
    /* MF-1077: eine GROESSENTAFEL ueber rohe Sektorabbilder. P3-340
     * sagt es woertlich: `hardsector` „beschreibt eine
     * Geometriefamilie, kein Dateiformat“ (MF-1058).
     * Kein Behaelterformat, also ist „unverifiziert“
     * die falsche Aussage — die Stufentabelle fuehrt diesen
     * Eintrag seither als `n/a`. */
    .kind = UFT_KIND_GEOMETRIEKATALOG,
    .features = uft_format_plugin_hardsector_features,  /* V415-PLAN PLUGIN.features (MF-263) */
    .feature_count = sizeof(uft_format_plugin_hardsector_features) / sizeof(uft_format_plugin_hardsector_features[0]),
};
