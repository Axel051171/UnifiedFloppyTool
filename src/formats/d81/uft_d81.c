...
static uft_error_t d81_write_track(uft_disk_t* disk, int cyl, int head,
    ...
    for (size_t s = 0; s < track->sector_count && (int)s < D81_SECTORS_PER_TRACK; s++) {
        ...
        if (!data || track->sectors[s].data_len == 0) {
            memset(pad, 0xE5, D81_SECTOR_SIZE); data = pad;
        }
...
static const uft_plugin_feature_t uft_format_plugin_d81_features[] = {
...
const uft_format_plugin_t uft_format_plugin_d81 = {
    ...
    .features = uft_format_plugin_d81_features,  /* V415-PLAN PLUGIN.features (MF-263) */
    .feature_count = sizeof(uft_format_plugin_d81_features) / sizeof(uft_format_plugin_d81_features[0]),
};
