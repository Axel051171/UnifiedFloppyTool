...
int uft_save_d64(const char *path, const uft_disk_image_t *image, d64_version_t version)
{
    ...
    if (image->size >= data_size) {
        fwrite(image->data, 1, data_size, fp);
    } else {
        fwrite(image->data, 1, image->size, fp);
        /* Pad with zeros */
        uint8_t zero = 0;
        for (size_t i = image->size; i < data_size; i++) {
            fwrite(&zero, 1, 1, fp);
        }
    }
    ...
    if (include_errors) {
        ...
        if (image->error_info) {
            fwrite(image->error_info, 1, error_count, fp);
        } else {
            /* Write default "no error" values */
            uint8_t no_error = 0x01;
            for (int i = 0; i < error_count; i++) {
                fwrite(&no_error, 1, 1, fp);
            }
        }
    }
...
int uft_save_atr(const char *path, const uft_disk_image_t *image, atr_version_t version)
{
    ...
    fwrite(image->data, 1, write_size, fp);
    
    /* Pad if needed */
    if (write_size < data_size) {
        uint8_t zero = 0;
        for (size_t i = write_size; i < data_size; i++) {
            fwrite(&zero, 1, 1, fp);
        }
    }
