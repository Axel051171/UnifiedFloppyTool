...
uff_file_t* uff_create(const char* path) {
    ...
    fwrite(&uff->header, sizeof(uff_header_t), 1, uff->fp);
...
uff_file_t* uff_open(const char* path) {
    ...
    while (!feof(uff->fp)) {
        ...
        switch (chunk.type) {
            case UFF_CHUNK_INFO:
                fread(&uff->info, sizeof(uff_info_t), 1, uff->fp);
                uff->info_valid = true;
                break;
                
            case UFF_CHUNK_CAPT:
                fread(&uff->capture, sizeof(uff_capture_t), 1, uff->fp);
                uff->capture_valid = true;
                break;
                
            case UFF_CHUNK_TRCK: {
                uff_track_header_t th;
                fread(&th, sizeof(th), 1, uff->fp);
            ...
            case UFF_CHUNK_FLUX: {
                ...
                if (fh.track < UFF_MAX_TRACKS && fh.side < UFF_MAX_SIDES &&
                    ...
                    if (t->flux_samples[fh.revolution]) {
                        fread(t->flux_samples[fh.revolution], 
                              sizeof(uint32_t), fh.sample_count, uff->fp);
                    }
            ...
            case UFF_CHUNK_WEAK: {
                ...
                if (wh.track < UFF_MAX_TRACKS && wh.side < UFF_MAX_SIDES) {
                    ...
                    fread(t->weak_regions, sizeof(uff_weak_region_t),
                          t->weak_count, uff->fp);
                }
                break;
            }
            ...
            case UFF_CHUNK_PROT: {
                if (uff->protection_count < 16) {
                    fread(&uff->protection[uff->protection_count++],
                          sizeof(uff_protection_info_t), 1, uff->fp);
                }
                break;
            }
            ...
            case UFF_CHUNK_AUDT: {
                if (uff->audit_count < UFF_MAX_AUDIT) {
                    fread(&uff->audit[uff->audit_count++],
                          sizeof(uff_audit_entry_t), 1, uff->fp);
                }
                break;
            }
            ...
            default:
                /* Skip unknown chunk */
                fseek(uff->fp, chunk.size, SEEK_CUR);
                break;
        }
...
void uff_close(uff_file_t* uff) {
    ...
    if (uff->write_mode && uff->fp) {
        ...
        if (uff->info_valid) {
            ...
            fwrite(&chunk, sizeof(chunk), 1, uff->fp);
            fwrite(&uff->info, sizeof(uff_info_t), 1, uff->fp);
            uff->total_chunks++;
        }
        ...
        if (uff->capture_valid) {
            ...
            fwrite(&chunk, sizeof(chunk), 1, uff->fp);
            fwrite(&uff->capture, sizeof(uff_capture_t), 1, uff->fp);
            uff->total_chunks++;
        }
        ...
        for (int t = 0; t < UFF_MAX_TRACKS; t++) {
            for (int s = 0; s < UFF_MAX_SIDES; s++) {
                ...
                fwrite(&chunk, sizeof(chunk), 1, uff->fp);
                fwrite(&track->header, sizeof(uff_track_header_t), 1, uff->fp);
                uff->total_chunks++;
                ...
                for (int r = 0; r < track->revolutions; r++) {
                    ...
                    fwrite(&fchunk, sizeof(fchunk), 1, uff->fp);
                    fwrite(&fh, sizeof(fh), 1, uff->fp);
                    fwrite(track->flux_samples[r], sizeof(uint32_t),
                           track->flux_count[r], uff->fp);
                ...
                if (track->weak_count > 0) {
                    ...
                    fwrite(&wchunk, sizeof(wchunk), 1, uff->fp);
                    fwrite(&wh, sizeof(wh), 1, uff->fp);
                    fwrite(track->weak_regions, sizeof(uff_weak_region_t),
                           track->weak_count, uff->fp);
        ...
        for (int i = 0; i < uff->protection_count; i++) {
            ...
            fwrite(&chunk, sizeof(chunk), 1, uff->fp);
            fwrite(&uff->protection[i], sizeof(uff_protection_info_t), 1, uff->fp);
            uff->total_chunks++;
        }
        ...
        for (int i = 0; i < uff->audit_count; i++) {
            ...
            fwrite(&chunk, sizeof(chunk), 1, uff->fp);
            fwrite(&uff->audit[i], sizeof(uff_audit_entry_t), 1, uff->fp);
            uff->total_chunks++;
        }
