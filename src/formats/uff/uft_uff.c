...
uff_file_t* uff_create(const char* path, uint8_t cylinders, uint8_t heads, 
    ...
    if (fwrite(uff->track_index, sizeof(uff_track_index_t), track_count, f) != track_count) { /* write error */ }
...
void uff_close(uff_file_t* uff) {
    ...
    if (uff->is_write && f) {
        ...
        /* Calculate CRC64 over entire file content (header through last track) */
        {
            long file_end = ftell(f);
            if (file_end > 0 && fseek(f, 0, SEEK_SET) == 0) {
                uint64_t crc64 = 0;
                uint8_t buf[4096];
                long remaining = file_end;
                while (remaining > 0) {
                    size_t chunk = (remaining > (long)sizeof(buf)) ? sizeof(buf) : (size_t)remaining;
                    if (fread(buf, 1, chunk, f) != chunk) break;
                    crc64 = compute_crc64(buf, chunk);  /* Note: reseeds each chunk */
                    remaining -= (long)chunk;
                }
                /* Re-compute as single pass for correctness */
                if (fseek(f, 0, SEEK_SET) == 0 && file_end <= 64 * 1024 * 1024) {
                    uint8_t *all = (uint8_t *)malloc((size_t)file_end);
                    if (all) {
                        if (fread(all, 1, (size_t)file_end, f) == (size_t)file_end) {
                            crc64 = compute_crc64(all, (size_t)file_end);
                        }
                        free(all);
                    }
                }
                fseek(f, file_end, SEEK_SET);
                footer.file_crc64 = crc64;
            } else {
                footer.file_crc64 = 0;
            }
        }
        ...
        /* Rewrite track index */
        if (fwrite(uff->track_index, sizeof(uff_track_index_t), 
               uff->track_index_count, f) != uff->track_index_count) { /* I/O error */ }
    }
...
int uff_write_track(uff_file_t* uff, const uff_track_data_t* track) {
    ...
    /* Write track header */
    if (fwrite(&thdr, sizeof(uff_track_header_t), 1, f) != 1) { /* write error */ }
    
    /* Write revolution headers */
    if (fwrite(track->revolutions, sizeof(uff_revolution_t), track->revolution_count, f) != track->revolution_count) { /* write error */ }
    
    /* Write flux data */
    for (uint32_t r = 0; r < track->revolution_count; r++) {
        if (fwrite(track->flux_data[r], sizeof(uint32_t), track->flux_counts[r], f) != track->flux_counts[r]) { /* write error */ }
    }
    
    /* Write weak bit map */
    if (track->weak_count > 0) {
        if (fwrite(&track->weak_count, sizeof(uint32_t), 1, f) != 1) { /* write error */ }
        if (fwrite(track->weak_regions, sizeof(uff_weak_region_t), track->weak_count, f) != track->weak_count) { /* write error */ }
    }
    
    /* Write splice points */
    if (track->splice_count > 0) {
        if (fwrite(&track->splice_count, sizeof(uint32_t), 1, f) != 1) { /* write error */ }
        if (fwrite(track->splices, sizeof(uff_splice_point_t), track->splice_count, f) != track->splice_count) { /* write error */ }
    }
    
    /* Write hash */
    if (fwrite(track->sha256, 32, 1, f) != 1) { /* write error */ }
...
void test_uff_format(void) {
    ...
    printf("Test 4: Forensic metadata... ");
    {
        ...
        strncpy(forensic.capture_device, "Greaseweazle F7", sizeof(forensic.capture_device)-1);
        strncpy(forensic.examiner, "Test User", sizeof(forensic.examiner)-1);
        strncpy(forensic.case_number, "CASE-001", sizeof(forensic.case_number)-1);
