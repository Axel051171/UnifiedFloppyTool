                           ...
                           uft_format_detect_t* result)
{
    ...
    result->confidence = 0;
    strncpy(result->name, "Unknown", sizeof(result->name) - 1); result->name[sizeof(result->name) - 1] = '\0';
    ...
    if (size >= 16) {
        ...
        if ((wp == 0x00 || wp == 0xFF) &&
            ...
            strncpy(result->name, "DMK", sizeof(result->name) - 1); result->name[sizeof(result->name) - 1] = '\0';
            return true;
        }
    }
    ...
    if (size >= 12) {
        if (data[0] == 'G' && data[1] == 'C' && data[2] == 'R' && data[3] == '-') {
            ...
            result->confidence = 100;
            strncpy(result->name, "G64", sizeof(result->name) - 1); result->name[sizeof(result->name) - 1] = '\0';
            return true;
        }
    }
    ...
    if (size_type != UFT_FORMAT_UNKNOWN) {
        ...
        strncpy(result->name, uft_libflux_format_name(size_type), 31);
        result->name[31] = '\0';
        return true;
    }
