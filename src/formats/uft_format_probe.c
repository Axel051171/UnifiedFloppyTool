...
static int probe_adf_structure(const uint8_t* data, size_t size) {
    ...
    uint32_t checksum = 0;
    for (int i = 0; i < 256; i++) {
        uint32_t word = ((uint32_t)data[i*4] << 24) | ((uint32_t)data[i*4+1] << 16) |
    ...
    if (size >= 901120) {
        ...
        uint32_t type = ((uint32_t)data[root_offset] << 24) | ((uint32_t)data[root_offset+1] << 16) |
...
static int probe_ipf_structure(const uint8_t* data, size_t size) {
    ...
    uint32_t encoder = ((uint32_t)data[12] << 24) | ((uint32_t)data[13] << 16) |
                       (data[14] << 8) | data[15];
