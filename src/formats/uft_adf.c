...
int uft_adf_get_info(uft_adf_volume_t *vol, uft_adf_info_t *info) {
    ...
    /* TODO: Calculate free blocks from bitmap */
    info->free_blocks = 0;
    info->used_blocks = vol->total_blocks;
...
int uft_adf_create(const char *path, uft_adf_density_t density,
    ...
    *(uint32_t *)bootblock = cpu_to_be32(dos_type);
    ...
    /* Write root block */
    fseek(fp, (long)root_block * UFT_ADF_SECTOR_SIZE, SEEK_SET);
    ...
    /* Write bitmap */
    fseek(fp, (long)(root_block + 1) * UFT_ADF_SECTOR_SIZE, SEEK_SET);
