...
int uft_apl_prodos_po_do_open(FloppyDevice *dev, const char *path){
...
int uft_apl_prodos_po_do_close(FloppyDevice *dev){
...
int uft_apl_prodos_po_do_read_sector(FloppyDevice *dev, uint32_t t, uint32_t h, uint32_t s, uint8_t *buf){
    (void)h;
...
int uft_apl_prodos_po_do_write_sector(FloppyDevice *dev, uint32_t t, uint32_t h, uint32_t s, const uint8_t *buf){
    (void)h;
    if(!dev || !dev->internal_ctx || !buf) return UFT_EINVAL;
    ProdosCtx *ctx = dev->internal_ctx;
...
int uft_apl_prodos_po_do_analyze_protection(FloppyDevice *dev){
    if(!dev || !dev->internal_ctx) return UFT_EINVAL;
