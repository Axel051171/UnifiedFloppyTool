...
typedef struct {
    ...
    uint32_t data_size;
    uint32_t format;     /* 0=DOS3.3, 1=ProDOS, 2=NIB */
} TwoMgMeta;
...
int uft_apl_2mg_open(FloppyDevice *dev, const char *path){
...
int uft_apl_2mg_close(FloppyDevice *dev){
...
int uft_apl_2mg_read_sector(FloppyDevice *dev,uint32_t t,uint32_t h,uint32_t s,uint8_t *buf){
    (void)h;
...
int uft_apl_2mg_write_sector(FloppyDevice *dev,uint32_t t,uint32_t h,uint32_t s,const uint8_t *buf){
    (void)h;
    if(!dev||!dev->internal_ctx||!buf) return UFT_EINVAL;
    TwoMgCtx *ctx=dev->internal_ctx;
...
int uft_apl_2mg_analyze_protection(FloppyDevice *dev){
    if(!dev||!dev->internal_ctx) return UFT_EINVAL;
