...
static char* trd_catalog_to_text(const trd_disk_t* disk) {
    ...
    offset += snprintf(buf + offset, buf_size - offset,
        ...
        disk->geometry.name,
        (unsigned)disk->geometry.tracks,
        (unsigned)disk->geometry.sides,
        (unsigned)(disk->file_count - disk->deleted_count),
        (unsigned)disk->deleted_count,
        (unsigned)disk->system.free_sectors,
        (unsigned)(disk->system.free_sectors * TRD_SECTOR_SIZE / 1024),
