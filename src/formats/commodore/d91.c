/**
 * @file d91.c
 * @brief Commodore D91 disk image format
 * @version 3.8.0
 */
// d91.c - Commodore D91 (C11)
...
/* DOS 1.x sectors per track (2040) */
...
int uft_cbm_d91_open(FloppyDevice *dev,const char*path){
    if(!dev||!path) return UFT_EINVAL;
    D91Ctx *ctx=calloc(1,sizeof(D91Ctx));
    ...
    log_msg(dev,"D91 opened (Commodore 2040 DOS 1.x).");
    return UFT_OK;
}

int uft_cbm_d91_close(FloppyDevice *dev){
    if(!dev||!dev->internal_ctx) return UFT_EINVAL;
    D91Ctx *ctx=dev->internal_ctx;
    fclose(ctx->fp);
    free(ctx);
...
int uft_cbm_d91_read_sector(FloppyDevice *dev,uint32_t t,uint32_t h,uint32_t s,uint8_t *buf){
...
int uft_cbm_d91_write_sector(FloppyDevice *dev,uint32_t t,uint32_t h,uint32_t s,const uint8_t *buf){
    if(!dev||!dev->internal_ctx||!buf) return UFT_EINVAL;
    D91Ctx *ctx=dev->internal_ctx;
...
int uft_cbm_d91_analyze_protection(FloppyDevice *dev){
    log_msg(dev,"Analyzer(D91): Commodore 2040 DOS 1.x sector image.");
    log_msg(dev,"Analyzer(D91): no GCR timing or copy-protection preserved.");
    return UFT_OK;
}
