...
int uft_cbm_dnp2_open(FloppyDevice *dev,const char*path){
...
int uft_cbm_dnp2_close(FloppyDevice *dev){
...
int uft_cbm_dnp2_read_sector(FloppyDevice *dev,uint32_t t,uint32_t h,uint32_t s,uint8_t *buf){
    (void)t; (void)h;
...
int uft_cbm_dnp2_write_sector(FloppyDevice *dev,uint32_t t,uint32_t h,uint32_t s,const uint8_t *buf){
    (void)t; (void)h;
    if(!dev||!dev->internal_ctx||!buf) return UFT_EINVAL;
    DNP2Ctx *ctx=dev->internal_ctx;
...
int uft_cbm_dnp2_analyze_protection(FloppyDevice *dev){
