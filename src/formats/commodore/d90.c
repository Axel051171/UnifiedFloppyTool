...
/* 4040 / 2031 sectors per track (tracks 1-35) */
...
int uft_cbm_d90_open(FloppyDevice *dev,const char*path){
    if(!dev||!path) return UFT_EINVAL;
    D90Ctx *ctx=calloc(1,sizeof(D90Ctx));
    ...
    log_msg(dev,"D90 opened (Commodore 4040/2031).");
    return UFT_OK;
}

int uft_cbm_d90_close(FloppyDevice *dev){
    if(!dev||!dev->internal_ctx) return UFT_EINVAL;
    D90Ctx *ctx=dev->internal_ctx;
    fclose(ctx->fp);
    free(ctx);
...
int uft_cbm_d90_read_sector(FloppyDevice *dev,uint32_t t,uint32_t h,uint32_t s,uint8_t *buf){
...
int uft_cbm_d90_write_sector(FloppyDevice *dev,uint32_t t,uint32_t h,uint32_t s,const uint8_t *buf){
    if(!dev||!dev->internal_ctx||!buf) return UFT_EINVAL;
    D90Ctx *ctx=dev->internal_ctx;
...
int uft_cbm_d90_analyze_protection(FloppyDevice *dev){
    log_msg(dev,"Analyzer(D90): Commodore 4040/2031 sector image.");
    log_msg(dev,"Analyzer(D90): no GCR timing or copy-protection data preserved.");
    return UFT_OK;
}
