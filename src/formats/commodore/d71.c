...
int uft_cbm_d71_open(FloppyDevice *dev, const char *path){
...
int uft_cbm_d71_close(FloppyDevice *dev){
...
int uft_cbm_d71_read_sector(FloppyDevice *dev, uint32_t t, uint32_t h, uint32_t s, uint8_t *buf){
...
int uft_cbm_d71_write_sector(FloppyDevice *dev, uint32_t t, uint32_t h, uint32_t s, const uint8_t *buf){
    if(!dev || !dev->internal_ctx || !buf) return UFT_EINVAL;
    D71Ctx *ctx = (D71Ctx*)dev->internal_ctx;
...
int uft_cbm_d71_analyze_protection(FloppyDevice *dev){
    if(!dev || !dev->internal_ctx) return UFT_EINVAL;
