...
int uft_cbm_x64_open(FloppyDevice *dev,const char*path){
...
int uft_cbm_x64_close(FloppyDevice *dev){
...
int uft_cbm_x64_read_sector(FloppyDevice *dev,uint32_t t,uint32_t h,uint32_t s,uint8_t *buf){
...
int uft_cbm_x64_write_sector(FloppyDevice *dev,uint32_t t,uint32_t h,uint32_t s,const uint8_t *buf){
    if(!dev||!dev->internal_ctx||!buf) return UFT_EINVAL;
    X64Ctx *ctx=dev->internal_ctx;
...
int uft_cbm_x64_analyze_protection(FloppyDevice *dev){
