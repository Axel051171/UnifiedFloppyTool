...
const char* fdi_flags_description(uint8_t flags) {
    ...
    if (flags & FDI_FLAG_NO_DATA)   strcat(buf, "No Data, ");
    if (flags & FDI_FLAG_DELETED)   strcat(buf, "Deleted, ");
    if (flags & FDI_FLAG_CRC_ERROR) strcat(buf, "CRC Error, ");
    if (flags & FDI_FLAG_NO_ID)     strcat(buf, "No ID, ");
    if (flags & FDI_FLAG_NO_DAM)    strcat(buf, "No DAM, ");
    if (flags & FDI_FLAG_WEAK)      strcat(buf, "Weak, ");
