...
uft_track_t* uft_track_read(uft_disk_t* disk, int cylinder, int head,
    ...
    // Im Cache suchen
    size_t cache_idx = cylinder * disk->geometry.heads + head;
    if (disk->track_cache && cache_idx < disk->cache_size) {
        if (disk->track_cache[cache_idx]) {
            // Kopie zurückgeben? Oder Referenz mit Refcount?
            // Für jetzt: einfach Referenz (User darf nicht freigeben!)
            // TODO: Besseres Cache-Management
        }
    }
