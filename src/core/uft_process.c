...
uft_async_process_t* uft_process_start_async(
    ...
    void *user_data)
{
    /* Async process execution using fork() and pipe() on POSIX.
     * Creates a child process and returns handle for monitoring. */
    if (!command) return NULL;
    
    uft_async_process_t *proc = calloc(1, sizeof(uft_async_process_t));
    if (!proc) return NULL;
...
#ifdef _WIN32
    /* Windows: use CreateProcess (stub for cross-platform) */
    free(proc);
    return NULL;
#else
    int pipefd[2];
    if (pipe(pipefd) == -1) { free(proc); return NULL; }
    ...
    if (pid == 0) {
        ...
        if (options && options->shell) shell = options->shell;
