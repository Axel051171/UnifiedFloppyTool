...
uft_audit_session_t* uft_audit_session_load(const char* path) {
    /* Load binary audit log from file.
     * Format: [4-byte magic "UFTL"][4-byte version][4-byte entry_count]
     *         [entries: each is fixed-size uft_audit_entry_t] */
    if (!path) return NULL;
    ...
    char magic[4];
    if (fread(magic, 1, 4, f) != 4 || memcmp(magic, "UFTL", 4) != 0) {
        fclose(f);
        return NULL;
    }
    ...
    if (entry_count > 100000) { fclose(f); return NULL; }  /* Sanity limit */
    
    uft_audit_session_t *session = calloc(1, sizeof(uft_audit_session_t));
    if (!session) { fclose(f); return NULL; }
    
    session->entries = calloc(entry_count, sizeof(uft_audit_entry_t));
    if (!session->entries) {
        free(session);
        fclose(f);
        return NULL;
    ...
    for (uint32_t i = 0; i < entry_count; i++) {
        size_t rd = fread(&session->entries[i], sizeof(uft_audit_entry_t), 1, f);
        if (rd != 1) break;
        session->entry_count++;
    }
