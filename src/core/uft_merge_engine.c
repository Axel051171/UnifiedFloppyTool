    ...
    uft_merged_sector_t *out)
{
    ...
    switch (strategy) {
        ...
        case UFT_MERGE_MAJORITY:
            /* Byte-by-byte majority voting across candidates */
            {
                /* Find a base candidate for size reference */
                size_t data_size = 0;
                for (int i = 0; i < bucket->count; i++) {
                    if (bucket->candidates[i].data && bucket->candidates[i].data_size > data_size)
                        data_size = bucket->candidates[i].data_size;
                }
                
                if (data_size > 0 && bucket->count > 1) {
                    /* Allocate voted output */
                    uint8_t *voted = (uint8_t *)malloc(data_size);
                    if (voted) {
                        for (size_t pos = 0; pos < data_size; pos++) {
                            ...
                            for (int c = 0; c < bucket->count; c++) {
                                if (bucket->candidates[c].data && 
                                    pos < bucket->candidates[c].data_size) {
                                    /* Weight CRC-OK revisions higher */
                                    int weight = bucket->candidates[c].crc_ok ? 3 : 1;
                                    int val = bucket->candidates[c].data[pos];
                                    if (counts[val] + weight <= 255)
                                        counts[val] += weight;
                                    else
                                        counts[val] = 255;
                                    total_votes += weight;
                        ...
                        /* Create a synthetic winner from voted data */
                        winner = &bucket->candidates[0];  /* Use first as template */
                        memcpy(winner->data, voted, 
                               data_size < winner->data_size ? data_size : winner->data_size);
                        free(voted);
                        
                        snprintf(out->merge_reason, sizeof(out->merge_reason),
                                 "Majority vote (%d candidates)", bucket->count);
                    } else {
                        ...
                        snprintf(out->merge_reason, sizeof(out->merge_reason),
                                 "Majority fallback (alloc failed)");
                    }
                } else {
                    /* Single candidate or no data: use highest score */
                    for (int i = 0; i < bucket->count; i++) {
                        if (bucket->candidates[i].score.total > best_score) {
                            best_score = bucket->candidates[i].score.total;
                            winner = &bucket->candidates[i];
