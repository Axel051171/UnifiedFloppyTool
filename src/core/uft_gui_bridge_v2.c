...
typedef struct {
    ...
    atomic_int version;  // Incremented on each change
...
int uft_gui_bridge_init(void) {
    ...
    atomic_store(&g_bridge.version, 1);
...
int uft_gui_set_bool(const char* name, bool value) {
    ...
    atomic_fetch_add(&g_bridge.version, 1);
...
int uft_gui_to_json(char* buf, size_t buf_size) {
    ...
    for (int i = 0; i < g_bridge.param_count && (size_t)pos < buf_size - 100; i++) {
...
int uft_gui_get_version(void) {
    return atomic_load(&g_bridge.version);
}
