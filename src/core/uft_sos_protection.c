...
bool uft_sos_detect_ipf(const char *ipf_path) {
    /* TODO: Open IPF and check track format */
    (void)ipf_path;
    return false;
}

bool uft_sos_detect_kf_raw(const char *raw_path) {
    /* TODO: Open KryoFlux RAW and detect */
    (void)raw_path;
    return false;
}
...
int uft_sos_decode_mfm(const uint8_t *mfm, size_t mfm_size,
    ...
    while (src_pos + 8 <= mfm_size && decoded < data_size) {
        ...
        uint32_t odd = (mfm[src_pos] << 24) | (mfm[src_pos + 1] << 16) |
                       (mfm[src_pos + 2] << 8) | mfm[src_pos + 3];
        uint32_t even = (mfm[src_pos + 4] << 24) | (mfm[src_pos + 5] << 16) |
                        (mfm[src_pos + 6] << 8) | mfm[src_pos + 7];
...
int uft_sos_decode_track(uft_sos_t *sos,
    ...
    if (decoded >= 16) {
        ...
        track->header.checksum = (track->data[4] << 24) | (track->data[5] << 16) |
                                  (track->data[6] << 8) | track->data[7];
        track->header.data_length = (track->data[8] << 24) | (track->data[9] << 16) |
                                     (track->data[10] << 8) | track->data[11];
...
int uft_sos_open_kf_raw(uft_sos_t *sos, const char *base_path) {
    ...
    /* TODO: Implement KryoFlux RAW loading */
    ...
    for (int t = 0; t < sos->num_tracks; t++) {
        for (int h = 0; h < sos->num_heads; h++) {
            ...
            /* TODO: Proper KryoFlux stream parsing */
...
int uft_sos_read_all_tracks(uft_sos_t *sos, uint8_t *data, size_t max_size) {
    ...
    if (sos->tracks) {
        int total_tracks = sos->num_tracks * sos->num_heads;
        for (int i = 0; i < total_tracks && offset < max_size; i++) {
