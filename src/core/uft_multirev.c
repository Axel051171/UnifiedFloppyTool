...
static void align_all_revolutions(uft_mrv_context_t* ctx) {
    ...
    for (int i = 1; i < ctx->rev_count; i++) {
        ...
        /* Apply offset by rotating the bit stream.
         * Positive offset means rev is ahead of ref, so shift bits left.
         * Negative means rev is behind, shift right. */
        if (offset != 0 && rev->bit_count > 0) {
            int abs_off = offset < 0 ? -offset : offset;
            if ((uint32_t)abs_off < rev->bit_count) {
                size_t byte_count = (rev->bit_count + 7) / 8;
                uint8_t *tmp = (uint8_t *)malloc(byte_count);
                if (tmp) {
                    memset(tmp, 0, byte_count);
                    for (uint32_t b = 0; b < rev->bit_count; b++) {
                        uint32_t src_bit;
                        if (offset > 0) {
                            /* Shift left: source is offset bits ahead */
                            src_bit = (b + (uint32_t)offset) % rev->bit_count;
                        } else {
                            /* Shift right: source is |offset| bits behind */
                            src_bit = (b + rev->bit_count - (uint32_t)abs_off) % rev->bit_count;
                        }
                        /* Copy bit from src_bit position to b position */
                        if (rev->bits[src_bit / 8] & (0x80 >> (src_bit % 8))) {
                            tmp[b / 8] |= (0x80 >> (b % 8));
                        }
                    }
                    memcpy(rev->bits, tmp, byte_count);
                    
                    /* Also rotate confidence array if present */
                    if (rev->confidence) {
                        uint8_t *tmp_conf = (uint8_t *)malloc(rev->bit_count);
                        if (tmp_conf) {
                            for (uint32_t b = 0; b < rev->bit_count; b++) {
                                uint32_t src = offset > 0 ?
                                    (b + (uint32_t)offset) % rev->bit_count :
                                    (b + rev->bit_count - (uint32_t)abs_off) % rev->bit_count;
                                tmp_conf[b] = rev->confidence[src];
                            }
                            memcpy(rev->confidence, tmp_conf, rev->bit_count);
                            free(tmp_conf);
                        }
                    }
                    free(tmp);
...
char* uft_mrv_to_json(const uft_mrv_result_t* result, bool include_bit_stats) {
    ...
    if (include_bit_stats && result->bit_stats) {
        ...
        for (uint32_t i = 0; i < result->stats_count && (size_t)pos + 100 < size; i++) {
...
#define MULTIREV_SNPRINTF(fmt, ...) do { \
    if (pos >= 0 && pos < buf_size) { \
        int _n = snprintf(summary + pos, (size_t)(buf_size - pos), fmt, ##__VA_ARGS__); \
        if (_n > 0) pos += _n; \
    } \
} while(0)
