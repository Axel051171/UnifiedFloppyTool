...
char *uft_params_to_json(const uft_params_t *params, bool pretty) {
    ...
    for (int i = 0; i < PARAM_COUNT; i++) {
        ...
        if (!first) p += sprintf(p, ",%s", nl);
        first = false;
        
        p += sprintf(p, "%s\"%s\": ", indent, def->json_key);
        
        switch (def->type) {
            case UFT_PARAM_TYPE_BOOL:
                p += sprintf(p, "%s", params->values[i].value.bool_val ? "true" : "false");
                break;
            case UFT_PARAM_TYPE_INT:
            case UFT_PARAM_TYPE_RANGE:
                p += sprintf(p, "%d", params->values[i].value.int_val);
                break;
            case UFT_PARAM_TYPE_FLOAT:
                p += sprintf(p, "%.2f", params->values[i].value.float_val);
                break;
            case UFT_PARAM_TYPE_ENUM:
                if (def->enum_values && params->values[i].value.enum_index < def->enum_count) {
                    p += sprintf(p, "\"%s\"", def->enum_values[params->values[i].value.enum_index]);
                }
                break;
            case UFT_PARAM_TYPE_STRING:
            case UFT_PARAM_TYPE_PATH:
                if (params->values[i].value.string_val) {
                    p += sprintf(p, "\"%s\"", params->values[i].value.string_val);
                } else {
                    p += sprintf(p, "null");
                }
                break;
    ...
    *p = '\0';
    ...
    for (int i = 0; i < PARAM_COUNT; i++) {
        ...
        switch (def->type) {
            ...
            case UFT_PARAM_TYPE_INT:
            case UFT_PARAM_TYPE_RANGE:
                p += sprintf(p, "%s %d ", opt, params->values[i].value.int_val);
                break;
            case UFT_PARAM_TYPE_FLOAT:
                p += sprintf(p, "%s %.2f ", opt, params->values[i].value.float_val);
                break;
            case UFT_PARAM_TYPE_ENUM:
                if (def->enum_values && params->values[i].value.enum_index < def->enum_count) {
                    p += sprintf(p, "%s %s ", opt, 
            ...
            case UFT_PARAM_TYPE_PATH:
                if (params->values[i].value.string_val) {
                    p += sprintf(p, "%s \"%s\" ", opt, params->values[i].value.string_val);
                }
                break;
        }
    }
