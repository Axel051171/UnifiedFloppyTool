...
int uft_ir_disk_save(const uft_ir_disk_t* disk, const char* path,
    ...
    for (uint16_t i = 0; i < disk->track_count; i++) {
        ...
        /* Calculate CRC32 over track flux data */
        {
            uint32_t crc = 0xFFFFFFFF;
            for (int r = 0; r < track->revolution_count; r++) {
                if (track->revolutions[r] && track->revolutions[r]->flux_deltas) {
                    const uint8_t *d = (const uint8_t *)track->revolutions[r]->flux_deltas;
                    uint32_t nbytes = track->revolutions[r]->data_size;
