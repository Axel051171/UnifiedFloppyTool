...
uft_error_t uft_unregister_decoder_plugin(uft_encoding_t encoding) {
    for (size_t i = 0; i < g_decoder_plugin_count; i++) {
        if (g_decoder_plugins[i]->encoding == encoding) {
            ...
            for (size_t j = i; j < g_decoder_plugin_count - 1; j++) {
...
extern const uft_decoder_plugin_t uft_decoder_plugin_fm __attribute__((weak));
extern const uft_decoder_plugin_t uft_decoder_plugin_amiga_mfm __attribute__((weak));
