...
void uft_flux_track_free(uft_flux_track_data_t* track) {
    ...
    if (track->revolutions) {
        // BUGFIX: Also cleanup any pending revolution (revolution_count+1)
        // that was started but never ended
        size_t cleanup_count = track->revolution_count;
        if (cleanup_count < track->revolution_capacity) {
            // Check if there's a pending revolution with allocated transitions
            if (track->revolutions[cleanup_count].transitions != NULL) {
                revolution_cleanup(&track->revolutions[cleanup_count]);
            }
        }
...
uft_error_t uft_flux_track_begin_revolution(uft_flux_track_data_t* track) {
    ...
    uft_error_t err = revolution_init(&track->revolutions[track->revolution_count], 
                                       FLUX_DEFAULT_TRANS_CAPACITY);
    if (err == UFT_OK) {
        // BUGFIX: Track pending revolution to prevent memory leak if end_revolution
        // is never called. Mark revolution as "in progress" via flag.
        track->revolutions[track->revolution_count].flags |= 0x01;  // REV_IN_PROGRESS
    }
    return err;
}
