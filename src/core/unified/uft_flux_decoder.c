...
int uft_flux_process_interval(uft_flux_decoder_t *dec, double interval_ns)
{
    ...
    while (dec->flux_accumulator >= dec->clock * dec->config.pll_phase) {
        double threshold = dec->clock * dec->config.pll_phase;
        
        if (dec->flux_accumulator >= threshold) {
            ...
            /* No pulse yet - output 0 */
            output_bit(dec, 0);
            bits_output++;
            dec->flux_accumulator -= dec->clock;
            dec->clocked_zeros++;
            
            /* Check for sync loss (too many zeros) */
            if (dec->clocked_zeros > 16) {
                if (!dec->sync_lost) {
                    dec->sync_lost = true;
                    dec->sync_losses++;
                }
                           ...
                           uint32_t sync_pattern, int sync_bits)
{
    ...
    /* TODO: Implement sync pattern search */
    return -1;
}
