/**
 * @file uft_hxc_pll_enhanced.c
 * @brief Enhanced PLL algorithms extracted from HxCFloppyEmulator
 * @version 1.0.0
 * @date 2026-01-06
 *
 ...
 *
 * Integration: R50 - HxCFloppyEmulator extraction
 */
...
#include "uft_hxc_pll_enhanced.h"
...
{
    uft_hxc_pll_t* pll = calloc(1, sizeof(uft_hxc_pll_t));
    if (!pll) return NULL;
                                ...
                                uint32_t* outdata)
{
    ...
    memset(outdata, 0, sizeof(uint32_t) * UFT_HXC_HISTOGRAM_SIZE);
    
    for (size_t i = 0; i < size; i++) {
        if (indata[i] < UFT_HXC_HISTOGRAM_SIZE) {
            outdata[indata[i]]++;
        }
    }
}
...
uint32_t uft_hxc_detectpeaks(uft_hxc_pll_t* pll, const uint32_t* histogram)
{
    ...
    for (int i = 0; i < UFT_HXC_HISTOGRAM_SIZE; i++) {
        total += histogram[i];
    ...
    hxc_stathisto_t* stattab = calloc(nbval + 1, sizeof(hxc_stathisto_t));
    if (!stattab) return 0;
...
static int get_band_cells(uft_hxc_pll_t* pll, int current_pulsevalue)
{
    int i = 0;
...
uft_hxc_bitrate_t uft_hxc_detect_bitrate(const uint32_t* flux_data, 
                                          ...
                                          uint32_t tick_freq)
{
    uft_hxc_bitrate_t result = {0};
    
    if (!flux_data || num_samples == 0) return result;
    
    /* Allocate histogram */
    uint32_t* histogram = calloc(UFT_HXC_HISTOGRAM_SIZE, sizeof(uint32_t));
    if (!histogram) return result;
    
    /* Compute histogram */
    uft_hxc_compute_histogram(flux_data, num_samples, histogram);
    ...
    /* Detect bitrate */
    uint32_t cell_ticks = uft_hxc_detectpeaks(&pll, histogram);
    ...
    if (cell_ticks > 0) {
        ...
        if (result.bitrate_hz >= 450000) {
            result.encoding = UFT_HXC_ENC_HD;
        } else if (result.bitrate_hz >= 280000) {
            result.encoding = UFT_HXC_ENC_DD_300K;
        } else {
            result.encoding = UFT_HXC_ENC_DD_250K;
        }
    }
