...
int uft_pll_process_to_byte(uft_pll_t *pll,
    ...
    *byte_ready = false;
    ...
    if (pll->bits_pending >= 8) {
        ...
        /* Track clock pattern: in MFM, clock bits are at even positions.
         * Extract clock bits from the last 16 raw bits (8 data + 8 clock) */
        {
            uint8_t clock = 0;
            uint32_t raw = pll->shift_reg >> 8;  /* Previous 16 bits */
            for (int b = 0; b < 8; b++) {
                /* Clock bits are at even positions in the raw MFM stream */
                if (raw & (1u << (b * 2 + 1))) {
                    clock |= (1 << b);
                }
            }
            byte_out->clock_pattern = clock;
        }
