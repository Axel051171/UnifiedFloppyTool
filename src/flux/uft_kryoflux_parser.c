        ...
        if (op <= 0x07) {
            ...
            if (oob_type == UFT_UFT_KF_OOB_INDEX && oob_size >= 12) {
                ...
                if (ctx->index_count < UFT_UFT_KF_MAX_REVOLUTIONS + 1) {
                    ...
                    if (data_offset + 12 <= ctx->stream_size) {
                        ...
                        ctx->indices[ctx->index_count].stream_pos = 
                            p[0] | (p[1] << 8) | ((uint32_t)p[2] << 16) | ((uint32_t)p[3] << 24);
                        ctx->indices[ctx->index_count].sample_counter = 
                            p[4] | (p[5] << 8) | ((uint32_t)p[6] << 16) | ((uint32_t)p[7] << 24);
                        ctx->indices[ctx->index_count].index_counter = 
                            p[8] | (p[9] << 8) | ((uint32_t)p[10] << 16) | ((uint32_t)p[11] << 24);
                        ctx->index_count++;
                    }
                }
                ...
                if (data_offset + 8 <= ctx->stream_size) {
                    uint8_t* p = &ctx->stream_data[data_offset];
                    ctx->stream_info_pos = p[0] | (p[1] << 8) | ((uint32_t)p[2] << 16) | ((uint32_t)p[3] << 24);
                    ctx->xfer_time = p[4] | (p[5] << 8) | ((uint32_t)p[6] << 16) | ((uint32_t)p[7] << 24);
                }
