...
## UFT Integration: VFO/PLL Status

These are experimental VFO (Variable Frequency Oscillator) implementations
from the fdc_bitstream library. The production PLL for UnifiedFloppyTool is:

- `src/decoder/uft_pll_v2.c` (primary, with SIMD dispatch)
- `src/flux/pll/uft_pll_pi.c` (PI controller variant)

The `vfo_*` files in this directory are experimental alternatives:

| File | Description |
|---|---|
| `vfo_simple.cpp` | Basic VFO implementation |
| `vfo_simple2.cpp` | Improved basic VFO |
...
These files are compiled into the build (via `UnifiedFloppyTool.pro`) but are
not used in the primary decode path. They are retained for research, testing,
and potential future use with the fdc_bitstream MFM codec.
