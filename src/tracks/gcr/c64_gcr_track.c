...
// C64 GCR encoding
int get_next_C64_sector(LIBFLUX_CTX* flux_ctx,LIBFLUX_SIDE * track,LIBFLUX_SECTCFG * sector,int track_offset)
{
		...
		switch(sector_extractor_sm)
		{
			...
			case LOOKFOR_ADDM:
				...
				if( !CRC16_Low )
				{
					...
					sector->use_alternate_header_crc = 0x00;
					flux_ctx->libflux_printf(MSG_DEBUG,"Valid C64 GCR sector header found - Sect:%d",tmp_buffer[2]);
					old_bit_offset = bit_offset;
