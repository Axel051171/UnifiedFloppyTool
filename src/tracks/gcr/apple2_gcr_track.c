...
int get_next_A2GCR2_sector(LIBFLUX_CTX* flux_ctx,LIBFLUX_SIDE * track,LIBFLUX_SECTCFG * sector,int track_offset)
{
	...
	memset(sector,0,sizeof(LIBFLUX_SECTCFG));
	...
	do
	{
		switch(sector_extractor_sm)
		{
			...
			case LOOKFOR_ADDM:
				...
				flux_ctx->libflux_printf(MSG_DEBUG,test_buffer);
				#endif
				...
				if(1)
				{
					...
					#ifdef DBG_A2_GCR
					flux_ctx->libflux_printf(MSG_DEBUG,"DBG %.2X %.2X %.2X %.2X %.2X %.2X %.2X",tmp_buffer[0],tmp_buffer[1],tmp_buffer[2],tmp_buffer[3],tmp_buffer[4],tmp_buffer[5],tmp_buffer[6]);
					#endif
					...
					{ // crc ok !!!
						...
						if(bit_offset!=-1)
						{
							bit_offset = chgbitptr( track->tracklen, bit_offset, ( 6 * 8 ));
							sector->use_alternate_header_crc = 0x00;
							flux_ctx->libflux_printf(MSG_DEBUG,"Valid Apple II 6 and 2 GCR sector header found - Sect:%d",tmp_buffer[2]);
							old_bit_offset = bit_offset;
							...
							if( bit_offset!=-1 )
							{
								...
								if(sector->input_data)
								{
									...
									{ // crc ok !!!
										...
										#ifdef DBG_A2_GCR
										flux_ctx->libflux_printf(MSG_DEBUG,"DBG2 %d",i);
										i = 0;
										memset(test_buffer,0,sizeof(test_buffer));
										strncpy(test_buffer, "TST: ", sizeof(test_buffer)-1); test_buffer[sizeof(test_buffer)-1] = '\0';
										do
										{
											snprintf(test_buffer2, sizeof(test_buffer2),"%d",getbit(track->databuffer,( (jj + i) % track->tracklen)));
											strncat(test_buffer, test_buffer2, sizeof(test_buffer) - strlen(test_buffer) - 1);
											i++;
										}while(i<1024);
										flux_ctx->libflux_printf(MSG_DEBUG,test_buffer);
										...
										i = 0;
										memset(test_buffer,0,sizeof(test_buffer));
										strncpy(test_buffer, "TST2: ", sizeof(test_buffer)-1); test_buffer[sizeof(test_buffer)-1] = '\0';
										do
										{
											snprintf(test_buffer2, sizeof(test_buffer2),"%d",getbit(track->databuffer,( (bit_offset + i) % track->tracklen)));
											strncat(test_buffer, test_buffer2, sizeof(test_buffer) - strlen(test_buffer) - 1);
											i++;
										}while(i<1024);
										flux_ctx->libflux_printf(MSG_DEBUG,test_buffer);
										...
										flux_ctx->libflux_printf(MSG_DEBUG,"crc data ok.");
										sector->use_alternate_data_crc = 0x00;
									}
									else
									{
										flux_ctx->libflux_printf(MSG_DEBUG,"crc data error!");
							...
							else
							{
								...
								flux_ctx->libflux_printf(MSG_DEBUG,"No data!");
								sector_extractor_sm=ENDOFSECTOR;
							}
						}
						else
						{
							...
							flux_ctx->libflux_printf(MSG_DEBUG,"No data! 2");
							sector_extractor_sm=ENDOFSECTOR;
							sector->use_alternate_data_crc = 0xFF;
						}
					}
...
/* Apple 5-and-3 GCR encoding (DOS 3.2 and earlier, 13 sectors/track) */
int get_next_A2GCR1_sector(LIBFLUX_CTX* flux_ctx,LIBFLUX_SIDE * track,LIBFLUX_SECTCFG * sector,int track_offset)
{
	return -1;
}

void tg_addAppleSectorToTrack(track_generator *tg,LIBFLUX_SECTCFG * sectorconfig,LIBFLUX_SIDE * currentside)
{
	int32_t         trackenc;
