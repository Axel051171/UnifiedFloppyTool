 ...
 * @brief Membrain MFM track handler
 * @version 3.8.0
 */
/*
//
...
int get_next_MEMBRAIN_sector(LIBFLUX_CTX* flux_ctx,LIBFLUX_SIDE * track,LIBFLUX_SECTCFG * sector,int track_offset)
{
		...
		switch(sector_extractor_sm)
		{
			...
			case LOOKFOR_ADDM:
				...
				if(tmp_buffer[1]==0xFE)
				{
					...
					{ // crc ok !!!
						flux_ctx->libflux_printf(MSG_DEBUG,"Valid MEMBRAIN MFM sector header found - Cyl:%d Side:%d Sect:%d Size:%d",tmp_buffer[4],tmp_buffer[5],tmp_buffer[6],sectorsize[tmp_buffer[7]&0x7]);
						sector->use_alternate_header_crc = 0;
					}
					else
					{
						flux_ctx->libflux_printf(MSG_DEBUG,"Bad MEMBRAIN MFM sector header found - Cyl:%d Side:%d Sect:%d Size:%d",tmp_buffer[4],tmp_buffer[5],tmp_buffer[6],sectorsize[tmp_buffer[7]&0x7]);
					...
					if((bit_offset!=-1))
					{
						...
						if( tmp_sector )
						{
							...
							if(!CRC16_High && !CRC16_Low)
							{ // crc ok !!!
								flux_ctx->libflux_printf(MSG_DEBUG,"Data CRC Ok. (0x%.4X)",sector->data_crc);
							}
							else
							{
								flux_ctx->libflux_printf(MSG_DEBUG,"Data CRC ERROR ! (0x%.4X)",sector->data_crc);
						...
						{
							flux_ctx->libflux_printf(MSG_ERROR,"get_next_MEMBRAIN_sector : Alloc error!");
						}
				...
				else
				{
					if(tmp_buffer[3]==0xF8 || tmp_buffer[3]==0xF9 || tmp_buffer[3]==0xFA || tmp_buffer[3]==0xFB)
					{
						...
						flux_ctx->libflux_printf(MSG_DEBUG,"get_next_MEMBRAIN_sector : Data sector without sector header !?!");
