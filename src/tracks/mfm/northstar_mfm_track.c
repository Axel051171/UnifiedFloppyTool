...
int get_next_MFM_Northstar_sector(LIBFLUX_CTX* flux_ctx,LIBFLUX_SIDE * track,LIBFLUX_SECTCFG * sector,int track_offset)
{
		...
		switch(sector_extractor_sm)
		{
			...
			case LOOKFOR_ADDM:
				...
				if( tmp_buffer[0] == 0xFB )
				{
					...
					{ // checksum ok !!!
						flux_ctx->libflux_printf(MSG_DEBUG,"Valid MFM Northstar sector found - Cyl:%d Side:%d Sect:%d Size:%d",tmp_buffer[4],tmp_buffer[5],tmp_buffer[6],sectorsize[tmp_buffer[7]&0x7]);
					}
					else
					{
						flux_ctx->libflux_printf(MSG_DEBUG,"Bad MFM Northstar header found - Cyl:%d Side:%d Sect:%d Size:%d",tmp_buffer[4],tmp_buffer[5],tmp_buffer[6],sectorsize[tmp_buffer[7]&0x7]);
...
void tg_addNorthstarSectorToTrack(track_generator *tg,LIBFLUX_SECTCFG * sectorconfig,LIBFLUX_SIDE * currentside)
{
