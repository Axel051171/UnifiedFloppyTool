		...
		switch(sector_extractor_sm)
		{
			...
			case LOOKFOR_ADDM:
				...
				if(tmp_buffer[3]==0xFE)
				{
					...
					{ // crc ok !!!
						flux_ctx->libflux_printf(MSG_DEBUG,"Valid MFM sector header found - Cyl:%d Side:%d Sect:%d Size:%d",tmp_buffer[4],tmp_buffer[5],tmp_buffer[6],sector->sectorsize);
						sector->use_alternate_header_crc = 0;
					}
					else
					{
						flux_ctx->libflux_printf(MSG_DEBUG,"Bad MFM sector header found - Cyl:%d Side:%d Sect:%d Size:%d",tmp_buffer[4],tmp_buffer[5],tmp_buffer[6],sector->sectorsize);
					}
