...
int get_next_Centurion_MFM_sector(LIBFLUX_CTX* flux_ctx,LIBFLUX_SIDE * track,LIBFLUX_SECTCFG * sector,int track_offset)
{
	int bit_offset;
	int tmp_bit_offset;
	...
	do
	{
		...
		{
			case LOOKFOR_GAP1:
				flux_ctx->libflux_printf(MSG_DEBUG, "Looking for Centurion sector mark starting at %d\n", bit_offset);
				...
				if(bit_offset!=-1)
				{
					flux_ctx->libflux_printf(MSG_DEBUG, "Found Centurion MFM sector mark at offset %d\n", bit_offset);
			...
			case LOOKFOR_ADDM:
				...
				{ // crc ok !!!
					flux_ctx->libflux_printf(MSG_DEBUG, "Valid Centurion MFM sector header found - Cyl: %d Sect:%d\n", sector->cylinder, sector->sector);
					sector->use_alternate_header_crc = 0;
				}
				else
				{
					flux_ctx->libflux_printf(MSG_DEBUG, "Bad Centurion MFM sector header found - Cyl: %d, Sect:%d\n", sector->cylinder, sector->sector);
				}
				...
				if(tmp_bit_offset != -1)
				{
					...
					if (sector_key == 0) {
						...
						if( sector->input_data && sector->input_data_index)
						{
							...
							if(!CRC16_High && !CRC16_Low)
							{ // crc ok !!!
								flux_ctx->libflux_printf(MSG_DEBUG, "Valid Centurion MFM data found - Cyl: %d Sect:%d\n", sector->cylinder, sector->sector);
								sector->use_alternate_data_crc = 0;
							}
							else
							{
								flux_ctx->libflux_printf(MSG_DEBUG, "Bad Centurion MFM data found - Cyl: %d, Sect:%d\n", sector->cylinder, sector->sector);
							}
						}
						else
						{
							...
							flux_ctx->libflux_printf(MSG_ERROR, "get_next_Centurion_MFM_sector : Allocation error !\n");
							return -1;
						}
					} else {
						flux_ctx->libflux_printf(MSG_DEBUG, "Don't know how to decode Centurion MFM sector key %d", sector_key);
					}
