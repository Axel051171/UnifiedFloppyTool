...
int get_next_AMIGAMFM_sector(LIBFLUX_CTX* flux_ctx,LIBFLUX_SIDE * track,LIBFLUX_SECTCFG * sector_conf,int track_offset)
{
	int bit_offset;
	...
	memset(sector_conf,0,sizeof(LIBFLUX_SECTCFG));
		...
		switch(sector_extractor_sm)
		{
			...
			case LOOKFOR_ADDM:
				...
				if( (header[0]==0xFF) && ( (headerparity[0] == sector_data[26]) && (headerparity[1] == sector_data[27]) ) )
				{
					...
					flux_ctx->libflux_printf(MSG_DEBUG,"Valid Amiga MFM sector header found - Cyl:%d Side:%d Sect:%d LeftSect:%d Size:%d",header[1]>>1,header[1]&1,header[2],header[3],sector_size);
					...
					{
						// parity ok !!!
						flux_ctx->libflux_printf(MSG_DEBUG,"data parity ok.");
						sector_conf->use_alternate_data_crc = 0x00;

					}
					else
					{
						flux_ctx->libflux_printf(MSG_DEBUG,"data parity error!");
...
int write_AMIGAMFM_sectordata(LIBFLUX_CTX* flux_ctx,LIBFLUX_SIDE * track,LIBFLUX_SECTCFG * sector,unsigned char * buffer,int buffersize)
{
	int bit_offset;
	unsigned char sectorparity[2];
	int i,l;
...
void tg_addAmigaSectorToTrack(track_generator *tg,LIBFLUX_SECTCFG * sectorconfig,LIBFLUX_SIDE * currentside)
{
