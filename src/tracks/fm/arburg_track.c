		...
		switch(sector_extractor_sm)
		{
			...
			case LOOKFOR_ADDM:
				...
				if(sector->input_data)
				{
					...
					{
						flux_ctx->libflux_printf(MSG_DEBUG,"get_next_Arburg_sector : Checksum data ok.");
						sector->use_alternate_data_crc = 0x00;
					...
					{
						flux_ctx->libflux_printf(MSG_DEBUG,"get_next_Arburg_sector : Checksum data Error !");
		...
		switch(sector_extractor_sm)
		{
			...
			case LOOKFOR_ADDM:
				...
				if(sector->input_data)
				{
					...
					{
						flux_ctx->libflux_printf(MSG_DEBUG,"Checksum data ok.");
						sector->use_alternate_data_crc = 0x00;
					...
					{
						flux_ctx->libflux_printf(MSG_DEBUG,"Checksum data Error !");
						sector->use_alternate_data_crc = 0xFF;
						sector->data_crc = sector->input_data[0xEFE];
						sector->data_crc = sector->data_crc | (sector->input_data[0xEFF]<<8);
					}
	...
	*/
	...
	if(finalsize<=current_buffer_size)
	{
		...
		if(tempdata && tempclock)
		{
			...
			{
				flux_ctx->libflux_printf(MSG_WARNING,"BuildArburgTrack : Block Checksum error !");
			}
	...
	{
		flux_ctx->libflux_printf(MSG_ERROR,"BuildArburgTrack : No enough space on this track !");
		return finalsize;
	}
}
...
int32_t BuildArburgSysTrack(LIBFLUX_CTX* flux_ctx,int32_t tracknumber,int32_t sidenumber,uint8_t* datain, uint8_t * fmdata, int32_t * fmsizebuffer,int32_t trackformat)
{
	...
	*/
	...
	{
		flux_ctx->libflux_printf(MSG_WARNING,"BuildArburgSysTrack : Block Checksum error !");
	}
