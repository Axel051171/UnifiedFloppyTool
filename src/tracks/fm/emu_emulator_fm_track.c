			...
			case LOOKFOR_ADDM:
				...
				if((LUT_ByteBitsInverter[tmp_buffer[0]]==0xFA) && (LUT_ByteBitsInverter[tmp_buffer[1]]==0x96))
				{
					...
					{ // crc ok !!!
						...
						flux_ctx->libflux_printf(MSG_DEBUG,"Valid EmuII FM sector header found - Sect:%d",LUT_ByteBitsInverter[tmp_buffer[2]]);
...
int32_t BuildEmuIITrack(LIBFLUX_CTX* flux_ctx,int tracknumber,int sidenumber,unsigned char* datain,unsigned char * fmdata,int32_t * fmsizebuffer,int trackformat)
{
	...
	switch(trackformat)
	{
		...
		break;
		default:
			flux_ctx->libflux_printf(MSG_ERROR,"BuildEmuIITrack : Bad track format !");
			return finalsize;
		break;
	}
	...
	{
		flux_ctx->libflux_printf(MSG_ERROR,"BuildEmuIITrack : No enough space on this track !");
		return finalsize;
	}
