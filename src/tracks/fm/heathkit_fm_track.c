...
int get_next_FM_Heathkit_sector(LIBFLUX_CTX* flux_ctx,LIBFLUX_SIDE * track,LIBFLUX_SECTCFG * sector,int track_offset)
{
		...
		switch(sector_extractor_sm)
		{
			...
			case LOOKFOR_ADDM:
				...
				if( tmp_buffer[0] == LUT_ByteBitsInverter[0xFD] )
				{
					...
					if( checksum == LUT_ByteBitsInverter[tmp_buffer[ 4 ]] )
					{ // checksum ok !!!
						flux_ctx->libflux_printf(MSG_DEBUG,"Valid FM Heathkit sector found - Cyl:%d Side:%d Sect:%d Size:%d",LUT_ByteBitsInverter[ tmp_buffer[2] ],tmp_buffer[5],LUT_ByteBitsInverter[ tmp_buffer[3] ],sectorsize[tmp_buffer[7]&0x7]);
					}
					else
					{
						flux_ctx->libflux_printf(MSG_DEBUG,"Bad FM Heathkit header found - Cyl:%d Side:%d Sect:%d Size:%d",LUT_ByteBitsInverter[ tmp_buffer[2] ],tmp_buffer[5],LUT_ByteBitsInverter[ tmp_buffer[3] ],sectorsize[tmp_buffer[7]&0x7]);
...
void tg_addHeathkitSectorToTrack(track_generator *tg,LIBFLUX_SECTCFG * sectorconfig,LIBFLUX_SIDE * currentside)
{
	int32_t  i;
	int32_t  trackenc;
