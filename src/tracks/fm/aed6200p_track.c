...
int get_next_AED6200P_sector(LIBFLUX_CTX* flux_ctx,LIBFLUX_SIDE * track,LIBFLUX_SECTCFG * sector,int track_offset)
{
		...
		switch(sector_extractor_sm)
		{
			...
			case LOOKFOR_ADDM:
				...
				if(1)
				{
					...
					{ // crc ok !!!
						flux_ctx->libflux_printf(MSG_DEBUG,"Valid AED6200P sector header found - Cyl:%d Side:%d Sect:%d Size:%d",tmp_buffer[4],tmp_buffer[5],tmp_buffer[6],sectorsize[tmp_buffer[7]&0x7]);
						sector->use_alternate_header_crc = 0;
					}
					else
					{
						flux_ctx->libflux_printf(MSG_DEBUG,"Bad AED6200P sector header found - Cyl:%d Side:%d Sect:%d Size:%d",tmp_buffer[4],tmp_buffer[5],tmp_buffer[6],sectorsize[tmp_buffer[7]&0x7]);
					}
				...
				else
				{
					if( ( (tmp_buffer[3]&0xFC)==0xF8) && (bit_offset > (88 * 8)) )
					{
						...
						flux_ctx->libflux_printf(MSG_DEBUG,"get_next_MFM_sector : Data sector without sector header !?!");
