...
int get_next_QDMO5_sector(LIBFLUX_CTX* flux_ctx,LIBFLUX_SIDE * track,LIBFLUX_SECTCFG * sector,int track_offset)
{
	...
	do
	{
		switch(sector_extractor_sm)
		{
			...
			case LOOKFOR_ADDM:
				...
				flux_ctx->libflux_printf(MSG_DEBUG,"Valid QD MO5 sector header found - Cyl:%d Side:%d Sect:%d Size:%d",tmp_buffer[4],tmp_buffer[5],tmp_buffer[6],sectorsize[tmp_buffer[7]&0x7]);
				...
				if((bit_offset!=-1))
				{
					...
					{ // crc ok !!!
						flux_ctx->libflux_printf(MSG_DEBUG,"Data checksum Ok. (0x%.2X)",sector->data_crc);
					}
					else
					{
						flux_ctx->libflux_printf(MSG_DEBUG,"Data checksum ERROR ! (0x%.2X)",sector->data_crc);
