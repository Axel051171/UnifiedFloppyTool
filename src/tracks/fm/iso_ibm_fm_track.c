...
int get_next_FM_sector(LIBFLUX_CTX* flux_ctx,LIBFLUX_SIDE * track,LIBFLUX_SECTCFG * sector,int track_offset)
{
		...
		switch(sector_extractor_sm)
		{
			...
			case LOOKFOR_ADDM:
				...
				if(tmp_buffer[0]==0xFE)
				{
					...
					mask = libflux_getEnvVarValue( flux_ctx, "SECTOR_SIZE_FIELD_MASK" );
					if( mask <= 0 )
					{
						mask = 0x7;
					}
					...
					{ // crc ok !!!
						...
						flux_ctx->libflux_printf(MSG_DEBUG,"Valid FM sector header found - Cyl:%d Side:%d Sect:%d Size:%d",tmp_buffer[1],tmp_buffer[2],tmp_buffer[3],sectorsize[tmp_buffer[4] & mask]);
...
int write_FM_sectordata(LIBFLUX_CTX* flux_ctx,LIBFLUX_SIDE * track,LIBFLUX_SECTCFG * sector,unsigned char * buffer,int buffersize)
{
	int bit_offset,i;
	unsigned char CRC16_High;
	unsigned char CRC16_Low;
