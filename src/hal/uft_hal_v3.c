...
int uft_hal_discover(uft_hal_discover_cb callback, void* user_data) {
    ...
    /* Additional controller discovery: FluxEngine, KryoFlux, SuperCard Pro */
    uft_fe_discover(uft_gw_discover_callback, &ctx);
...
int uft_hal_open_first(uft_hal_device_t** device) {
    ...
    ret = uft_hal_open(UFT_HAL_FLUXENGINE, NULL, device);
    if (ret == UFT_HAL_OK) return ret;
    
    ret = uft_hal_open(UFT_HAL_KRYOFLUX, NULL, device);
    if (ret == UFT_HAL_OK) return ret;
    
    ret = uft_hal_open(UFT_HAL_SUPERCARD, NULL, device);
    if (ret == UFT_HAL_OK) return ret;
