...
uft_kf_config_t* uft_kf_config_from_params(const void *params) {
    ...
    /* 
     * Read configuration from UFT parameter system.
     * Maps parameter keys to kf_config fields:
     * - "kryoflux.dtc_path"    -> cfg.dtc_path
     * - "kryoflux.device"      -> cfg.device_index
     * - "kryoflux.start_track" -> cfg.start_track
     * - "kryoflux.end_track"   -> cfg.end_track
     * - "kryoflux.side"        -> cfg.side
     * - "kryoflux.revolutions" -> cfg.revolutions
     * - "kryoflux.double_step" -> cfg.double_step
     * - "kryoflux.retry_count" -> cfg.retry_count
     * - "kryoflux.platform"    -> cfg.platform
     * Parameter system integration happens at link time. */
...
int uft_kf_config_to_params(const uft_kf_config_t *cfg, void *params) {
    ...
    /*
     * Write configuration back to UFT parameter system.
     * Inverse of config_from_params; serializes kf_config to params. */
...
int uft_kf_write_disk(uft_kf_config_t* config, const char* input_dir,
    ...
    for (int track = config->start_track; track <= config->end_track; track++) {
        ...
        for (int side = side_start; side <= side_end; side++) {
            ...
            size_t bytes_read = fread(raw, 1, size, f);
            fclose(f);
