    ...
    *count = out_pos;
    ...
    if (out_pos == 0) {
        snprintf(hal->error, sizeof(hal->error),
                 "No flux data received — check hardware connection");
        return -1;
    }
...
static int gw_write_flux(uft_hal_t *hal, int track, int side,
    ...
    /* Encode flux ns → GW variable-length wire format
     * 0x01-0xF9: 1-byte delta (ticks)
     * 0xFA-0xFD: 2-byte delta (high nibble | low byte)
     * 0xFE:      Space (+0x100 ticks)
     * 0x00:      End-of-stream */
    size_t wire_size = count * 3 + 16;
    ...
    if (gw_command(hal, GW_CMD_WRITE_FLUX, write_params, 5, NULL, 0) != 0) {
        free(wire);
        snprintf(hal->error, sizeof(hal->error), "Write flux command failed");
        return -1;
    }
    
    /* Stream encoded flux data */
    if (serial_write(hal->serial, wire, pos) != (int)pos) {
        free(wire);
        snprintf(hal->error, sizeof(hal->error), "Flux data stream failed");
        return -1;
    }
    
    free(wire);
    
    /* Read flux status to verify completion */
    uint8_t status_cmd[2] = {0x09, 2};  /* GET_FLUX_STATUS */
    if (serial_write(hal->serial, status_cmd, 2) < 0) {
        return -1;
    }
    uint8_t status_ack[2];
    if (serial_read(hal->serial, status_ack, 2) == 2) {
        if (status_ack[1] == GW_ACK_FLUX_UNDERFLOW) {
            snprintf(hal->error, sizeof(hal->error), "Flux underflow during write");
            return -1;
        }
        if (status_ack[1] == GW_ACK_WRPROT) {
            snprintf(hal->error, sizeof(hal->error), "Disk is write-protected");
            return -1;
        }
    }
...
static int gw_enumerate(uft_hal_controller_t *out, int max) {
...
#ifdef UFT_OS_LINUX
    /* Scan /dev/ttyACM* for Greaseweazle CDC-ACM devices */
    DIR *dir = opendir("/dev");
    if (!dir) return 0;
    ...
    while ((entry = readdir(dir)) != NULL && found < max) {
        if (strncmp(entry->d_name, "ttyACM", 6) == 0) {
            /* Try to probe: open, send GET_INFO, check response */
            char path[128];
            snprintf(path, sizeof(path), "/dev/%s", entry->d_name);
            
            int fd = serial_open(path, 115200);
            if (fd >= 0) {
                /* Quick probe: send GET_INFO [0x00, 0x04, 0x00, 0x00] */
                uint8_t probe[4] = {0x00, 0x04, 0x00, 0x00};
                uint8_t ack[2] = {0xFF, 0xFF};
                
                if (serial_write(fd, probe, 4) == 4 &&
                    serial_read(fd, ack, 2) == 2 &&
                    ack[0] == 0x00 && ack[1] == GW_ACK_OK) {
                    out[found++] = HAL_CTRL_GREASEWEAZLE;
                }
                serial_close(fd);
            }
        }
...
static int fe_enumerate(uft_hal_controller_t *out, int max) {
    /* FluxEngine also appears as /dev/ttyACM* (VID:PID varies by board).
     * Uses same protocol as Greaseweazle — probe is identical but
     * firmware version response differentiates. For now, rely on 
     * GW enumerate; users select FluxEngine via explicit path. */
    (void)out;
    (void)max;
    return 0;  /* FluxEngine detected via user config, not auto-enumerate */
}
...
static int scp_enumerate(uft_hal_controller_t *out, int max) {
...
#if defined(UFT_OS_LINUX) && defined(HAVE_LIBUSB)
    /* SCP uses libusb (VID:PID = 04D8:FBBD Microchip/SuperCard Pro). */
    ...
    for (ssize_t i = 0; i < cnt && found < max; i++) {
        ...
        if (libusb_get_device_descriptor(devlist[i], &desc) == 0) {
            if (desc.idVendor == 0x04D8 && desc.idProduct == 0xFBBD) {
                out[found++] = HAL_CTRL_SCP;
            }
        }
    }
