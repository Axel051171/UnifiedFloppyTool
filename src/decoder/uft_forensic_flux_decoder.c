    ...
    uft_track_decode_result_t *result)
{
    ...
    if (encoding != UFT_ENC_MFM && encoding != UFT_ENC_AMIGA_MFM) {
...
const char *uft_sector_status_str(uint32_t status) {
    ...
    if (status & UFT_SECTOR_ID_CRC_ERR) strcat(buf, "ID_CRC_ERR ");
    if (status & UFT_SECTOR_DATA_CRC_ERR) strcat(buf, "DATA_CRC_ERR ");
    if (status & UFT_SECTOR_DELETED) strcat(buf, "DELETED ");
    if (status & UFT_SECTOR_WEAK_BITS) strcat(buf, "WEAK_BITS ");
    if (status & UFT_SECTOR_CORRECTED) strcat(buf, "CORRECTED ");
    if (status & UFT_SECTOR_DUBIOUS) strcat(buf, "DUBIOUS ");
    if (status & UFT_SECTOR_RECOVERED) strcat(buf, "RECOVERED ");
