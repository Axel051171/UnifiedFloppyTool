...
int32_t pushTrackCode(track_generator *tg,uint8_t data,uint8_t clock,LIBFLUX_SIDE * side,int32_t trackencoding)
{
	int i;
...
void FastMFMFMgenerator(track_generator *tg,LIBFLUX_SIDE * side,unsigned char * track_data,int size,int trackencoding)
{
	switch(trackencoding)
	{
		case IBMFORMAT_SD:
...
int searchgap3value(unsigned int numberofsector,LIBFLUX_SECTCFG * sectorconfigtab)
{
	unsigned int i,j;
	unsigned char gap3;
...
int32_t tg_computeMinTrackSize(track_generator *tg,int32_t trackencoding,int32_t bitrate,int32_t numberofsector,LIBFLUX_SECTCFG * sectorconfigtab,int32_t pregaplen,int32_t * track_period)
{
	int32_t j;
	int32_t tck_period;
...
LIBFLUX_SIDE * tg_initTrack(track_generator *tg,int32_t tracksize,int32_t numberofsector,int32_t trackencoding,int32_t bitrate,LIBFLUX_SECTCFG * sectorconfigtab,int32_t pregap)
{
	...
	currentside = (LIBFLUX_SIDE*)malloc(sizeof(LIBFLUX_SIDE));
	if ( !currentside )
		return NULL;
...
void tg_addISOSectorToTrack(track_generator *tg,LIBFLUX_SECTCFG * sectorconfig,LIBFLUX_SIDE * currentside)
{
	int32_t  i,j,k;
	uint8_t  c,trackenc;
	uint8_t  CRC16_High;
...
void tg_addSectorToTrack(track_generator *tg,LIBFLUX_SECTCFG * sectorconfig,LIBFLUX_SIDE * currentside)
{
...
void tg_completeTrack(track_generator *tg, LIBFLUX_SIDE * currentside,int32_t trackencoding)
{
...
LIBFLUX_SIDE * tg_generateTrackEx(int32_t number_of_sector,LIBFLUX_SECTCFG * sectorconfigtab,int32_t interleave,int32_t skew,int32_t bitrate,int32_t rpm,int32_t trackencoding,int32_t pregap,int32_t indexlen,int32_t indexpos)
{
	int32_t i;
	int32_t tracksize;
	...
	if(gap3tocompute && (wanted_trackperiod>(track_period+indexperiod)))
	{
		...
		for(i=0;i<number_of_sector;i++)
		{
			...
			if(sectorconfigtab[i].gap3==0xFF)
			{
				/* Integer approximation of gap3 = gap3period * bitrate / 100000 */
				computedgap3=(uint32_t)((uint64_t)gap3period * (uint64_t)sectorconfigtab[i].bitrate / 100000ULL);
				...
				//flux_ctx->libflux_printf(MSG_DEBUG,"Sector:%d Computed Gap:%d",sectorconfigtab[i].sector, computedgap3);
			}
		}
	}
...
LIBFLUX_SIDE * tg_generateTrack(unsigned char * sectors_data,int32_t sector_size,int32_t number_of_sector,int32_t track,int32_t side,int32_t sectorid,int32_t interleave,int32_t skew,int32_t bitrate,int32_t rpm,int32_t trackencoding,int32_t gap3,int32_t pregap, int32_t indexlen,int32_t indexpos)
{
	unsigned short i;
	LIBFLUX_SIDE * currentside;
	LIBFLUX_SECTCFG * sectorconfigtab;

	sectorconfigtab = malloc(sizeof(LIBFLUX_SECTCFG)*number_of_sector);
	if(!sectorconfigtab)
		return NULL;

	memset(sectorconfigtab,0,sizeof(LIBFLUX_SECTCFG)*number_of_sector);
...
LIBFLUX_SIDE * tg_alloctrack(int32_t bitrate,int32_t trackencoding,int32_t rpm,int32_t tracksize,int32_t indexlen,int32_t indexpos,int32_t buffertoalloc)
{
	LIBFLUX_SIDE * currentside;
