...
bool uft_partial_fuse(uft_partial_sector_t *sector) {
    ...
    /* Recalculate CRC-CCITT on fused data */
    if (sector->data_len > 0 && sector->revision_count > 0) {
        uint16_t crc = 0xFFFF;
        /* CRC covers sync bytes + DAM + data */
        uint8_t prefix[4] = {0xA1, 0xA1, 0xA1, sector->revisions[0].dam};
        for (int b = 0; b < 4; b++) {
        ...
        for (size_t i = 0; i < sector->data_len; i++) {
            crc ^= (uint16_t)sector->data[i] << 8;
            for (int bit = 0; bit < 8; bit++)
                crc = (crc << 1) ^ ((crc & 0x8000) ? 0x1021 : 0);
        }
        sector->data_crc_valid = (crc == sector->revisions[0].crc_stored);
    }
...
bool uft_partial_fix_crc(uft_partial_sector_t *sector, int max_bits) {
    ...
    for (size_t byte_idx = 0; byte_idx < len && max_bits >= 1; byte_idx++) {
        for (int bit = 0; bit < 8; bit++) {
            ...
            /* Recalculate CRC-CCITT and check */
            uint16_t crc = 0xFFFF;
            uint8_t prefix[4] = {0xA1, 0xA1, 0xA1,
                                  sector->revision_count > 0 ? sector->revisions[0].dam : 0xFB};
            for (int b = 0; b < 4; b++) {
                crc ^= (uint16_t)prefix[b] << 8;
            ...
            uint16_t stored = sector->revision_count > 0 ? sector->revisions[0].crc_stored : 0;
            if (crc == stored) {
                /* CRC matches — this bit flip fixed it */
                sector->data_crc_valid = true;
                if (sector->byte_status) {
                    sector->byte_status[byte_idx].is_error = false;
                    sector->byte_status[byte_idx].confidence = 200;
                }
                return true;
            }
