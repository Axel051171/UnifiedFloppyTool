...
bool uft_crc_correct(uint8_t* data, size_t size, int crc_type,
    ...
    for (size_t byte = 0; byte + 2 < size; byte++) {
...
int uft_fuzzy_sync_find(const uint8_t* bits, size_t bit_count,
    ...
    if (bit_count < (size_t)pattern_bits) return 0;
    
    int match_count = 0;
    (void)((pattern_bits + 7) / 8); // pattern_bytes for potential future use
    
    for (size_t pos = 0; pos <= bit_count - (size_t)pattern_bits && match_count < max_matches; pos++) {
        int mismatches = 0;
