...
void* uft_pool_alloc(uft_pool_t *pool)
{
    ...
    if (!pool->free_list) {
        /* Allocate new chunk of objects and add to free list */
        size_t chunk_count = pool->chunk_size > 0 ? pool->chunk_size : 64;
        size_t alloc_size = chunk_count * (sizeof(uft_pool_block_t) + pool->object_size);
        uint8_t *chunk = (uint8_t *)malloc(alloc_size);
        
        if (!chunk) {
            pthread_mutex_unlock(&pool->mutex);
            fprintf(stderr, "UFT WARNING: Pool expansion failed, falling back to malloc\n");
            return malloc(pool->object_size);
        }
        
        /* Chain new blocks into free list */
        size_t block_stride = sizeof(uft_pool_block_t) + pool->object_size;
        for (size_t i = 0; i < chunk_count; i++) {
            uft_pool_block_t *blk = (uft_pool_block_t *)(chunk + i * block_stride);
            blk->next = pool->free_list;
            pool->free_list = blk;
            pool->total_free++;
        }
        pool->total_allocated += chunk_count;
    }
