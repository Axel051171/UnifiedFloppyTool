 ...
 * @brief Track visualization and display
 * @version 3.8.0
 */
/*
//
...
void libflux_td_draw_rules( LIBFLUX_TD *td )
{
	...
	// Print pixel density
	snprintf(tmp_str, sizeof(tmp_str),"x:%.3f uS/pix y:%.3f uS/pix",x_us_per_pixel,y_us_per_pixel);
	...
	for(i=0;i < 80;i++)
	{
		...
		snprintf(tmp_str, sizeof(tmp_str),"%.2duS",i);
				...
				do
				{
					...
					if(sc)
					{
						...
						if(sc->startdataindex != sc->startsectorindex)
						{
							...
							snprintf(tempstr, sizeof(tempstr),"---- %.3d Bytes",sc->sectorsize);

							switch(sc->trackencoding)
							{
								case ISOFORMAT_SD:
									if(sc->startdataindex != sc->endsectorindex)
										snprintf(tempstr, sizeof(tempstr),"FM   %.3dB DM:%.2Xh",sc->sectorsize,sc->alternate_datamark);
									else
										snprintf(tempstr, sizeof(tempstr),"FM   %.3dB DM: ??",sc->sectorsize);
									break;
								case ISOFORMAT_DD:
									if(sc->startdataindex != sc->endsectorindex)
										snprintf(tempstr, sizeof(tempstr),"MFM  %.3dB DM:%.2Xh",sc->sectorsize,sc->alternate_datamark);
									else
										snprintf(tempstr, sizeof(tempstr),"MFM  %.3dB DM: ??",sc->sectorsize);
									break;
								case DECRX02_SDDD:
									if(sc->startdataindex != sc->endsectorindex)
										snprintf(tempstr, sizeof(tempstr),"DEC M2FM  %.3dB DM:%.2Xh",sc->sectorsize,sc->alternate_datamark);
									else
										snprintf(tempstr, sizeof(tempstr),"DEC M2FM  %.3dB DM: ??",sc->sectorsize);
									break;
								case AMIGAFORMAT_DD:
									snprintf(tempstr, sizeof(tempstr),"AMFM %.3dB ",sc->sectorsize);
									break;
								case TYCOMFORMAT_SD:
									snprintf(tempstr, sizeof(tempstr),"TYFM %.3dB ",sc->sectorsize);
								break;
								case MEMBRAINFORMAT_DD:
									snprintf(tempstr, sizeof(tempstr),"MEMBRAIN %.3dB DM:%.2Xh",sc->sectorsize,sc->alternate_datamark);
								break;
								case EMUFORMAT_SD:
									snprintf(tempstr, sizeof(tempstr),"E-mu %.3dB ",sc->sectorsize);
								break;
								case APPLE2_GCR5A3:
									snprintf(tempstr, sizeof(tempstr),"Apple II GCR5A3 %.3dB ",sc->sectorsize);
								break;
								case APPLE2_GCR6A2:
									snprintf(tempstr, sizeof(tempstr),"Apple II GCR6A2 %.3dB ",sc->sectorsize);
								break;
								case APPLEMAC_GCR6A2:
									snprintf(tempstr, sizeof(tempstr),"Apple Macintosh GCR6A2 %.3dB ",sc->sectorsize);
								break;
								case ARBURG_DAT:
									snprintf(tempstr, sizeof(tempstr),"Arburg DATA %.3dB ",sc->sectorsize);
								break;
								case ARBURG_SYS:
									snprintf(tempstr, sizeof(tempstr),"Arburg SYSTEM %.3dB ",sc->sectorsize);
								break;
								case AED6200P_DD:
									if(sc->startdataindex != sc->endsectorindex)
										snprintf(tempstr, sizeof(tempstr),"AED 6200P %.3dB DM:%.2Xh",sc->sectorsize,sc->alternate_datamark);
									else
										snprintf(tempstr, sizeof(tempstr),"AED 6200P %.3dB DM: ??",sc->sectorsize);
									break;
								break;
								case QD_MO5_MFM:
									snprintf(tempstr, sizeof(tempstr),"QD MO5 %.3dB",sc->sectorsize);
								break;
								case C64_GCR:
									snprintf(tempstr, sizeof(tempstr),"C64 GCR %.3dB",sc->sectorsize);
								break;
								case VICTOR9K_GCR:
									snprintf(tempstr, sizeof(tempstr),"VICTOR 9K GCR %.3dB",sc->sectorsize);
								break;
							}

							if(sc->fill_byte_used)
								snprintf(tempstr2, sizeof(tempstr2)," F:%.2Xh",sc->fill_byte);
							else
							...
							if(sc->startdataindex != sc->endsectorindex)
								snprintf(tempstr, sizeof(tempstr),"T:%.2d H:%d S:%.3d CRC:%.4X",sc->cylinder,sc->head,sc->sector,sc->data_crc);
							else
								snprintf(tempstr, sizeof(tempstr),"T:%.2d H:%d S:%.3d NO DATA?",sc->cylinder,sc->head,sc->sector);
						...
						else
						{
							...
							{
								case ISOFORMAT_SD:
									snprintf(tempstr, sizeof(tempstr),"FM   DATA ? DM:%.2Xh",sc->alternate_datamark);
									break;
								case ISOFORMAT_DD:
									snprintf(tempstr, sizeof(tempstr),"MFM  DATA ? DM:%.2Xh",sc->alternate_datamark);
									break;
								case AMIGAFORMAT_DD:
									snprintf(tempstr, sizeof(tempstr),"AMFM DATA ?");
									break;

								case TYCOMFORMAT_SD:
									snprintf(tempstr, sizeof(tempstr),"TYFM DATA ? DM:%.2Xh",sc->alternate_datamark);
								break;

								case MEMBRAINFORMAT_DD:
									snprintf(tempstr, sizeof(tempstr),"MEMBRAIN DATA ? DM:%.2Xh",sc->alternate_datamark);
								break;
								case EMUFORMAT_SD:
									snprintf(tempstr, sizeof(tempstr),"E-mu Data ?");
								break;
								case APPLE2_GCR5A3:
									snprintf(tempstr, sizeof(tempstr),"Apple II 5A3 Data ?");
								break;
								case APPLE2_GCR6A2:
									snprintf(tempstr, sizeof(tempstr),"Apple II 6A2 Data ?");
								break;
								case ARBURG_DAT:
									snprintf(tempstr, sizeof(tempstr),"Arburg DATA Data ?");
								break;
								case ARBURG_SYS:
									snprintf(tempstr, sizeof(tempstr),"Arburg SYSTEM Data ?");
								break;
								case AED6200P_DD:
									snprintf(tempstr, sizeof(tempstr),"AED 6200P Data ?");
								break;
								case QD_MO5_MFM:
									snprintf(tempstr, sizeof(tempstr),"QD MO5 Data ?");
								break;
								case VICTOR9K_GCR:
									snprintf(tempstr, sizeof(tempstr),"Victor 9K Data ?");
								break;
							}
							putstring8x8(td,LAYER_TEXT,xpos_startheader,225,tempstr,0x000,0x000,1,1);
						}
							...
							if(x2-x1>=8)
							{
								for(i=0;i<sc->sectorsize;i++)
								{
									...
									if( xpos_tmp < td->xsize )
									{
										if(x2-x1>=16)
										{
											snprintf(tempstr, sizeof(tempstr),"%.2X",sc->input_data[i]);
											putstring8x8(td,LAYER_TEXT,xpos_tmp,225,tempstr,0x000,0x000,0,1);
										}

										if( isAsciiChar(sc->input_data[i]) )
										{
											snprintf(tempstr, sizeof(tempstr),"%c",sc->input_data[i]);
...
void libflux_td_draw_track( LIBFLUX_TD *td, LIBFLUX_FLOPPY * floppydisk, int32_t track, int32_t side )
{
	...
	if(td->name)
	{
		snprintf(tmp_str, sizeof(tmp_str),"%s T:%.3d S:%.1d",td->name,track,side);
	}
	else
	{
		snprintf(tmp_str, sizeof(tmp_str),"T:%.3d S:%.1d",track,side);
	}
		...
		do
		{
			...
			if( currentside->databuffer[i>>3] & (0x80>>(i&7)) )
			{
				...
				if(bitrate <= 0)
				{
					// Invalid bitrate...
					if(currentside->bitrate == VARIABLEBITRATE)
						snprintf(tmp_str, sizeof(tmp_str),"ERROR : Invalid bitrate -> bitrate[%d] == %d ! Please check the loader or report the issue !",i>>3,bitrate);
					else
						snprintf(tmp_str, sizeof(tmp_str),"ERROR : Invalid bitrate -> bitrate == %d ! Please check the loader or report the issue !",bitrate);
				...
				if( currentside->indexbuffer[i>>3] )
				{
					if(!old_index_state)
					{
						if(last_index_xpos != -1)
						{
							...
							snprintf(tmp_str, sizeof(tmp_str),"%.2f RPM / %.2f ms", (float)(60 * 1000) / (index_period / (float)1000), index_period / (float)1000 );
...
void libflux_td_draw_stream_track( LIBFLUX_TD *td, LIBFLUX_FLOPPY * floppydisk, int32_t track, int32_t side )
{
	...
	if(!floppydisk->floppyNumberOfTrack || !floppydisk->floppyNumberOfSide)
	{
		...
		snprintf(tmp_str, sizeof(tmp_str),"This track doesn't exist !");
...
void libflux_td_draw_disk(LIBFLUX_TD *td,LIBFLUX_FLOPPY * floppydisk)
{
	...
	snprintf(tempstr, sizeof(tempstr),"Side %d, %d Tracks",0,floppydisk->floppyNumberOfTrack);
	putstring8x8(td,LAYER_TEXT,1,1,tempstr,0xAAAAAA,0x000000,0,1);

	if(floppydisk->floppyNumberOfSide == 2)
	{
		snprintf(tempstr, sizeof(tempstr),"Side %d, %d Tracks",1,floppydisk->floppyNumberOfTrack);
	...
	snprintf(tempstr, sizeof(tempstr),"%d Bytes", countSize(td->sl,0));
	putstring8x8(td,LAYER_TEXT,1,21,tempstr,0xAAAAAA,0x000000,0,1);

	snprintf(tempstr, sizeof(tempstr),"CRC32: --------");
	...
	snprintf(tempstr, sizeof(tempstr),"%d Bytes", countSize(td->sl,1));
	...
	snprintf(tempstr, sizeof(tempstr),"CRC32: Processing");
	putstring8x8(td,LAYER_TEXT,td->xsize/2 - (8*18),td->ysize - 9,tempstr,0xAAAAAA,0x000000,0,1);
