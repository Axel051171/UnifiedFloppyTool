...
int atx_load(const char *filename, atx_image_t *img)
{
    ...
    /* Seek to track data */
    fseek(fp, img->header.start, SEEK_SET);
    ...
    while (ftell(fp) < (long)img->header.end) {
        ...
        if (track_hdr.type != ATX_CHUNK_TRACK) {
            fseek(fp, track_hdr.size - sizeof(track_hdr), SEEK_CUR);
            continue;
        }
        ...
        for (int s = 0; s < track_hdr.sector_count; s++) {
            ...
            fread(&sect_hdr, sizeof(sect_hdr), 1, fp);
            ...
            if (sect_hdr.start_data > 0) {
                long cur = ftell(fp);
                fseek(fp, sector_data_start + sect_hdr.start_data, SEEK_SET);
                
                int sector_size = (img->density == 2) ? 256 : 128;
                fread(img->tracks[t].sectors[s].data, 1, sector_size, fp);
        ...
        fseek(fp, sector_data_start + track_hdr.size - sizeof(track_hdr) - track_hdr.header_size, SEEK_CUR);
