/**
 * @file dmk_loader.c
 * @brief DMK disk image format loader
 * @version 3.8.0
 */
/*
//
...
LIBFLUX_SIDE* DMKpatchtrack(LIBFLUX_CTX* flux_ctx,unsigned char * trackdata, unsigned char * trackclk,unsigned short * idamoffset,unsigned int tracklen,uint32_t * tracktotalsize, dmk_header *dmkh,int s)
{
	...
	/*{
		...
		snprintf(fname, sizeof(fname),"t_%.2dc.bin",s);
		savebuffer(fname,trackclk, tracklen);
		snprintf(fname, sizeof(fname),"t_%.2dd.bin",s);
		savebuffer(fname,trackdata, tracklen);
		snprintf(fname, sizeof(fname),"t_%.2db.bin",s);
