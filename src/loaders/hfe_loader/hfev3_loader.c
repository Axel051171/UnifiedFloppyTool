...
int HFEV3_libIsValidDiskFile( LIBFLUX_IMGLDR * imgldr_ctx, LIBFLUX_IMGLDR_FILEINFOS * imgfile )
{
	picfileformatheader * header;

	imgldr_ctx->ctx->libflux_printf(MSG_DEBUG,"HFEV3_libIsValidDiskFile");
	...
	if(imgfile)
	{
		...
		{
			imgldr_ctx->ctx->libflux_printf(MSG_DEBUG,"HFEV3_libIsValidDiskFile : HFE V3 file !");
			return LIBFLUX_VALIDFILE;
		}
		else
		{
			imgldr_ctx->ctx->libflux_printf(MSG_DEBUG,"HFEV3_libIsValidDiskFile : non HFE V3 file !");
...
int HFEV3_libLoad_DiskFile(LIBFLUX_IMGLDR * imgldr_ctx,LIBFLUX_FLOPPY * floppydisk,char * imgfile,void * parameters)
{
	FILE * f;
	picfileformatheader header;
	int i,j,k,l;
	...
	imgldr_ctx->ctx->libflux_printf(MSG_DEBUG,"HFEV3_libLoad_DiskFile %s",imgfile);
	...
	if(!strncmp((char*)header.HEADERSIGNATURE,"HXCHFEV3",8))
	{
		...
		imgldr_ctx->ctx->libflux_printf(MSG_DEBUG,"HFEv3 : %d track, %d side, %d bit/s, %d sectors, interface mode %s, track encoding:%s",
		...
		for(i=0;i<floppydisk->floppyNumberOfTrack;i++)
		{
			...
			if (fseek(f,(trackoffsetlist[i].offset*512),SEEK_SET) != 0) { /* seek error */ }
			...
			imgldr_ctx->ctx->libflux_printf(MSG_DEBUG,"HFEv3 : reading track %d, track size:%d - file offset:%.8X",
					...
					if( ( hfetrack2[l] & OPCODE_MASK ) == OPCODE_MASK )
					{
						switch( hfetrack2[l] )
						{
							case NOP_OPCODE:
								...
								imgldr_ctx->ctx->libflux_printf(MSG_DEBUG,"HFEv3 : T%.3dS%d Off[%.5d] : NOP_OPCODE",i,j,l);
							...
							case SETINDEX_OPCODE:
								...
								imgldr_ctx->ctx->libflux_printf(MSG_DEBUG,"HFEv3 : T%.3dS%d Off[%.5d] : SETINDEX_OPCODE",i,j,l);
							...
							case SETBITRATE_OPCODE:
								...
								{
									imgldr_ctx->ctx->libflux_printf(MSG_ERROR,"HFEv3 : T%.3dS%d Off[%.5d] : SETBITRATE_OPCODE : %d. NULL Rate divisor !",i,j,l,hfetrack2[l+1]);
								}

								imgldr_ctx->ctx->libflux_printf(MSG_DEBUG,"HFEv3 : T%.3dS%d Off[%.5d] : SETBITRATE_OPCODE : %d - %d",i,j,l,hfetrack2[l+1],bitrate);
							...
							case SKIPBITS_OPCODE:
								...
								imgldr_ctx->ctx->libflux_printf(MSG_DEBUG,"HFEv3 : T%.3dS%d Off[%.5d] : SKIPBITS_OPCODE : %d 0x%.2X",i,j,l,next_data_bitskip,hfetrack2[l+2]);
							...
							case RAND_OPCODE:
								...
								imgldr_ctx->ctx->libflux_printf(MSG_DEBUG,"HFEv3 : T%.3dS%d Off[%.5d] : RAND_OPCODE",i,j,l);
							...
							default:
								...
								imgldr_ctx->ctx->libflux_printf(MSG_DEBUG,"HFEv3 : T%.3dS%d Off[%.5d] : Unknown Opcode !?! : 0x%.2X",hfetrack2[l]);
...
int HFEV3_libWrite_DiskFile(LIBFLUX_IMGLDR* imgldr_ctx,LIBFLUX_FLOPPY * floppy,char * filename);
