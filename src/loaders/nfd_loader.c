    ...
    if (strncmp(sig, NFD_R0_SIGNATURE, 14) == 0) {
        ...
        for (int t = 0; t < 82; t++) {
            for (int h = 0; h < img->heads; h++) {
                ...
                fseek(fp, header.track_offset[idx], SEEK_SET);
                ...
                for (int s = 0; s < sectors; s++) {
                    ...
                    fread(img->track_data[idx].sectors[s].data, 1, sector_size, fp);
                }
            }
        }
        ...
        for (int t = 0; t < 164; t++) {
            ...
            for (int s = 0; s < track_hdr.sectors; s++) {
                ...
                fread(img->track_data[t].sectors[s].data, 1, size, fp);
            }
        }
    ...
    if (img->is_r1) {
        ...
        for (int t = 0; t < 164; t++) {
            ...
            for (int s = 0; s < track_hdr.sectors; s++) {
                ...
                fwrite(&sect_hdr, sizeof(sect_hdr), 1, fp);
            }
            
            /* Write sector data */
            for (int s = 0; s < track_hdr.sectors; s++) {
                fwrite(img->track_data[t].sectors[s].data, 1,
                       img->track_data[t].sectors[s].size, fp);
            }
        }
