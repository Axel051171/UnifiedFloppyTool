...
int woz_save(const woz_image_t *img, const char *filename)
{
    ...
    if (img->is_woz2) {
        ...
        for (int t = 0; t < 40; t++) {
            if (img->tracks[t].data) {
                ...
                fwrite(img->tracks[t].data, 1, img->tracks[t].byte_count, fp);
                /* Padding */
                uint8_t pad[512] = {0};
        ...
        for (int t = 0; t < 40; t++) {
            ...
            fwrite(&bytes_used, 2, 1, fp);
            fwrite(&bit_count_lo, 2, 1, fp);
    ...
    fread(buffer, 1, file_size - 12, fp);
    uint32_t crc = woz_crc32(buffer, file_size - 12);
    free(buffer);
    
    fseek(fp, crc_pos, SEEK_SET);
    fwrite(&crc, 4, 1, fp);
