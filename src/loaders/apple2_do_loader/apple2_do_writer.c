...
int Apple2_do_libWrite_DiskFile(LIBFLUX_IMGLDR* imgldr_ctx,LIBFLUX_FLOPPY * floppy,char * filename)
{
	...
	if( libflux_checkfileext(filename,"po",SYS_PATH_TYPE) )
	{
		sector_mapping = (unsigned char *)&PhysicalToLogicalSectorMap_ProDos;
		imgldr_ctx->ctx->libflux_printf(MSG_INFO_1,"Write PO file %s...",filename);
	}
	else
	{
		sector_mapping = (unsigned char *)&PhysicalToLogicalSectorMap_Dos33;
		imgldr_ctx->ctx->libflux_printf(MSG_INFO_1,"Write DO file %s...",filename);
	}
	...
	for(j=0;j<(int)floppy->floppyNumberOfTrack;j++)
	{
		for(i=0;i<(int)floppy->floppyNumberOfSide;i++)
		{
			...
			do
			{
				...
				{
					case 0:
						sca = libflux_getAllTrackSectors(ss,j,i,APPLEII_GCR1_ENCODING,&nbsector);
					break;
					case 1:
						sca = libflux_getAllTrackSectors(ss,j,i,APPLEII_GCR2_ENCODING,&nbsector);
					break;
					case 2:
						sca = libflux_getAllTrackSectors(ss,j,i,APPLEII_HDDD_A2_GCR1_ENCODING,&nbsector);
					break;
					case 3:
						sca = libflux_getAllTrackSectors(ss,j,i,APPLEII_HDDD_A2_GCR2_ENCODING,&nbsector);
					break;
				}
						...
						for(l=0;l<256;l++)
						{
							...
							do
							{
								if(sca[k]->sector==l)
								{
									...
									log_str = libflux_dyn_sprintfcat(NULL,"%d ",sca[k]->sector);
