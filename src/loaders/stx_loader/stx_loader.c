...
int * getSectorHeaderOffset(LIBFLUX_CTX* flux_ctx,unsigned char * trackbuffer,int buffersize,int numsector)
{
	...
	if(numsector)
	{
		/* Sanity cap: max 256 sectors per track */
		if(numsector > 256)
			return NULL;
		offsetlist = malloc(sizeof(int) * (size_t)numsector);
		if( !offsetlist )
			return NULL;
...
int STX_libLoad_DiskFile(LIBFLUX_IMGLDR * imgldr_ctx,LIBFLUX_FLOPPY * floppydisk,char * imgfile,void * parameters)
{
	...
	if(!strcmp((char*)&fileheader.headertag,"RSY"))
	{
		...
		for(i=0;i<numberoftrack;i++)
		{
			...
			if (fseek(f,trackheaderpos,SEEK_SET) != 0) { goto alloc_error; }
			if (fseek(f,trackheader.track_block_size,SEEK_CUR) != 0) { goto alloc_error; }
		}

		numberoftrackperside++;

		if (fseek(f,t,SEEK_SET) != 0) { goto alloc_error; }
...
#ifdef PASTI_DBG
		snprintf(tempstring, sizeof(tempstring),"File header :");
		for(debug_i=0;debug_i<sizeof(pasti_fileheader);debug_i++)
		{
			snprintf(tempstring + strlen(tempstring), sizeof(tempstring) - strlen(tempstring),"%.2X ",*(((unsigned char*)&fileheader)+debug_i));
		}
		...
		{
			/* Sanity cap: max 168 tracks (STX maximum) */
			...
			for(i=0;i<numberoftrack;i++)
			{
				...
				switch(trackmode)
				{
					...
					case 0:
						...
						{
							/* Sanity cap: max 256 sectors per track */
							if(trackheader.numberofsector > 256)
								goto alloc_error;
							sector = malloc(sizeof(pasti_sector)*(size_t)trackheader.numberofsector);
							sectorconfig = (LIBFLUX_SECTCFG *) malloc(sizeof(LIBFLUX_SECTCFG)*(size_t)trackheader.numberofsector);
							if( !sector || !sectorconfig )
								goto alloc_error;

							memset(sectorconfig,0,sizeof(LIBFLUX_SECTCFG)*(size_t)trackheader.numberofsector);
							memset(sector,0,sizeof(pasti_sector)*(size_t)trackheader.numberofsector);
							...
							for(j=0;j<(trackheader.numberofsector);j++)
							{
								...
								{
									if (fseek(f,sectorlistoffset + (sizeof(pasti_sector)*j) ,SEEK_SET) != 0) { goto alloc_error; }
									...
									{
										snprintf(tempstring + strlen(tempstring), sizeof(tempstring) - strlen(tempstring),"%.2X",*(((unsigned char*)&sector[j])+debug_i));
						...
						if (fseek(f,trackpos,SEEK_SET) != 0) { goto alloc_error; }
...
#endif
							if (fseek(f,trackpos + sector[j].sector_pos,SEEK_SET) != 0) { goto alloc_error; }
						...
						for(j=0;j<(trackheader.numberofsector);j++)
						{
							if((sector[j].FDC_status&0x80)==0x80)
							{
								...
								if(weaksectorbuffer)
								{
									if (fseek(f,weaksectoroffset,SEEK_SET) != 0) { goto alloc_error; }
					...
					case 1:
						...
						for(j=0;j<(trackheader.numberofsector);j++)
						{
							...
							///////////////////////////// debug ///////////////////////////////
#ifdef PASTI_DBG
							snprintf(tempstring, sizeof(tempstring),"Sector:%.2d Size:%.8d Cylcode:%.2d HeadCode:%d SectorCode:%.2d Size:%d",j,sectorconfig[j].sectorsize,sectorconfig[j].cylinder,sectorconfig[j].head,sectorconfig[j].sector,sectorconfig[j].sectorsize);
							snprintf(tempstring + strlen(tempstring), sizeof(tempstring) - strlen(tempstring)," Sector header:");
							for(debug_i=0;debug_i<sizeof(pasti_sector);debug_i++)
							{
								snprintf(tempstring + strlen(tempstring), sizeof(tempstring) - strlen(tempstring),"%.2X",*(((unsigned char*)&sector[j])+debug_i));
								if(debug_i==3 || debug_i==5 ||  debug_i==7 || debug_i==8 || debug_i==9 || debug_i==10 || debug_i==11 || debug_i==13 || debug_i==15)
						...
						if(temptrack && tempclock && tempmask)
						{
							...
							for(j=0;j<(trackheader.numberofsector);j++)
							{
								...
								if (fseek(f,trackpos,SEEK_SET) != 0) { goto alloc_error; }
								if (fseek(f,sector[j].sector_pos,SEEK_CUR) != 0) { goto alloc_error; }
								libflux_fread(sectorconfig[j].input_data,sectorconfig[j].sectorsize,f);
							}
