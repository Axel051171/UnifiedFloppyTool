/**
 * @file d88_loader.c
 * @brief NEC PC-88/98 D88 loader (standalone)
 * @version 3.8.0
 */
/*
//
...
int D88_libLoad_DiskFile(LIBFLUX_IMGLDR * imgldr_ctx,LIBFLUX_FLOPPY * floppydisk,char * imgfile,void * parameters)
{
	...
	while((!feof(f)) && (partcount<256) && (totalfilesize<truetotalfilesize))
	{
		...
		if(fileheader.file_size)
		{
			...
			if (fseek(f,fileheader.file_size-sizeof(fileheader),SEEK_CUR) != 0) { libflux_fclose(f); return LIBFLUX_ACCESSERROR; }
		...
		else
		{
			...
			if (fseek(f,truetotalfilesize,SEEK_SET) != 0) { libflux_fclose(f); return LIBFLUX_ACCESSERROR; }
		}
	}
	...
	if (fseek(f,basefileptr+sizeof(d88_fileheader),SEEK_SET) != 0) { libflux_fclose(f); return LIBFLUX_ACCESSERROR; }
	...
	if(track_offset >= (sizeof(d88_fileheader) + sizeof(uint32_t)) )
	{
		...
		do
		{
			if (fseek(f,basefileptr+sizeof(d88_fileheader)+((number_of_track-1)*sizeof(uint32_t)),SEEK_SET) != 0) { libflux_fclose(f); return LIBFLUX_ACCESSERROR; }
	...
	do
	{
		...
		if(track_offset)
		{
			if (fseek(f,track_offset+basefileptr,SEEK_SET) != 0) { libflux_fclose(f); return LIBFLUX_ACCESSERROR; }
		...
		if(side==2)
		{
			...
			if (fseek(f,basefileptr + sizeof(d88_fileheader)  + (i * sizeof(uint32_t)),SEEK_SET) != 0) { libflux_fclose(f); return LIBFLUX_ACCESSERROR; }
		}
		else
		{
			i=i+2;
			imgldr_ctx->ctx->libflux_printf(MSG_DEBUG,"Track %d offset: 0x%X",i>>1,track_offset);
			if (fseek(f,basefileptr + sizeof(d88_fileheader)  + ((i>>1) * sizeof(uint32_t)),SEEK_SET) != 0) { libflux_fclose(f); return LIBFLUX_ACCESSERROR; }
		}
