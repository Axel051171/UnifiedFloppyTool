                   ...
                   uft_bbc_format_t *format_out)
{
    ...
    /* ADFS detection: check for ADFS signatures.
     * ADFS stores a "free space map" at sector 0-1 with specific patterns.
     * ADFS L/D: Byte 0 of sector 0 = free space start (non-zero for valid disk).
     * ADFS E/F ("big dir"): Check for "Hugo" or "Nick" at offset 0x401. */
    
    /* ADFS old map (L/D): sectors 0-1 contain free space map */
    if (size >= 1024) {
        int adfs_score = 0;
        
        /* Check for ADFS old format: byte 0xFC of sector 0 = disk size in sectors (LE) */
        uint32_t disk_size_sectors = data[0xFC] | ((uint32_t)data[0xFD] << 8) |
                                     ((uint32_t)data[0xFE] << 16);
        if (disk_size_sectors > 0 && disk_size_sectors <= 0x100000) {
            adfs_score += 20;
        }
        
        /* Check for valid free space map checksum at byte 0xFF */
        uint8_t cksum = 0;
        for (int i = 0xFE; i >= 0; i--) {
            uint16_t sum = (uint16_t)cksum + data[i];
            cksum = (uint8_t)(sum + (sum >> 8));
        }
        if (cksum == data[0xFF]) adfs_score += 30;
        
        /* Second copy at sector 1 */
        if (size >= 512) {
            cksum = 0;
            for (int i = 0xFE; i >= 0; i--) {
                uint16_t sum = (uint16_t)cksum + data[256 + i];
                cksum = (uint8_t)(sum + (sum >> 8));
            }
            if (cksum == data[256 + 0xFF]) adfs_score += 20;
        }
        
        /* ADFS E/F (new map): check for "Hugo" or "Nick" at 0x401 */
        if (size >= 0x410) {
            if (data[0x401] == 'H' && data[0x402] == 'u' && 
                data[0x403] == 'g' && data[0x404] == 'o') {
                adfs_score = 90;  /* Strong ADFS E+ indicator */
            }
            if (data[0x401] == 'N' && data[0x402] == 'i' && 
                data[0x403] == 'c' && data[0x404] == 'k') {
                adfs_score = 90;  /* ADFS F+ */
            }
        }
        
        if (adfs_score >= 50) {
            if (format_out) *format_out = UFT_BBC_DFS_80T_SS;  /* Closest match */
            return adfs_score;
        }
    }
