...
int match(const char *a, const char *pattern) 
{
  ...
  if (user==-1) snprintf(pat, sizeof(pat),"??%s",pattern);
  else snprintf(pat, sizeof(pat),"%02d%s",user,pattern);
...
void cpmglob(int optin, int argc, char * const argv[], struct cpmInode *root, int *gargc, char ***gargv)
{
  ...
  *gargc=0;
  ...
  if (!dirent) return;  /* BUG-001 fix: NULL check */
...
int cpmCheckDs(struct cpmSuperBlock *sb)
{
  ...
  /* Allocate buffer */
  sb->ds=uft_safe_malloc_array(dsblks, sb->blksiz);
...
int cpmReaddir(struct cpmFile *dir, struct cpmDirent *ent)
{
  ...
  while (1)
  {
    ...
    if (dir->pos==0) /* first entry is . */ /*{{{*/
    {
      ...
      ent->reclen=1;
      strncpy(ent->name, ".", sizeof(ent->name)-1); ent->name[sizeof(ent->name)-1] = '\0';
      ent->off=dir->pos;
      ++dir->pos;
      return 1;
    }
    ...
    else if (dir->pos==2)
    {
      if (dir->ino->sb->passwdLength) /* next entry is [passwd] */ /*{{{*/
      {
        ...
        ent->reclen=8;
        strncpy(ent->name, "[passwd]", sizeof(ent->name)-1); ent->name[sizeof(ent->name)-1] = '\0';
        ent->off=dir->pos;
        ++dir->pos;
        return 1;
      }
    ...
    else if (dir->pos==3)
    {
      if (dir->ino->sb->labelLength) /* next entry is [label] */ /*{{{*/
      {
        ...
        ent->reclen=7;
        strncpy(ent->name, "[label]", sizeof(ent->name)-1); ent->name[sizeof(ent->name)-1] = '\0';
        ent->off=dir->pos;
        ++dir->pos;
        return 1;
      }
      ...
      if ((cur=dir->ino->sb->dir+(dir->pos-RESERVED_ENTRIES))->status>=0 && cur->status<=(dir->ino->sb->type&CPMFS_HI_USER ? 31 : 15))
      {
        ...
        if (first==(dir->pos-RESERVED_ENTRIES))
        {
          ...
          *bufp='\0';
          ...
          ent->reclen=bufp-buf;
          strncpy(ent->name, buf, sizeof(ent->name)-1); ent->name[sizeof(ent->name)-1] = '\0';
          ent->off=dir->pos;
          ++dir->pos;
          return 1;
        }
...
int cpmRead(struct cpmFile *file, char *buf, int count)
{
  ...
  {
    char *buffer = (char*)malloc(16384); if (!buffer) return -1;
