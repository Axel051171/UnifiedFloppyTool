...
int uft_dos33_create_ts_list(uft_apple_ctx_t *ctx, const uint8_t *data, size_t size,
    ...
    while (written_sectors < data_sectors) {
        ...
        while (pair_idx < 122 && written_sectors < data_sectors) {
            ...
            if (ret < 0) {
                /* Cleanup: free T/S list sector and any data sectors in tslist */
                for (int ci = 0; ci < pair_idx; ci++) {
                    uft_apple_free_sector(ctx, tslist->pairs[ci].track,
                                          tslist->pairs[ci].sector);
                }
                uft_apple_free_sector(ctx, cur_ts_track, cur_ts_sector);
                return ret;
            }
