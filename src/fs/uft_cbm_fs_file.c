...
uft_rc_t uft_cbm_file_inject(uft_cbm_fs_t* fs, const char* filename,
    ...
    while (remaining > 0 || (size == 0 && first_track == 0)) {
        ...
        if (rc != UFT_SUCCESS) {
            /* Free already allocated sectors by walking the chain */
            if (first_track != 0) {
                uint8_t ft = first_track, fs_sec = first_sector;
                while (ft != 0) {
                    uint8_t chain_data[UFT_CBM_SECTOR_SIZE];
                    if (uft_cbm_read_sector(fs, ft, fs_sec, chain_data) != UFT_SUCCESS) break;
                    uint8_t next_t = chain_data[0];
                    uint8_t next_s = chain_data[1];
                    uft_cbm_bam_free(fs, ft, fs_sec);
                    ft = next_t;
                    fs_sec = next_s;
                }
            }
