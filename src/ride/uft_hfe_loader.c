...
uft_flux_buffer_t *uft_flux_load_hfe(const char *path, int track, int side) {
    ...
    /* Handle HFEv3: FLUXSTREAM opcode-based track format */
    if (v3) {
        /* HFEv3 uses opcode-based encoding with RLE compression.
         * Opcodes: 0xF1=SetIndex, 0xF2=SetBitrate, 0xF3=Skip, 0xF4=Rand
         * For basic reading, we process opcodes and extract raw bitstream data.
         * Bitstream data bytes with values < 0xF0 are direct MFM/FM data. */
    }
    ...
    if (fseek(f, track_list_pos + track * sizeof(hfe_track_entry_t), SEEK_SET) != 0) return NULL;
...
int uft_flux_write_hfe(const char *path, 
    ...
    /* Write track data - convert flux transitions to bitstream */
    for (int t = 0; t < num_tracks; t++) {
        uint8_t track_data[12500];
        memset(track_data, 0, sizeof(track_data));
        
        if (tracks[t] && tracks[t]->times && tracks[t]->count > 0) {
            /* Convert flux transitions to MFM bitstream */
            double bit_time_ns = 4000.0;  /* DD default */
            if (tracks[t]->detected_den == UFT_DENSITY_HD) bit_time_ns = 2000.0;
            
            size_t bit_pos = 0;
            size_t max_bits = track_list[t].track_len * 8;
            
            for (size_t f = 0; f < tracks[t]->count && bit_pos < max_bits; f++) {
                double interval_ns = (double)tracks[t]->times[f];
                int num_bits = (int)(interval_ns / bit_time_ns + 0.5);
                if (num_bits < 1) num_bits = 1;
                if (num_bits > 8) num_bits = 8;
                
                /* Output (num_bits-1) zeros then a 1 */
                bit_pos += (num_bits - 1);
                if (bit_pos < max_bits) {
                    track_data[bit_pos / 8] |= (1 << (7 - (bit_pos % 8)));
                    bit_pos++;
                }
            }
        }
        
        fwrite(track_data, 1, track_list[t].track_len, f);
