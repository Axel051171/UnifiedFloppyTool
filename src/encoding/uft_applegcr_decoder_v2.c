        ...
        switch (ctx->state) {
            ...
            case UFT_APPLE_STATE_DATA_DATA:
                ...
                if (ctx->nibble_len >= expected) {
                    ...
                    /* 5/3 GCR decode (Apple DOS 3.2, 13-sector format) */
                    if (ctx->gcr_mode == UFT_APPLE_GCR_53 && ctx->nibble_len >= 411) {
                        /* Decode nibbles to 5-bit values using gcr53 table */
                        uint8_t dec53[411];
                        bool valid = true;
                        for (int di = 0; di < 411 && valid; di++) {
                            dec53[di] = ctx->gcr53_decode[ctx->nibbles[di]];
                            if (dec53[di] == 0xFF) valid = false;
                        }
                        if (valid) {
                            /* XOR-decode chain */
                            uint8_t xck = 0;
                            for (int di = 0; di < 410; di++) {
                                dec53[di] ^= xck;
                                xck = dec53[di];
                            }
                            /* Verify checksum */
                            if (dec53[410] == xck) {
                                /* Reconstruct: first 153 nibbles = 3-bit aux,
                                   next 256 = 5-bit primary. 
                                   Byte[i] = (primary[i] << 3) | (aux[i/5*3 + ...]) */
                                for (int di = 0; di < 256; di++) {
                                    int aux_group = di / 5;
                                    int aux_bit   = di % 5;
                                    int aux_idx   = aux_group * 3;
                                    uint8_t lo3 = 0;
                                    if (aux_bit < 3 && aux_idx + aux_bit < 153) {
                                        lo3 = dec53[aux_idx + aux_bit] & 0x07;
                                    } else if (aux_bit >= 3 && aux_idx + (aux_bit-3) < 153) {
                                        /* Remaining bits wrap */
                                        lo3 = (dec53[aux_idx + 2] >> (aux_bit - 2)) & 0x07;
                                    }
                                    ctx->decoded[di] = (dec53[153 + di] << 3) | lo3;
                                }
                                ctx->decoded_len = 256;
                                ok = true;
                            }
