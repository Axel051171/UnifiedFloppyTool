/**
 * @file catalogtab.cpp
 * @brief Catalog Tab Implementation
 * 
...
CatalogTab::CatalogTab(QWidget *parent)
    : QWidget(parent)
    , ui(new Ui::TabCatalog)
    , m_imageLoaded(false)
{
    ui->setupUi(this);
    setupConnections();
...
void CatalogTab::setupConnections()
{
    connect(ui->btnOpenImage, &QPushButton::clicked, this, &CatalogTab::onOpenImage);
    connect(ui->btnCloseImage, &QPushButton::clicked, this, &CatalogTab::onCloseImage);
    connect(ui->btnRefresh, &QPushButton::clicked, this, &CatalogTab::onRefresh);
    connect(ui->btnUp, &QPushButton::clicked, this, &CatalogTab::onNavigateUp);
    connect(ui->btnExtractSelected, &QPushButton::clicked, this, &CatalogTab::onExtractSelected);
    connect(ui->btnExtractAll, &QPushButton::clicked, this, &CatalogTab::onExtractAll);
    connect(ui->btnBrowseExtract, &QPushButton::clicked, this, &CatalogTab::onBrowseExtractPath);
    
    connect(ui->tableFiles, &QTableWidget::cellDoubleClicked,
            this, &CatalogTab::onItemDoubleClicked);
    connect(ui->tableFiles, &QTableWidget::itemSelectionChanged,
            this, &CatalogTab::onSelectionChanged);
}

void CatalogTab::loadImage(const QString& imagePath)
{
...
void CatalogTab::onCloseImage()
{
    clear();
    emit statusMessage(tr("Image closed"));
}

void CatalogTab::onRefresh()
{
    if (m_imageLoaded) {
        updateFileList();
    }
...
void CatalogTab::onExtractSelected()
{
...
void CatalogTab::onBrowseExtractPath()
{
...
void CatalogTab::onItemDoubleClicked(int row, int column)
{
    Q_UNUSED(column);
...
void CatalogTab::populateFileTable(const QList<FileEntry>& entries)
{
...
QString CatalogTab::formatSize(qint64 size) const
{
    if (size >= 1024*1024) {
...
QList<FileEntry> CatalogTab::readDirectory(const QString& path)
{
