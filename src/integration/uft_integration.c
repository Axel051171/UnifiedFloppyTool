                         ...
                         size_t start, size_t *sync_pos)
{
    ...
    for (size_t i = start; i < bit_count - 48; i++) {
        bool match = true;
                                 ...
                                 uft_bitstream_result_t *result)
{
    ...
    while (pos < bit_count - 1024 && result->sector_count < 64) {
        ...
        /* Verify header CRC */
        uint8_t hdr_data[5] = {0xA1, 0xA1, 0xA1, 0xFE, 0};
        memcpy(hdr_data + 4, idam + 1, 4);
        /* Simplified CRC check */
        sec->header_crc_ok = true;  /* TODO: proper CRC */
        ...
        /* Verify data CRC */
        sec->data_crc_ok = true;  /* TODO: proper CRC */
