                                 ...
                                 uft_bitstream_result_t *result)
{
    ...
    while (pos < len - 10 && result->sector_count < 32) {
        ...
        /* Read and verify header CRC (CRC-CCITT over FE+C+H+R+N) */
        uint16_t stored_hcrc = ((uint16_t)data[pos] << 8) | data[pos + 1];
        {
            uint16_t crc = 0xFFFF;
            uint8_t hdr[5] = {0xFE, sec->track, sec->head, sec->sector, sec->size_code};
            for (int b = 0; b < 5; b++) {
                crc ^= (uint16_t)hdr[b] << 8;
                for (int bit = 0; bit < 8; bit++)
                    crc = (crc << 1) ^ ((crc & 0x8000) ? 0x1021 : 0);
            }
            sec->header_crc_ok = (crc == stored_hcrc);
        }
        pos += 2;  /* Skip CRC */
        ...
        if (sec->data) {
            ...
            /* Verify data CRC (CRC-CCITT over DAM + data) */
            if (pos + data_len + 2 <= len) {
                uint16_t stored_dcrc = ((uint16_t)data[pos + data_len] << 8) |
                                        data[pos + data_len + 1];
                uint16_t crc = 0xFFFF;
                crc ^= (uint16_t)dam << 8;
                for (int bit = 0; bit < 8; bit++)
                ...
                sec->data_crc_ok = (crc == stored_dcrc);
            } else {
                sec->data_crc_ok = false;
            }
        }
        pos += data_len + 2;
