...
{
    if (dam < sizeof(dam_names) / sizeof(dam_names[0]) && dam_names[dam]) {
                               ...
                               uft_gw_encoding_t encoding)
{
    ...
    /**
     * Greaseweazle flux stream decoding (reference: usb.py _decode_flux)
     *   0x00         → End of stream
     *   0x01-0xF9    → Direct flux transition (1-249 ticks)
     *   0xFA-0xFE    → 2-byte: 250 + (byte-250)*255 + next - 1
     *   0xFF         → Opcode prefix:
     *     0xFF 0x01  → Index  + N28 (skip)
     *     0xFF 0x02  → Space  + N28 (add to accumulator)
     *     0xFF 0x03  → Astable + N28 (skip)
     */
    size_t i = 0;
    uint32_t ticks = 0;  /* Accumulator for Space opcodes */
    
    while (i < flux_len && bit_pos < *bit_len * 8) {
        uint8_t val = flux[i++];
        uint32_t sample = 0;
        ...
        if (val == 0x00) {
            ...
            /* Opcode prefix */
            if (i >= flux_len) break;
            uint8_t opcode = flux[i++];
            if (opcode == 0x01) {         /* FluxOp.Index */
                if (i + 4 > flux_len) break;
                i += 4;  /* Skip N28 index timing */
                continue;
            } else if (opcode == 0x02) {  /* FluxOp.Space */
                if (i + 4 > flux_len) break;
                /* Read N28 and add to accumulator */
                uint32_t n28  = (flux[i] & 0xFE) >> 1;
                n28 += (uint32_t)(flux[i+1] & 0xFE) << 6;
                n28 += (uint32_t)(flux[i+2] & 0xFE) << 13;
                n28 += (uint32_t)(flux[i+3] & 0xFE) << 20;
                i += 4;
                ticks += n28;
                continue;
            } else if (opcode == 0x03) {  /* FluxOp.Astable */
                if (i + 4 > flux_len) break;
                i += 4;  /* Skip N28 astable period */
                continue;
            } else {
            ...
            /* 2-byte encoding (250-254): 250 + (val-250)*255 + next - 1 */
            if (i >= flux_len) break;
            sample = ticks + 250 + (uint32_t)(val - 250) * 255 + flux[i++] - 1;
            ticks = 0;
        }
