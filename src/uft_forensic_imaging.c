...
int uft_fi_hash_init(uft_fi_job_t *job) {
    ...
    /* Initialize hash contexts based on job->hash_flags.
     * Uses algorithm callbacks from registered hash modules. */
...
void uft_fi_hash_update(uft_fi_job_t *job, const void *data, size_t len) {
    ...
    /* Update all active hash contexts */
    uft_fi_hash_output_t *ho = job->hash_outputs;
    while (ho) {
...
void uft_fi_hash_finalize(uft_fi_job_t *job) {
    ...
    /* Finalize all active hash contexts and generate hex results */
    uft_fi_hash_output_t *ho = job->hash_outputs;
    while (ho) {
