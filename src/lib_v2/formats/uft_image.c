...
uft_image_t* uft_image_open(const char *filename, const char *mode) {
    ...
    switch (type) {
        ...
        case UFT_IMG_ADF:
            ...
            if (fseek(img->file, 0, SEEK_END) != 0) { /* seek error */ }
            if (ftell(img->file) > 1000000) {
                img->geometry = adf_hd_geometry;
            } else {
                img->geometry = adf_dd_geometry;
            }
            if (fseek(img->file, 0, SEEK_SET) != 0) { /* seek error */ }
            img->caps = UFT_IMG_CAP_READ | UFT_IMG_CAP_WRITE;
            break;
...
uft_image_t* uft_image_create(const char *filename, uft_image_type_t type,
    ...
    /* Pre-allocate file */
    uint8_t zero = 0;
    if (fseek(img->file, (long)geometry->total_size - 1, SEEK_SET) != 0) { /* seek error */ }
    if (fwrite(&zero, 1, 1, img->file) != 1) { /* I/O error */ }
    if (fseek(img->file, 0, SEEK_SET) != 0) { /* seek error */ }
    img->caps = UFT_IMG_CAP_READ | UFT_IMG_CAP_WRITE;
...
bool uft_image_read_track(uft_image_t *image, uint8_t track, uint8_t head,
    ...
    switch (image->type) {
        case UFT_IMG_D64: {
            ...
            if (fseek(image->file, (long)offset, SEEK_SET) != 0) { /* seek error */ }
