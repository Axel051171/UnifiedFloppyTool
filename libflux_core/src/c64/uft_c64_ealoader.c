...
#include "uft_c64_ealoader.h"

#include <string.h>
#include <stdio.h>

static uft_ea_p_insn_t make_insn(uft_ea_p_op_t op, uft_ea_p_addrmode_t mode, uint8_t size){
    uft_ea_p_insn_t i;
    i.op = op;
    i.mode = mode;
    i.size = size;
...
uft_ea_p_insn_t uft_ea_p_decode(uint8_t opcode){
    /* Based on the opcode table in EaLoader.txt. */
    switch(opcode){
        case 0x00: return make_insn(UFT_EA_P_PJMP,  UFT_EA_P_ABS16,   3);
        case 0x01: return make_insn(UFT_EA_P_ANDR,  UFT_EA_P_IMM8,    2);
        case 0x02: return make_insn(UFT_EA_P_PCAL,  UFT_EA_P_ABS16,   3);
        case 0x03: return make_insn(UFT_EA_P_CALL,  UFT_EA_P_ABS16,   3);
        case 0x04: return make_insn(UFT_EA_P_LDR_I, UFT_EA_P_IMM8,    2);
        case 0x05: return make_insn(UFT_EA_P_LDR_A, UFT_EA_P_ABS16,   3);
        case 0x06: return make_insn(UFT_EA_P_BEQL,  UFT_EA_P_ABS16,   3);
        case 0x07: return make_insn(UFT_EA_P_STR,   UFT_EA_P_ABS16,   3);
        case 0x08: return make_insn(UFT_EA_P_SUB_I, UFT_EA_P_IMM8,    2);
        case 0x09: return make_insn(UFT_EA_P_JUMP,  UFT_EA_P_ABS16,   3);
        case 0x0A: return make_insn(UFT_EA_P_PRET,  UFT_EA_P_IMPLIED, 1);
        case 0x0B: return make_insn(UFT_EA_P_LDR_X, UFT_EA_P_INDEXED, 3);
        case 0x0C: return make_insn(UFT_EA_P_SHL,   UFT_EA_P_IMPLIED, 1);
        case 0x0D: return make_insn(UFT_EA_P_INC,   UFT_EA_P_ABS16,   3);
        case 0x0E: return make_insn(UFT_EA_P_ADD,   UFT_EA_P_ABS16,   3);
        case 0x0F: return make_insn(UFT_EA_P_DECR,  UFT_EA_P_IMPLIED, 1);
        case 0x10: return make_insn(UFT_EA_P_BNEQ,  UFT_EA_P_ABS16,   3);
        case 0x11: return make_insn(UFT_EA_P_SUB_A, UFT_EA_P_ABS16,   3);
        case 0x12: return make_insn(UFT_EA_P_BPLU,  UFT_EA_P_ABS16,   3);
        case 0x13: return make_insn(UFT_EA_P_LD16,  UFT_EA_P_IMM8,    3);
        default:   return make_insn((uft_ea_p_op_t)opcode, UFT_EA_P_IMPLIED, 1);
    }
}

uint8_t uft_ea_p_decrypt_imm8(uint8_t enc){
    return (uint8_t)(enc ^ 0x6B);
}

uint16_t uft_ea_p_decrypt_abs16(uint16_t enc_le){
    return (uint16_t)(enc_le ^ 0x292B);
}

bool uft_ea_p_decr(uint8_t* mem, size_t mem_len, uint16_t ptr, uint8_t* io_r8){
...
int uft_ea_autoloader_score(const uint8_t* ram_0000_03ff, size_t len){
...
int uft_ea_fattrack_score(const uft_ea_fattrack_observation_t* obs,
