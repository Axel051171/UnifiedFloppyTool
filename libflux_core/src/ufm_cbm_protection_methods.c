/*
 * ufm_cbm_protection_methods.c
 */

#include "uft/uft_error.h"
#include "ufm_cbm_protection_methods.h"
#include <string.h>
#include <stdio.h>
...
const char* ufm_cbm_method_name(ufm_cbm_prot_method_t m){
    switch(m){
        case UFM_CBM_PROT_UNKNOWN: return "unknown";
        case UFM_CBM_PROT_INTENTIONAL_ERRORS: return "intentional_disk_errors";
        case UFM_CBM_PROT_TRACK_SKEW: return "track_skew";
        case UFM_CBM_PROT_FAT_TRACKS: return "fat_tracks";
        case UFM_CBM_PROT_HALF_TRACKS: return "half_tracks";
        case UFM_CBM_PROT_EXTRA_TRACKS: return "extra_tracks";
        case UFM_CBM_PROT_CHANGED_BITRATES: return "changed_bitrates";
        case UFM_CBM_PROT_GAP_SIGNATURES: return "gap_signatures";
        case UFM_CBM_PROT_LONG_SECTORS: return "long_sectors";
        case UFM_CBM_PROT_CUSTOM_FORMATS: return "custom_formats";
        case UFM_CBM_PROT_LONG_TRACKS: return "long_tracks";
        case UFM_CBM_PROT_SYNC_COUNTING: return "sync_counting";
        case UFM_CBM_PROT_TRACK_SYNCHRONIZATION: return "track_synchronization";
        case UFM_CBM_PROT_WEAK_BITS_UNFORMATTED: return "weak_bits_unformatted";
        case UFM_CBM_PROT_SIGNATURE_KEY_TRACKS: return "signature_key_tracks";
        case UFM_CBM_PROT_NO_SYNC: return "no_sync";
        case UFM_CBM_PROT_SPIRADISC_LIKE: return "spiradisc_like";
        default: return "unknown";
    }
}

static void push_hit(ufm_cbm_method_hit_t* hits, size_t cap, size_t* io_n,
                     ufm_cbm_prot_method_t m, int track_x2, int conf){
...
static int score_errors(const ufm_cbm_track_metrics_t* t){
    if(!t) return 0;
...
static int score_longtrack(const ufm_cbm_track_metrics_t* t){
...
static int score_sync_sensitive(const ufm_cbm_track_metrics_t* t){
...
static int score_weakbits(const ufm_cbm_track_metrics_t* t){
    if(!t) return 0;
...
static int score_halft_or_extra(const ufm_cbm_track_metrics_t* t, bool want_half){
    if(!t) return 0;
...
static int score_bitrate(const ufm_cbm_track_metrics_t* t){
    if(!t) return 0;
    return t->nonstandard_bitrate ? 70 : 0;
}

static int score_gap_sigs(const ufm_cbm_track_metrics_t* t){
    if(!t) return 0;
    int s = 0;
...
static int score_key_track_like(const ufm_cbm_track_metrics_t* t){
    if(!t) return 0;
...
static int score_track_sync(const ufm_cbm_track_metrics_t* t){
    if(!t) return 0;
...
bool ufm_cbm_prot_analyze(const ufm_cbm_track_metrics_t* tracks, size_t track_count,
    ...
    for(size_t i=0;i<track_count;i++){
        const ufm_cbm_track_metrics_t* t = &tracks[i];
        ...
        if(e){ any_errors=1; push_hit(hits, hits_cap, &n_hits, UFM_CBM_PROT_INTENTIONAL_ERRORS, t->track_x2, e); overall += 10 + e/12; flagged++; }
        if(lt){ any_longtrk=1; push_hit(hits, hits_cap, &n_hits, UFM_CBM_PROT_LONG_TRACKS, t->track_x2, lt); overall += 8 + lt/14; flagged++; }
        if(ss){ any_longsync=1; push_hit(hits, hits_cap, &n_hits, UFM_CBM_PROT_SYNC_COUNTING, t->track_x2, ss); overall += 6 + ss/18; flagged++; }
        if(wk){ any_weak=1; push_hit(hits, hits_cap, &n_hits, UFM_CBM_PROT_WEAK_BITS_UNFORMATTED, t->track_x2, wk); overall += 8 + wk/14; flagged++; }
        if(ht){ any_half=1; push_hit(hits, hits_cap, &n_hits, UFM_CBM_PROT_HALF_TRACKS, t->track_x2, ht); overall += 7 + ht/20; flagged++; }
        if(ex){ any_extra=1; push_hit(hits, hits_cap, &n_hits, UFM_CBM_PROT_EXTRA_TRACKS, t->track_x2, ex); overall += 7 + ex/20; flagged++; }
        if(br){ any_bitrate=1; push_hit(hits, hits_cap, &n_hits, UFM_CBM_PROT_CHANGED_BITRATES, t->track_x2, br); overall += 6 + br/20; flagged++; }
        if(gs){ any_gap=1; push_hit(hits, hits_cap, &n_hits, UFM_CBM_PROT_GAP_SIGNATURES, t->track_x2, gs); overall += 6 + gs/20; flagged++; }
        if(kt){ any_key=1; push_hit(hits, hits_cap, &n_hits, UFM_CBM_PROT_SIGNATURE_KEY_TRACKS, t->track_x2, kt); overall += 8 + kt/18; flagged++; }
        if(t->no_sync_detected){ any_nosync=1; push_hit(hits, hits_cap, &n_hits, UFM_CBM_PROT_NO_SYNC, t->track_x2, 80); overall += 10; flagged++; }
        if(ts){ any_tracksync=1; push_hit(hits, hits_cap, &n_hits, UFM_CBM_PROT_TRACK_SYNCHRONIZATION, t->track_x2, ts); overall += 10; flagged++; }
    }
    ...
    */
    if(any_tracksync){
        push_hit(hits, hits_cap, &n_hits, UFM_CBM_PROT_TRACK_SKEW, 0, 60);
        push_hit(hits, hits_cap, &n_hits, UFM_CBM_PROT_FAT_TRACKS, 0, 55);
        overall += 10;
    }

    if(any_longtrk && any_weak && any_gap == 0){
        push_hit(hits, hits_cap, &n_hits, UFM_CBM_PROT_CUSTOM_FORMATS, 0, 45);
    }

    if(any_half && any_tracksync){
        push_hit(hits, hits_cap, &n_hits, UFM_CBM_PROT_SPIRADISC_LIKE, 0, 55);
        overall += 8;
    }
