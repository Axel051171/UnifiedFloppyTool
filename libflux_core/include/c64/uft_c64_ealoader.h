 ...
 *
 * Build:
 *   cc -std=c11 -O2 -Wall -Wextra -pedantic -c uft_c64_ealoader.c
 */

#ifndef UFT_C64_EALOADER_H
#define UFT_C64_EALOADER_H
...
typedef enum uft_ea_p_addrmode {
    UFT_EA_P_IMPLIED = 0,
    UFT_EA_P_IMM8,
    UFT_EA_P_ABS16,
    UFT_EA_P_INDEXED /* "LDR indexed: R is index" */
} uft_ea_p_addrmode_t;

typedef struct uft_ea_p_insn {
    uft_ea_p_op_t op;
    uft_ea_p_addrmode_t mode;
    uint8_t size;          /* 1..3 */
} uft_ea_p_insn_t;

typedef struct uft_ea_p_state {
 ...
 */
typedef struct uft_ea_fattrack_observation {
