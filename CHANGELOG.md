...
| FM-Dekoder (`flux_decode_fm`) | ~~**vorhanden, nicht belegt** — echte PLL-Dekodierung, kein Stub~~ → **BERICHTIGT (MF-644): die Zeile war falsch.** Gemessen an `src/flux/uft_flux_decoder.c`: die Funktion findet Syncs, setzt `detected_encoding` und `avg_bitrate` — erhöht `sector_count` aber **kein einziges Mal** (0 Treffer in der ganzen Funktion). `return (track->sector_count > 0) ? FLUX_OK : FLUX_ERR_NO_SYNC;` kann deshalb **nur** den Fehler liefern. Im Rumpf steht wörtlich „FM sector decoding would go here". Es ist also sehr wohl ein Stub — einer mit Fassade, was schlimmer ist als ein offener. Folge: **Flux→ATR ist heute unmöglich**, und die Rundlauf-Matrix führt kein Atari-Paar |
