/**
 * @file test_do_write_roundtrip.c
 * @brief Apple DOS-order DO write_track -> read_track round-trip (Phase-6, MF-343).
 *
...
static void get_temp_path(char *path, size_t n) {
    ...
    if (!dir || !dir[0]) dir = ".";
    snprintf(path, n, "%s/uft_do_rt_%d.do", dir, rand() % 100000);
}
...
TEST(write_persists_to_read) {
    ...
    ASSERT(uft_format_plugin_do.open(&disk, path, false) == UFT_OK);
    ASSERT(disk.geometry.sector_size == SS);
    uft_track_t t; memset(&t, 0, sizeof(t));
    ASSERT(uft_format_plugin_do.read_track(&disk, 0, 0, &t) == UFT_OK);
    ...
    ASSERT(uft_format_plugin_do.write_track(&disk, 0, 0, &t) == UFT_OK);
    free_ts(&t);
    ...
    if (uft_format_plugin_do.close) uft_format_plugin_do.close(&disk);

    uft_disk_t disk2;
    memset(&disk2, 0, sizeof(disk2));
    disk2.read_only = true;
    ASSERT(uft_format_plugin_do.open(&disk2, path, true) == UFT_OK);

    ASSERT(uft_format_plugin_do.read_track(&disk2, 0, 0, &t2) == UFT_OK);
    ASSERT(t2.sectors[3].data[0] == 0xC7);
    ASSERT(t2.sectors[3].data[SS-1] == 0x5A);
    ASSERT(t2.sectors[2].data[0] == 0x00);
    free_ts(&t2);
    if (uft_format_plugin_do.close) uft_format_plugin_do.close(&disk2);
    remove(path);
}
int main(void) {
    printf("=== Apple DOS-order DO write round-trip (Phase-6) ===\n");
