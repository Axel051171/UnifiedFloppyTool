...
TEST(write_persists_to_read) {
    ...
    ASSERT(uft_format_plugin_adf.open(&disk2, path, true) == UFT_OK);

    ASSERT(uft_format_plugin_adf.read_track(&disk2, 1, 0, &t2) == UFT_OK);
    ...
    if (uft_format_plugin_adf.close) uft_format_plugin_adf.close(&disk2);
    remove(path);
}
