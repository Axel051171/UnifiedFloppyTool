...
TEST(write_persists_to_read) {
    ...
    ASSERT(uft_format_plugin_img.open(&disk2, path, true) == UFT_OK);

    ASSERT(uft_format_plugin_img.read_track(&disk2, 0, 0, &t2) == UFT_OK);
    ASSERT(t2.sectors[3].data[0] == 0xC7);
    ASSERT(t2.sectors[3].data[SS-1] == 0x5A);
    ASSERT(t2.sectors[2].data[0] == 0x00);
    free_ts(&t2);
    if (uft_format_plugin_img.close) uft_format_plugin_img.close(&disk2);
    remove(path);
}
int main(void) {
