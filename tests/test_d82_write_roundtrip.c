/**
 * @file test_d82_write_roundtrip.c
 * @brief Commodore D82 (8250) write_track -> read_track round-trip (Phase-6, MF-343).
 *
 * Builds a synthetic image, reads a track, modifies a sector, writes it back,
 * re-reads and asserts the change persisted (write is not a silent no-op) and
 * a neighbour sector stayed untouched.
 */
...
static void get_temp_path(char *path, size_t n) {
    ...
    if (!dir || !dir[0]) dir = ".";
    snprintf(path, n, "%s/uft_d82_rt_%d.d82", dir, rand() % 100000);
}
...
TEST(write_persists_to_read) {
    ...
    ASSERT(uft_format_plugin_d82.open(&disk, path, false) == UFT_OK);
    ASSERT(disk.geometry.sector_size == SS);
    uft_track_t t; memset(&t, 0, sizeof(t));
    ASSERT(uft_format_plugin_d82.read_track(&disk, 0, 0, &t) == UFT_OK);
    ...
    ASSERT(uft_format_plugin_d82.write_track(&disk, 0, 0, &t) == UFT_OK);
    free_ts(&t);
    ...
    if (uft_format_plugin_d82.close) uft_format_plugin_d82.close(&disk);

    uft_disk_t disk2;
    memset(&disk2, 0, sizeof(disk2));
    disk2.read_only = true;
    ASSERT(uft_format_plugin_d82.open(&disk2, path, true) == UFT_OK);

    ASSERT(uft_format_plugin_d82.read_track(&disk2, 0, 0, &t2) == UFT_OK);
    ASSERT(t2.sectors[3].data[0] == 0xC7);
    ASSERT(t2.sectors[3].data[SS-1] == 0x5A);
    ASSERT(t2.sectors[2].data[0] == 0x00);
    free_ts(&t2);
    if (uft_format_plugin_d82.close) uft_format_plugin_d82.close(&disk2);
    remove(path);
}
int main(void) {
    printf("=== Commodore D82 (8250) write round-trip (Phase-6) ===\n");
