#!/usr/bin/env python3
"""
diff.py — compare UFT's ADF-Copy constants against the official reference.
...
Verdict per key:
  PASS        UFT value == reference value (and reference is establishable)
  FAIL        UFT value != reference value
  UNVERIFIED  reference is needs-source (ADFC_CMD_* opcodes — header deleted)
    ...
    for k in keys:
        ...
        if r is None:
            rows.append({"key": k, "uft": uval, "ref": None,
                         ...
                         "uft_line": uline})
            continue
        if isinstance(r, dict):  # needs-source opcode
            rmatch = (uval == r.get("ref"))
            rows.append({"key": k, "uft": uval, "ref": r.get("ref"),
                         "verdict": "UNVERIFIED",
                         "detail": f"{r.get('kind')} — opcode value only in UFT "
                                   f"comments (adfcopyhardwareprovider.h deleted); "
                                   f"numeric {'matches' if rmatch else 'DIFFERS from'} "
                                   f"recalled value but not independently checkable",
                         "uft_line": uline})
            continue
...
def main():
    ...
    sections = {}
    for name in ("responses", "timing", "geometry", "cmd_opcodes", "usb_ids"):
    ...
    print(f"diff.py: adfcopy — wrote {EVIDENCE.name}")
    ...
    if fails:
        ...
        return 1
    print("OK — no FAIL (UNVERIFIED rows: ADFC_CMD_* opcodes need a vendored "
          "firmware header; authoritative source was deleted by MF-169)")
    return 0
