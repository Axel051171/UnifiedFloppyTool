...
} __attribute__((packed)) ipf_header_t;
...
typedef struct ipf_track_info {
    ...
    uint32_t    data_bits;
} __attribute__((packed)) ipf_track_info_t;
...
typedef struct stx_header {
    ...
    uint8_t     sides;
    uint32_t    reserved2;
} __attribute__((packed)) stx_header_t;
...
} __attribute__((packed)) stx_track_header_t;
...
typedef struct td0_header {
    ...
    uint8_t     sides;
    uint16_t    crc;
} __attribute__((packed)) td0_header_t;
...
typedef struct td0_track_header {
    ...
    uint8_t     head;
    uint8_t     crc;
} __attribute__((packed)) td0_track_header_t;
...
} __attribute__((packed)) imd_sector_header_t;
...
typedef struct woz_header {
    ...
    uint32_t    crc32;
} __attribute__((packed)) woz_header_t;
...
typedef struct woz_info_chunk {
    ...
    uint8_t     cleaned;
    char        creator[32];
} __attribute__((packed)) woz_info_chunk_t;
...
typedef struct a2r_header {
    ...
    uint8_t     high_bit;           // 0xFF
    uint8_t     lfcrlf[3];
} __attribute__((packed)) a2r_header_t;
...
} __attribute__((packed)) fdi_header_t;
