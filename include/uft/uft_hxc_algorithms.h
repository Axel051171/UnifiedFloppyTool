/**
 * @file uft_hxc_algorithms.h
 * @brief HxC Floppy Emulator Algorithms Integration for UFT
 * 
 * This file contains algorithms and data structures extracted from the
 * HxC Floppy Emulator library (libhxcfe) by Jean-François DEL NERO.
 * 
 * Original HxC Floppy Emulator: http://hxc2001.free.fr
...
#ifndef UFT_HXC_ALGORITHMS_H
#define UFT_HXC_ALGORITHMS_H
...
#define UFT_HXC_VALIDFILE                  1
#define UFT_HXC_NOERROR                    0
#define UFT_HXC_ACCESSERROR               -1
#define UFT_HXC_BADFILE                   -2
#define UFT_HXC_FILECORRUPTED             -3
#define UFT_HXC_BADPARAMETER              -4
#define UFT_HXC_INTERNALERROR             -5
#define UFT_HXC_UNSUPPORTEDFILE           -6
 ...
 */
typedef struct uft_hxc_pll_state {
 ...
 */
#define UFT_HXC_DEFAULT_MAXPULSESKEW     25      /* in per-256 */
#define UFT_HXC_DEFAULT_BLOCK_TIME       1000    /* in uS */
#define UFT_HXC_DEFAULT_SEARCHDEPTH      0.025
#define UFT_HXC_DEFAULT_PLL_PERCENT      10      /* +/- 10% */
#define UFT_HXC_DEFAULT_FAST_RATIO_N     7
#define UFT_HXC_DEFAULT_FAST_RATIO_D     8
#define UFT_HXC_DEFAULT_SLOW_RATIO_N     7
#define UFT_HXC_DEFAULT_SLOW_RATIO_D     8
#define UFT_HXC_DEFAULT_MAX_ERROR_TICKS  100

/* Internal stream tick frequency */
#define UFT_HXC_TICKFREQ 250000000  /* 250MHz tick */

/**
 * Initialize HxC-style PLL
 */
static inline void uft_hxc_pll_init(uft_hxc_pll_state_t *pll, 
                                     int tick_freq, 
                                     int bitcell_ticks)
{
    memset(pll, 0, sizeof(uft_hxc_pll_state_t));
    
    pll->tick_freq = tick_freq;
    pll->pump_charge = (bitcell_ticks * 16) / 2;
    pll->phase = 0;
    pll->pivot = bitcell_ticks * 16;
    pll->pll_min_max_percent = UFT_HXC_DEFAULT_PLL_PERCENT;
    ...
    pll->fast_correction_ratio_n = UFT_HXC_DEFAULT_FAST_RATIO_N;
    pll->fast_correction_ratio_d = UFT_HXC_DEFAULT_FAST_RATIO_D;
    pll->slow_correction_ratio_n = UFT_HXC_DEFAULT_SLOW_RATIO_N;
    pll->slow_correction_ratio_d = UFT_HXC_DEFAULT_SLOW_RATIO_D;
    
    pll->max_pll_error_ticks = UFT_HXC_DEFAULT_MAX_ERROR_TICKS;
 ...
 */
static inline int uft_hxc_get_cell_timing(uft_hxc_pll_state_t *pll,
 ...
 */
static const uint16_t UFT_HXC_LUT_Byte2MFM[256] = {
 ...
 */
static const uint16_t UFT_HXC_LUT_Byte2MFMClkMask[256] = {
 ...
 */
static const uint8_t UFT_HXC_LUT_Byte2EvenBits[256] = {
 ...
 */
static const uint8_t UFT_HXC_LUT_Byte2OddBits[256] = {
 ...
 * Bit reversal table
 */
static const uint8_t UFT_HXC_LUT_ByteBitsInverter[256] = {
 ...
 */
static const uint8_t UFT_HXC_GCR_ENCODE[16] = {
    0x0A, 0x0B, 0x12, 0x13,
    0x0E, 0x0F, 0x16, 0x17,
    0x09, 0x19, 0x1A, 0x1B,
    0x0D, 0x1D, 0x1E, 0x15
 ...
 */
static const uint8_t UFT_HXC_GCR_DECODE[32] = {
 ...
 */
static const uint8_t UFT_HXC_APPLE2_GCR6_ENCODE[64] = {
 ...
 */
static const uint8_t UFT_HXC_APPLE2_GCR6_DECODE[256] = {
...
static inline uint16_t uft_hxc_crc16_gen_entry(uint16_t index, int num_bits, uint16_t poly)
{
...
static inline void uft_hxc_crc16_init(uft_hxc_crc16_t *crc, uint16_t poly, uint16_t init)
{
    for (int i = 0; i < 16; i++) {
        uint16_t te = uft_hxc_crc16_gen_entry(i, 4, poly);
...
static inline void uft_hxc_crc16_update4(uft_hxc_crc16_t *crc, uint8_t val)
{
...
static inline void uft_hxc_crc16_update(uft_hxc_crc16_t *crc, uint8_t val)
{
    uft_hxc_crc16_update4(crc, val >> 4);
    uft_hxc_crc16_update4(crc, val & 0x0F);
}

static inline uint16_t uft_hxc_crc16_get(uft_hxc_crc16_t *crc)
{
...
typedef struct {
    ...
    uint16_t floppy_rpm;            /* 300 or 360 */
...
/* KryoFlux Constants */
#define UFT_KF_OOB_SIGN             0x0D
#define UFT_KF_OOBTYPE_STREAM_READ  0x01
#define UFT_KF_OOBTYPE_INDEX        0x02
#define UFT_KF_OOBTYPE_STREAM_END   0x03
#define UFT_KF_OOBTYPE_STRING       0x04
#define UFT_KF_OOBTYPE_END          0x0D

#define UFT_KF_DEFAULT_MCLOCK       48054857.14285714  /* (((18432000*73)/14)/2) */
#define UFT_KF_DEFAULT_SCLOCK       (UFT_KF_DEFAULT_MCLOCK / 2)  /* ~41.619ns */
 ...
 */
static const int UFT_HXC_VICTOR9K_BANDS[][8] = {
...
static const uft_gap3_config_t UFT_HXC_STD_GAP3_TABLE[] = {
    /* DD 5.25" */
